/**
 * SenseMatchPanel (formerly DnaPanel)
 *
 * Shows the user's Amazon Sense preference profile on the Account page.
 */

import React from "react";
import { Brain, Leaf, TrendingUp, RotateCcw, Wallet, Sprout, Flame } from "lucide-react";
import { useSense } from "../contexts/SenseContext.jsx";

const STAGE_CONFIG = {
  seedling:    { Icon: Sprout,     label: "Learning",     color: "text-green-600",  bg: "bg-green-50",  desc: "Keep browsing and buying — Amazon Sense is still learning your preferences." },
  growing:     { Icon: TrendingUp, label: "Growing",      color: "text-blue-600",   bg: "bg-blue-50",   desc: "Your profile is taking shape. Patterns are starting to emerge." },
  established: { Icon: Flame,      label: "Established",  color: "text-orange-600", bg: "bg-orange-50", desc: "Your profile is mature. Match scores and recommendations are most accurate." },
};
...
export default function SenseMatchPanel() {
  const { profile, senseReady } = useSense();
    ...
    return (
      <div className="bg-white border border-[#DDD] rounded p-5">
        <div className="flex items-center gap-2 mb-3">
          <Brain size={18} className="text-[#FF9900]" />
          <h2 className="font-bold text-[#0F1111] text-base">Amazon Sense™</h2>
        ...
        <div className="flex items-start gap-3 p-4 bg-[#FAFAFA] rounded-xl border border-dashed border-gray-300">
          ...
          <div>
            <p className="text-sm font-medium text-[#0F1111] mb-1">Amazon Sense is still learning your preferences.</p>
            <p className="text-xs text-[#565959]">
              Browse and buy a few products — Amazon Sense learns your preferences, spending patterns,
              and shopping behavior to predict how likely you are to love a product.
            </p>
  ...
  return (
    <div className="bg-white border border-[#DDD] rounded p-5">
        ...
        {profile.budgetRange?.avg > 0 && (
          <div>
            <div className="flex items-center gap-1.5 mb-3">
              ...
              <span className="text-xs font-semibold text-[#0F1111] uppercase tracking-wide">Your Spending Pattern</span>
            </div>
      ...
      <p className="text-[10px] text-gray-400 mt-5 pt-4 border-t border-[#F0F0F0]">
        Amazon Sense learns from your views, cart adds, purchases, and returns to predict product affinity.
