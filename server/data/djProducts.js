// AUTO-GENERATED — do not edit by hand. Re-run scripts/generateDJProducts.mjs to regenerate.
// DummyJSON products migrated to static data with enriched features and specs.
// Total: 184 products

export const djProducts = [
  {
    "id": "dj1",
    "name": "Essence Mascara Lash Princess",
    "brand": "Essence",
    "category": "Beauty",
    "price": 829,
    "originalPrice": 921,
    "discount": 10,
    "rating": 2.6,
    "reviewCount": 31893,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Day after Tomorrow",
    "deliveryFree": true,
    "trustScore": 78,
    "trustLabel": "Genuine",
    "buyNowOrWait": "buy",
    "waitReason": null,
    "spikePriceMonths": [],
    "priceHistory": [
      829,
      829,
      789,
      829,
      829,
      829,
      829,
      829,
      829,
      829,
      777,
      829
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/beauty/essence-mascara-lash-princess/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/beauty/essence-mascara-lash-princess/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/beauty/essence-mascara-lash-princess/1.webp"
    ],
    "description": "The Essence Mascara Lash Princess is a popular mascara known for its volumizing and lengthening effects. Achieve dramatic lashes with this long-lasting and cruelty-free formula.",
    "features": [
      "The Essence Mascara Lash Princess is a popular mascara known for its volumizing and lengthening effects",
      "Hypoallergenic formula — safe for sensitive skin",
      "Up to 16-hour colour retention — looks fresh all day",
      "Paraben-free — ethically produced",
      "Multiple finishes available"
    ],
    "specs": {
      "Volume": "4000 ml",
      "Skin Type": "Dry to Normal",
      "Key Ingredients": "Ceramides, Peptides",
      "Free From": "Alcohol, Non-comedogenic",
      "Shelf Life": "24 months",
      "Finish": "Satin"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Essence",
    "soldByRating": 4.4,
    "sellerSince": "2020",
    "fulfillment": "Fulfilled by Amazon",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 86,
        "detail": null
      },
      "returnRate": {
        "score": 83,
        "detail": null
      },
      "warrantyClaims": {
        "score": 79,
        "detail": null
      },
      "sellerReliability": {
        "score": 91,
        "detail": null
      },
      "priceStability": {
        "score": 85,
        "detail": null
      }
    }
  },
  {
    "id": "dj2",
    "name": "Eyeshadow Palette with Mirror",
    "brand": "Glamour Beauty",
    "category": "Beauty",
    "price": 1659,
    "originalPrice": 2023,
    "discount": 18,
    "rating": 2.9,
    "reviewCount": 49887,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Tomorrow, 6 PM",
    "deliveryFree": true,
    "trustScore": 79,
    "trustLabel": "Genuine",
    "buyNowOrWait": "wait",
    "waitReason": "Sale expected in the next 2 weeks based on historical patterns.",
    "spikePriceMonths": [],
    "priceHistory": [
      1659,
      1659,
      1659,
      1659,
      1659,
      1660,
      1659,
      1659,
      1659,
      1659,
      1659,
      1659
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/beauty/eyeshadow-palette-with-mirror/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/beauty/eyeshadow-palette-with-mirror/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/beauty/eyeshadow-palette-with-mirror/1.webp"
    ],
    "description": "The Eyeshadow Palette with Mirror offers a versatile range of eyeshadow shades for creating stunning eye looks. With a built-in mirror, it's convenient for on-the-go makeup application.",
    "features": [
      "The Eyeshadow Palette with Mirror offers a versatile range of eyeshadow shades for creating stunning eye looks",
      "Dermatologist-tested formula — safe for sensitive skin",
      "24-hour moisture lock — looks fresh all day",
      "Sulphate-free — ethically produced",
      "Multiple finishes available"
    ],
    "specs": {
      "Volume": "9000 ml",
      "Skin Type": "Oily to Combination",
      "Key Ingredients": "Vitamin E, Aloe Vera",
      "Free From": "Alcohol, Non-comedogenic",
      "Shelf Life": "18 months",
      "Finish": "Dewy/Luminous"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Glamour Beauty",
    "soldByRating": 4.1,
    "sellerSince": "2021",
    "fulfillment": "Seller fulfilled",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 91,
        "detail": null
      },
      "returnRate": {
        "score": 85,
        "detail": null
      },
      "warrantyClaims": {
        "score": 79,
        "detail": null
      },
      "sellerReliability": {
        "score": 90,
        "detail": null
      },
      "priceStability": {
        "score": 71,
        "detail": null
      }
    }
  },
  {
    "id": "dj3",
    "name": "Powder Canister",
    "brand": "Velvet Touch",
    "category": "Beauty",
    "price": 1244,
    "originalPrice": 1382,
    "discount": 10,
    "rating": 4.6,
    "reviewCount": 61618,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Tomorrow, 6 PM",
    "deliveryFree": true,
    "trustScore": 88,
    "trustLabel": "Genuine",
    "buyNowOrWait": "wait",
    "waitReason": "Sale expected in the next 2 weeks based on historical patterns.",
    "spikePriceMonths": [
      7
    ],
    "priceHistory": [
      1244,
      1244,
      1244,
      1244,
      1244,
      1244,
      1244,
      1586,
      1244,
      1244,
      1330,
      1244
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/beauty/powder-canister/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/beauty/powder-canister/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/beauty/powder-canister/1.webp"
    ],
    "description": "The Powder Canister is a finely milled setting powder designed to set makeup and control shine. With a lightweight and translucent formula, it provides a smooth and matte finish.",
    "features": [
      "The Powder Canister is a finely milled setting powder designed to set makeup and control shine",
      "Ophthalmologist-tested formula — safe for sensitive skin",
      "24-hour moisture lock — looks fresh all day",
      "No harmful chemicals — ethically produced",
      "Comes in 8 versatile shades"
    ],
    "specs": {
      "Volume": "8000 ml",
      "Skin Type": "Sensitive",
      "Key Ingredients": "Kojic Acid, Glycerin",
      "Free From": "Mineral Oil, SLS",
      "Shelf Life": "18 months",
      "Finish": "Matte"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Velvet Touch",
    "soldByRating": 4.8,
    "sellerSince": "2016",
    "fulfillment": "Fulfilled by Amazon",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 76,
        "detail": null
      },
      "returnRate": {
        "score": 91,
        "detail": null
      },
      "warrantyClaims": {
        "score": 80,
        "detail": null
      },
      "sellerReliability": {
        "score": 96,
        "detail": null
      },
      "priceStability": {
        "score": 91,
        "detail": null
      }
    }
  },
  {
    "id": "dj4",
    "name": "Red Lipstick",
    "brand": "Chic Cosmetics",
    "category": "Beauty",
    "price": 1078,
    "originalPrice": 1225,
    "discount": 12,
    "rating": 4.4,
    "reviewCount": 46054,
    "inStock": true,
    "isPrime": false,
    "delivery": "Get it by Tomorrow, 6 PM",
    "deliveryFree": true,
    "trustScore": 95,
    "trustLabel": "Genuine",
    "buyNowOrWait": "wait",
    "waitReason": "Sale expected in the next 2 weeks based on historical patterns.",
    "spikePriceMonths": [
      4,
      8,
      9,
      10
    ],
    "priceHistory": [
      1078,
      1078,
      1078,
      1018,
      1264,
      1078,
      1078,
      1078,
      1359,
      1265,
      1309,
      1078
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/beauty/red-lipstick/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/beauty/red-lipstick/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/beauty/red-lipstick/1.webp"
    ],
    "description": "The Red Lipstick is a classic and bold choice for adding a pop of color to your lips. With a creamy and pigmented formula, it provides a vibrant and long-lasting finish.",
    "features": [
      "The Red Lipstick is a classic and bold choice for adding a pop of color to your lips",
      "Clinically proven formula — safe for sensitive skin",
      "Up to 16-hour colour retention — looks fresh all day",
      "Sulphate-free — ethically produced",
      "Available in 12+ shades"
    ],
    "specs": {
      "Volume": "1000 ml",
      "Skin Type": "Dry to Normal",
      "Key Ingredients": "Vitamin E, Aloe Vera",
      "Free From": "Paraben, Sulphate, Fragrance",
      "Shelf Life": "18 months",
      "Finish": "Matte"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Chic Cosmetics",
    "soldByRating": 4,
    "sellerSince": "2019",
    "fulfillment": "Seller fulfilled",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 91,
        "detail": null
      },
      "returnRate": {
        "score": 96,
        "detail": null
      },
      "warrantyClaims": {
        "score": 96,
        "detail": null
      },
      "sellerReliability": {
        "score": 96,
        "detail": null
      },
      "priceStability": {
        "score": 86,
        "detail": null
      }
    }
  },
  {
    "id": "dj5",
    "name": "Red Nail Polish",
    "brand": "Nail Couture",
    "category": "Beauty",
    "price": 746,
    "originalPrice": 838,
    "discount": 11,
    "rating": 4.3,
    "reviewCount": 26110,
    "inStock": true,
    "isPrime": false,
    "delivery": "Get it by Tomorrow, 6 PM",
    "deliveryFree": true,
    "trustScore": 43,
    "trustLabel": "Suspicious",
    "buyNowOrWait": "wait",
    "waitReason": "Sale expected in the next 2 weeks based on historical patterns.",
    "spikePriceMonths": [
      2
    ],
    "priceHistory": [
      746,
      746,
      937,
      746,
      746,
      746,
      746,
      746,
      746,
      746,
      832,
      746
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/beauty/red-nail-polish/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/beauty/red-nail-polish/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/beauty/red-nail-polish/1.webp"
    ],
    "description": "The Red Nail Polish offers a rich and glossy red hue for vibrant and polished nails. With a quick-drying formula, it provides a salon-quality finish at home.",
    "features": [
      "The Red Nail Polish offers a rich and glossy red hue for vibrant and polished nails",
      "Dermatologist-tested formula — safe for sensitive skin",
      "12-hour fragrance — looks fresh all day",
      "Vegan-friendly — ethically produced",
      "Available in 12+ shades"
    ],
    "specs": {
      "Volume": "8000 ml",
      "Skin Type": "Mature",
      "Key Ingredients": "Vitamin C, Retinol",
      "Free From": "Paraben, Sulphate, Fragrance",
      "Shelf Life": "18 months",
      "Finish": "Matte"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Nail Couture",
    "soldByRating": 2.9,
    "sellerSince": "2023",
    "fulfillment": "Seller fulfilled",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 39,
        "detail": null
      },
      "returnRate": {
        "score": 35,
        "detail": null
      },
      "warrantyClaims": {
        "score": 49,
        "detail": null
      },
      "sellerReliability": {
        "score": 59,
        "detail": null
      },
      "priceStability": {
        "score": 37,
        "detail": null
      }
    }
  },
  {
    "id": "dj6",
    "name": "Calvin Klein CK One",
    "brand": "Calvin Klein",
    "category": "Beauty",
    "price": 4149,
    "originalPrice": 4234,
    "discount": 2,
    "rating": 4.4,
    "reviewCount": 6088,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Tomorrow, 6 PM",
    "deliveryFree": true,
    "trustScore": 93,
    "trustLabel": "Genuine",
    "buyNowOrWait": "buy",
    "waitReason": null,
    "spikePriceMonths": [],
    "priceHistory": [
      4331,
      3785,
      4149,
      4149,
      4149,
      4008,
      4149,
      4149,
      4149,
      4149,
      4149,
      4149
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/fragrances/calvin-klein-ck-one/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/fragrances/calvin-klein-ck-one/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/fragrances/calvin-klein-ck-one/1.webp",
      "https://cdn.dummyjson.com/product-images/fragrances/calvin-klein-ck-one/2.webp"
    ],
    "description": "CK One by Calvin Klein is a classic unisex fragrance, known for its fresh and clean scent. It's a versatile fragrance suitable for everyday wear.",
    "features": [
      "CK One by Calvin Klein is a classic unisex fragrance, known for its fresh and clean scent",
      "Top notes: mandarin & grapefruit",
      "Heart notes: neroli & gardenia",
      "Long-lasting 8–10 hour projection on skin",
      "Parfum Intense concentration — ideal for daily wear"
    ],
    "specs": {
      "Volume": "7000 ml",
      "Concentration": "Parfum Intense",
      "Top Notes": "Bergamot & Lemon",
      "Heart Notes": "Neroli & Gardenia",
      "Base Notes": "Vetiver & Tonka",
      "Longevity": "12 hours",
      "Gender": "Women's Fragrance"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Calvin Klein",
    "soldByRating": 4.9,
    "sellerSince": "2010",
    "fulfillment": "Fulfilled by Amazon",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 95,
        "detail": null
      },
      "returnRate": {
        "score": 96,
        "detail": null
      },
      "warrantyClaims": {
        "score": 94,
        "detail": null
      },
      "sellerReliability": {
        "score": 96,
        "detail": null
      },
      "priceStability": {
        "score": 87,
        "detail": null
      }
    }
  },
  {
    "id": "dj7",
    "name": "Chanel Coco Noir Eau De",
    "brand": "Chanel",
    "category": "Beauty",
    "price": 10789,
    "originalPrice": 12999,
    "discount": 17,
    "rating": 4.3,
    "reviewCount": 9292,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Tomorrow, 6 PM",
    "deliveryFree": true,
    "trustScore": 88,
    "trustLabel": "Genuine",
    "buyNowOrWait": "buy",
    "waitReason": null,
    "spikePriceMonths": [],
    "priceHistory": [
      10789,
      11977,
      10789,
      10789,
      10789,
      10789,
      10789,
      10789,
      10789,
      12051,
      10789,
      10789
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/fragrances/chanel-coco-noir-eau-de/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/fragrances/chanel-coco-noir-eau-de/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/fragrances/chanel-coco-noir-eau-de/1.webp",
      "https://cdn.dummyjson.com/product-images/fragrances/chanel-coco-noir-eau-de/2.webp"
    ],
    "description": "Coco Noir by Chanel is an elegant and mysterious fragrance, featuring notes of grapefruit, rose, and sandalwood. Perfect for evening occasions.",
    "features": [
      "Coco Noir by Chanel is an elegant and mysterious fragrance, featuring notes of grapefruit, rose, and sandalwood",
      "Top notes: rose & jasmine",
      "Heart notes: cedarwood & musk",
      "Long-lasting 8–10 hour projection on skin",
      "Body Mist concentration — ideal for daily wear"
    ],
    "specs": {
      "Volume": "7000 ml",
      "Concentration": "Eau de Parfum (EDP)",
      "Top Notes": "Bergamot & Lemon",
      "Heart Notes": "Patchouli & Vetiver",
      "Base Notes": "Vetiver & Tonka",
      "Longevity": "6 hours",
      "Gender": "Unisex"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Chanel",
    "soldByRating": 3.9,
    "sellerSince": "2012",
    "fulfillment": "Seller fulfilled",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 93,
        "detail": null
      },
      "returnRate": {
        "score": 93,
        "detail": null
      },
      "warrantyClaims": {
        "score": 94,
        "detail": null
      },
      "sellerReliability": {
        "score": 95,
        "detail": null
      },
      "priceStability": {
        "score": 89,
        "detail": null
      }
    }
  },
  {
    "id": "dj8",
    "name": "Dior J'adore",
    "brand": "Dior",
    "category": "Beauty",
    "price": 7469,
    "originalPrice": 8787,
    "discount": 15,
    "rating": 3.8,
    "reviewCount": 31214,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Day after Tomorrow",
    "deliveryFree": true,
    "trustScore": 90,
    "trustLabel": "Genuine",
    "buyNowOrWait": "wait",
    "waitReason": "Sale expected in the next 2 weeks based on historical patterns.",
    "spikePriceMonths": [
      4
    ],
    "priceHistory": [
      7469,
      7469,
      7469,
      7469,
      9506,
      7469,
      7469,
      7469,
      7469,
      7469,
      7469,
      7469
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/fragrances/dior-j'adore/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/fragrances/dior-j'adore/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/fragrances/dior-j'adore/1.webp",
      "https://cdn.dummyjson.com/product-images/fragrances/dior-j'adore/2.webp"
    ],
    "description": "J'adore by Dior is a luxurious and floral fragrance, known for its blend of ylang-ylang, rose, and jasmine. It embodies femininity and sophistication.",
    "features": [
      "J'adore by Dior is a luxurious and floral fragrance, known for its blend of ylang-ylang, rose, and jasmine",
      "Top notes: violet & iris",
      "Heart notes: cedarwood & musk",
      "All-day 12-hour projection on skin",
      "Body Mist concentration — ideal for daily wear"
    ],
    "specs": {
      "Volume": "4000 ml",
      "Concentration": "Eau de Parfum (EDP)",
      "Top Notes": "Rose & Jasmine",
      "Heart Notes": "Patchouli & Vetiver",
      "Base Notes": "Sandalwood & Oakmoss",
      "Longevity": "8 hours",
      "Gender": "Unisex"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Dior",
    "soldByRating": 4,
    "sellerSince": "2013",
    "fulfillment": "Seller fulfilled",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 95,
        "detail": null
      },
      "returnRate": {
        "score": 81,
        "detail": null
      },
      "warrantyClaims": {
        "score": 92,
        "detail": null
      },
      "sellerReliability": {
        "score": 96,
        "detail": null
      },
      "priceStability": {
        "score": 86,
        "detail": null
      }
    }
  },
  {
    "id": "dj9",
    "name": "Dolce Shine Eau de",
    "brand": "Dolce & Gabbana",
    "category": "Beauty",
    "price": 5809,
    "originalPrice": 5868,
    "discount": 1,
    "rating": 4,
    "reviewCount": 56500,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Tomorrow, 6 PM",
    "deliveryFree": true,
    "trustScore": 91,
    "trustLabel": "Genuine",
    "buyNowOrWait": "wait",
    "waitReason": "Sale expected in the next 2 weeks based on historical patterns.",
    "spikePriceMonths": [],
    "priceHistory": [
      5249,
      5809,
      5809,
      5809,
      5510,
      5273,
      5809,
      5809,
      5809,
      5809,
      5809,
      5809
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/fragrances/dolce-shine-eau-de/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/fragrances/dolce-shine-eau-de/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/fragrances/dolce-shine-eau-de/1.webp",
      "https://cdn.dummyjson.com/product-images/fragrances/dolce-shine-eau-de/2.webp"
    ],
    "description": "Dolce Shine by Dolce & Gabbana is a vibrant and fruity fragrance, featuring notes of mango, jasmine, and blonde woods. It's a joyful and youthful scent.",
    "features": [
      "Dolce Shine by Dolce & Gabbana is a vibrant and fruity fragrance, featuring notes of mango, jasmine, and blonde woods",
      "Top notes: mandarin & grapefruit",
      "Heart notes: patchouli & vetiver",
      "Moderate 4–6 hour projection on skin",
      "Eau de Toilette (EDT) concentration — ideal for daily wear"
    ],
    "specs": {
      "Volume": "6000 ml",
      "Concentration": "Eau Fraîche",
      "Top Notes": "Violet & Iris",
      "Heart Notes": "Amber & Vanilla",
      "Base Notes": "Vetiver & Tonka",
      "Longevity": "8 hours",
      "Gender": "Men's Fragrance"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Dolce & Gabbana",
    "soldByRating": 4.6,
    "sellerSince": "2021",
    "fulfillment": "Seller fulfilled",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 77,
        "detail": null
      },
      "returnRate": {
        "score": 84,
        "detail": null
      },
      "warrantyClaims": {
        "score": 96,
        "detail": null
      },
      "sellerReliability": {
        "score": 91,
        "detail": null
      },
      "priceStability": {
        "score": 91,
        "detail": null
      }
    }
  },
  {
    "id": "dj10",
    "name": "Gucci Bloom Eau de",
    "brand": "Gucci",
    "category": "Beauty",
    "price": 6639,
    "originalPrice": 7720,
    "discount": 14,
    "rating": 2.7,
    "reviewCount": 12478,
    "inStock": true,
    "isPrime": false,
    "delivery": "Get it by Tomorrow, 6 PM",
    "deliveryFree": true,
    "trustScore": 76,
    "trustLabel": "Genuine",
    "buyNowOrWait": "wait",
    "waitReason": "Sale expected in the next 2 weeks based on historical patterns.",
    "spikePriceMonths": [
      2,
      3,
      7
    ],
    "priceHistory": [
      6639,
      6639,
      7491,
      8917,
      6639,
      7060,
      6639,
      7557,
      7184,
      6639,
      6639,
      6639
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/fragrances/gucci-bloom-eau-de/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/fragrances/gucci-bloom-eau-de/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/fragrances/gucci-bloom-eau-de/1.webp",
      "https://cdn.dummyjson.com/product-images/fragrances/gucci-bloom-eau-de/2.webp"
    ],
    "description": "Gucci Bloom by Gucci is a floral and captivating fragrance, with notes of tuberose, jasmine, and Rangoon creeper. It's a modern and romantic scent.",
    "features": [
      "Gucci Bloom by Gucci is a floral and captivating fragrance, with notes of tuberose, jasmine, and Rangoon creeper",
      "Top notes: rose & jasmine",
      "Heart notes: neroli & gardenia",
      "Long-lasting 8–10 hour projection on skin",
      "Eau de Parfum (EDP) concentration — ideal for daily wear"
    ],
    "specs": {
      "Volume": "7000 ml",
      "Concentration": "Parfum Intense",
      "Top Notes": "Bergamot & Lemon",
      "Heart Notes": "Cedarwood & Musk",
      "Base Notes": "Vetiver & Tonka",
      "Longevity": "10 hours",
      "Gender": "Men's Fragrance"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Gucci",
    "soldByRating": 4.2,
    "sellerSince": "2011",
    "fulfillment": "Fulfilled by Amazon",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 69,
        "detail": null
      },
      "returnRate": {
        "score": 84,
        "detail": null
      },
      "warrantyClaims": {
        "score": 76,
        "detail": null
      },
      "sellerReliability": {
        "score": 92,
        "detail": null
      },
      "priceStability": {
        "score": 69,
        "detail": null
      }
    }
  },
  {
    "id": "dj11",
    "name": "Annibale Colombo Bed",
    "brand": "Annibale Colombo",
    "category": "Home & Kitchen",
    "price": 157699,
    "originalPrice": 173296,
    "discount": 9,
    "rating": 4.8,
    "reviewCount": 43577,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Day after Tomorrow",
    "deliveryFree": true,
    "trustScore": 93,
    "trustLabel": "Genuine",
    "buyNowOrWait": "buy",
    "waitReason": null,
    "spikePriceMonths": [
      3,
      4
    ],
    "priceHistory": [
      176356,
      157699,
      157699,
      182760,
      181949,
      159435,
      157699,
      157699,
      157699,
      171415,
      157699,
      157699
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/furniture/annibale-colombo-bed/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/furniture/annibale-colombo-bed/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/furniture/annibale-colombo-bed/1.webp",
      "https://cdn.dummyjson.com/product-images/furniture/annibale-colombo-bed/2.webp"
    ],
    "description": "The Annibale Colombo Bed is a luxurious and elegant bed frame, crafted with high-quality materials for a comfortable and stylish bedroom.",
    "features": [
      "The Annibale Colombo Bed is a luxurious and elegant bed frame, crafted with high-quality materials for a comfortable and stylish bedroom",
      "Frame constructed from powder-coated steel for long-term durability",
      "Dimensions: 28W × 25H × 17D cm",
      "Weight capacity: 80 kg — built to handle everyday household use",
      "Delivered pre-assembled — complete setup in under 20 minutes"
    ],
    "specs": {
      "Material": "Steel Frame",
      "Dimensions": "28W × 25H × 17D cm",
      "Weight": "10 kg",
      "Weight Capacity": "80 kg",
      "Finish": "Teak Finish",
      "Assembly": "Pre-assembled",
      "Warranty": "1 year warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Annibale Colombo",
    "soldByRating": 4.4,
    "sellerSince": "2016",
    "fulfillment": "Seller fulfilled",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 86,
        "detail": null
      },
      "returnRate": {
        "score": 86,
        "detail": null
      },
      "warrantyClaims": {
        "score": 96,
        "detail": null
      },
      "sellerReliability": {
        "score": 96,
        "detail": null
      },
      "priceStability": {
        "score": 96,
        "detail": null
      }
    }
  },
  {
    "id": "dj12",
    "name": "Annibale Colombo Sofa",
    "brand": "Annibale Colombo",
    "category": "Home & Kitchen",
    "price": 207499,
    "originalPrice": 241278,
    "discount": 14,
    "rating": 3.9,
    "reviewCount": 44991,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Day after Tomorrow",
    "deliveryFree": true,
    "trustScore": 84,
    "trustLabel": "Genuine",
    "buyNowOrWait": "buy",
    "waitReason": null,
    "spikePriceMonths": [
      1,
      8
    ],
    "priceHistory": [
      207499,
      233325,
      207499,
      207499,
      207499,
      207499,
      203104,
      207499,
      281355,
      207499,
      207499,
      207499
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/furniture/annibale-colombo-sofa/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/furniture/annibale-colombo-sofa/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/furniture/annibale-colombo-sofa/1.webp",
      "https://cdn.dummyjson.com/product-images/furniture/annibale-colombo-sofa/2.webp"
    ],
    "description": "The Annibale Colombo Sofa is a sophisticated and comfortable seating option, featuring exquisite design and premium upholstery for your living room.",
    "features": [
      "The Annibale Colombo Sofa is a sophisticated and comfortable seating option, featuring exquisite design and premium upholstery for your living room",
      "Frame constructed from engineered hardwood for long-term durability",
      "Dimensions: 13W × 21H × 19D cm",
      "Weight capacity: 80 kg — built to handle everyday household use",
      "Delivered pre-assembled — complete setup in under 60 minutes"
    ],
    "specs": {
      "Material": "Engineered Wood (MDF)",
      "Dimensions": "13W × 21H × 19D cm",
      "Weight": "6 kg",
      "Weight Capacity": "80 kg",
      "Finish": "Teak Finish",
      "Assembly": "Flat-pack with instructions",
      "Warranty": "Lifetime warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Annibale Colombo",
    "soldByRating": 4.8,
    "sellerSince": "2016",
    "fulfillment": "Seller fulfilled",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 89,
        "detail": null
      },
      "returnRate": {
        "score": 93,
        "detail": null
      },
      "warrantyClaims": {
        "score": 86,
        "detail": null
      },
      "sellerReliability": {
        "score": 85,
        "detail": null
      },
      "priceStability": {
        "score": 76,
        "detail": null
      }
    }
  },
  {
    "id": "dj13",
    "name": "Bedside Table African Cherry",
    "brand": "Furniture Co.",
    "category": "Home & Kitchen",
    "price": 24899,
    "originalPrice": 30740,
    "discount": 19,
    "rating": 2.9,
    "reviewCount": 60020,
    "inStock": true,
    "isPrime": false,
    "delivery": "Get it by Day after Tomorrow",
    "deliveryFree": true,
    "trustScore": 34,
    "trustLabel": "Suspicious",
    "buyNowOrWait": "buy",
    "waitReason": null,
    "spikePriceMonths": [
      8
    ],
    "priceHistory": [
      24899,
      26196,
      26388,
      24899,
      24899,
      24899,
      24899,
      24899,
      28365,
      24899,
      24899,
      24899
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/furniture/bedside-table-african-cherry/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/furniture/bedside-table-african-cherry/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/furniture/bedside-table-african-cherry/1.webp",
      "https://cdn.dummyjson.com/product-images/furniture/bedside-table-african-cherry/2.webp"
    ],
    "description": "The Bedside Table in African Cherry is a stylish and functional addition to your bedroom, providing convenient storage space and a touch of elegance.",
    "features": [
      "The Bedside Table in African Cherry is a stylish and functional addition to your bedroom, providing convenient storage space and a touch of elegance",
      "Frame constructed from solid oak for long-term durability",
      "Dimensions: 13W × 25H × 27D cm",
      "Weight capacity: 100 kg — built to handle everyday household use",
      "Flat-pack with all hardware included — complete setup in under 30 minutes"
    ],
    "specs": {
      "Material": "Solid Sheesham Wood",
      "Dimensions": "13W × 25H × 27D cm",
      "Weight": "2 kg",
      "Weight Capacity": "100 kg",
      "Finish": "Walnut Veneer",
      "Assembly": "Pre-assembled",
      "Warranty": "5 year warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Furniture Co.",
    "soldByRating": 2.4,
    "sellerSince": "2024",
    "fulfillment": "Seller fulfilled",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 32,
        "detail": null
      },
      "returnRate": {
        "score": 24,
        "detail": null
      },
      "warrantyClaims": {
        "score": 36,
        "detail": null
      },
      "sellerReliability": {
        "score": 43,
        "detail": null
      },
      "priceStability": {
        "score": 38,
        "detail": null
      }
    }
  },
  {
    "id": "dj14",
    "name": "Knoll Saarinen Executive Conference Chair",
    "brand": "Knoll",
    "category": "Home & Kitchen",
    "price": 41499,
    "originalPrice": 42346,
    "discount": 2,
    "rating": 4.9,
    "reviewCount": 29685,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Day after Tomorrow",
    "deliveryFree": true,
    "trustScore": 92,
    "trustLabel": "Genuine",
    "buyNowOrWait": "buy",
    "waitReason": null,
    "spikePriceMonths": [],
    "priceHistory": [
      41499,
      41499,
      41499,
      41499,
      41499,
      44190,
      41499,
      41499,
      41499,
      41499,
      39128,
      41499
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/furniture/knoll-saarinen-executive-conference-chair/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/furniture/knoll-saarinen-executive-conference-chair/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/furniture/knoll-saarinen-executive-conference-chair/1.webp",
      "https://cdn.dummyjson.com/product-images/furniture/knoll-saarinen-executive-conference-chair/2.webp"
    ],
    "description": "The Knoll Saarinen Executive Conference Chair is a modern and ergonomic chair, perfect for your office or conference room with its timeless design.",
    "features": [
      "The Knoll Saarinen Executive Conference Chair is a modern and ergonomic chair, perfect for your office or conference room with its timeless design",
      "Frame constructed from engineered hardwood for long-term durability",
      "Dimensions: 14W × 8H × 6D cm",
      "Weight capacity: 100 kg — built to handle everyday household use",
      "Delivered pre-assembled — complete setup in under 60 minutes"
    ],
    "specs": {
      "Material": "Engineered Wood (MDF)",
      "Dimensions": "14W × 8H × 6D cm",
      "Weight": "10 kg",
      "Weight Capacity": "100 kg",
      "Finish": "White Matt",
      "Assembly": "Pre-assembled",
      "Warranty": "2 year warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Knoll",
    "soldByRating": 4,
    "sellerSince": "2014",
    "fulfillment": "Seller fulfilled",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 84,
        "detail": null
      },
      "returnRate": {
        "score": 89,
        "detail": null
      },
      "warrantyClaims": {
        "score": 96,
        "detail": null
      },
      "sellerReliability": {
        "score": 96,
        "detail": null
      },
      "priceStability": {
        "score": 96,
        "detail": null
      }
    }
  },
  {
    "id": "dj15",
    "name": "Wooden Bathroom Sink With Mirror",
    "brand": "Bath Trends",
    "category": "Home & Kitchen",
    "price": 66399,
    "originalPrice": 72966,
    "discount": 9,
    "rating": 3.6,
    "reviewCount": 56846,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Day after Tomorrow",
    "deliveryFree": true,
    "trustScore": 87,
    "trustLabel": "Genuine",
    "buyNowOrWait": "wait",
    "waitReason": "Sale expected in the next 2 weeks based on historical patterns.",
    "spikePriceMonths": [
      6
    ],
    "priceHistory": [
      66399,
      66399,
      66399,
      66399,
      66399,
      66399,
      79931,
      66399,
      66399,
      66399,
      66399,
      66399
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/furniture/wooden-bathroom-sink-with-mirror/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/furniture/wooden-bathroom-sink-with-mirror/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/furniture/wooden-bathroom-sink-with-mirror/1.webp",
      "https://cdn.dummyjson.com/product-images/furniture/wooden-bathroom-sink-with-mirror/2.webp"
    ],
    "description": "The Wooden Bathroom Sink with Mirror is a unique and stylish addition to your bathroom, featuring a wooden sink countertop and a matching mirror.",
    "features": [
      "The Wooden Bathroom Sink with Mirror is a unique and stylish addition to your bathroom, featuring a wooden sink countertop and a matching mirror",
      "Frame constructed from solid sheesham wood for long-term durability",
      "Dimensions: 8W × 9H × 28D cm",
      "Weight capacity: 100 kg — built to handle everyday household use",
      "Flat-pack with all hardware included — complete setup in under 60 minutes"
    ],
    "specs": {
      "Material": "Solid Pine",
      "Dimensions": "8W × 9H × 28D cm",
      "Weight": "10 kg",
      "Weight Capacity": "100 kg",
      "Finish": "Teak Finish",
      "Assembly": "Pre-assembled",
      "Warranty": "3 year warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Bath Trends",
    "soldByRating": 4.4,
    "sellerSince": "2015",
    "fulfillment": "Seller fulfilled",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 89,
        "detail": null
      },
      "returnRate": {
        "score": 92,
        "detail": null
      },
      "warrantyClaims": {
        "score": 87,
        "detail": null
      },
      "sellerReliability": {
        "score": 89,
        "detail": null
      },
      "priceStability": {
        "score": 90,
        "detail": null
      }
    }
  },
  {
    "id": "dj16",
    "name": "Apple",
    "brand": "Generic",
    "category": "Grocery",
    "price": 165,
    "originalPrice": 190,
    "discount": 13,
    "rating": 4.2,
    "reviewCount": 50237,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Tomorrow, 6 PM",
    "deliveryFree": true,
    "trustScore": 84,
    "trustLabel": "Genuine",
    "buyNowOrWait": "wait",
    "waitReason": "Sale expected in the next 2 weeks based on historical patterns.",
    "spikePriceMonths": [
      2,
      3
    ],
    "priceHistory": [
      165,
      165,
      187,
      217,
      164,
      165,
      172,
      165,
      165,
      165,
      165,
      165
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/groceries/apple/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/groceries/apple/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/groceries/apple/1.webp"
    ],
    "description": "Fresh and crisp apples, perfect for snacking or incorporating into various recipes.",
    "features": [
      "Fresh and crisp apples, perfect for snacking or incorporating into various recipes",
      "100% natural — clean-label formulation you can trust",
      "Packed with vitamins & minerals to support a balanced diet",
      "Minimal preparation needed — convenient for busy households",
      "Shelf life: 24 months when stored in a cool, dry place"
    ],
    "specs": {
      "Weight": "9 kg",
      "Serving Size": "one piece",
      "Calories": "350 kcal per serving",
      "Shelf Life": "3 months",
      "Storage": "Freeze after opening",
      "Country of Origin": "India"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Third-party Seller",
    "soldByRating": 4.1,
    "sellerSince": "2013",
    "fulfillment": "Seller fulfilled",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 92,
        "detail": null
      },
      "returnRate": {
        "score": 87,
        "detail": null
      },
      "warrantyClaims": {
        "score": 79,
        "detail": null
      },
      "sellerReliability": {
        "score": 91,
        "detail": null
      },
      "priceStability": {
        "score": 86,
        "detail": null
      }
    }
  },
  {
    "id": "dj17",
    "name": "Beef Steak",
    "brand": "Generic",
    "category": "Grocery",
    "price": 1078,
    "originalPrice": 1198,
    "discount": 10,
    "rating": 4.5,
    "reviewCount": 13346,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Day after Tomorrow",
    "deliveryFree": true,
    "trustScore": 95,
    "trustLabel": "Genuine",
    "buyNowOrWait": "wait",
    "waitReason": "Sale expected in the next 2 weeks based on historical patterns.",
    "spikePriceMonths": [
      7
    ],
    "priceHistory": [
      1078,
      1078,
      1078,
      1078,
      1078,
      1078,
      1078,
      1361,
      1078,
      1078,
      1078,
      1078
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/groceries/beef-steak/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/groceries/beef-steak/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/groceries/beef-steak/1.webp"
    ],
    "description": "High-quality beef steak, great for grilling or cooking to your preferred level of doneness.",
    "features": [
      "High-quality beef steak, great for grilling or cooking to your preferred level of doneness",
      "Organic origin — clean-label formulation you can trust",
      "High in dietary fibre to support a balanced diet",
      "Ready-to-cook — convenient for busy households",
      "Shelf life: 6 months when stored in a cool, dry place"
    ],
    "specs": {
      "Weight": "10 kg",
      "Serving Size": "100g",
      "Calories": "120 kcal per serving",
      "Shelf Life": "18 months",
      "Storage": "Cool, dry place",
      "Country of Origin": "India"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Third-party Seller",
    "soldByRating": 4,
    "sellerSince": "2017",
    "fulfillment": "Fulfilled by Amazon",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 81,
        "detail": null
      },
      "returnRate": {
        "score": 91,
        "detail": null
      },
      "warrantyClaims": {
        "score": 93,
        "detail": null
      },
      "sellerReliability": {
        "score": 96,
        "detail": null
      },
      "priceStability": {
        "score": 96,
        "detail": null
      }
    }
  },
  {
    "id": "dj18",
    "name": "Cat Food",
    "brand": "Generic",
    "category": "Grocery",
    "price": 746,
    "originalPrice": 829,
    "discount": 10,
    "rating": 3.1,
    "reviewCount": 37078,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Tomorrow, 6 PM",
    "deliveryFree": true,
    "trustScore": 76,
    "trustLabel": "Genuine",
    "buyNowOrWait": "wait",
    "waitReason": "Sale expected in the next 2 weeks based on historical patterns.",
    "spikePriceMonths": [],
    "priceHistory": [
      746,
      746,
      746,
      747,
      724,
      805,
      746,
      746,
      693,
      746,
      746,
      746
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/groceries/cat-food/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/groceries/cat-food/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/groceries/cat-food/1.webp"
    ],
    "description": "Nutritious cat food formulated to meet the dietary needs of your feline friend.",
    "features": [
      "Nutritious cat food formulated to meet the dietary needs of your feline friend",
      "100% natural — clean-label formulation you can trust",
      "Packed with vitamins & minerals to support a balanced diet",
      "Ready-to-eat — convenient for busy households",
      "Shelf life: 18 months when stored in a cool, dry place"
    ],
    "specs": {
      "Weight": "10 kg",
      "Serving Size": "200ml",
      "Calories": "80 kcal per serving",
      "Shelf Life": "24 months",
      "Storage": "Room temperature",
      "Country of Origin": "Thailand"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Third-party Seller",
    "soldByRating": 4,
    "sellerSince": "2018",
    "fulfillment": "Fulfilled by Amazon",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 75,
        "detail": null
      },
      "returnRate": {
        "score": 85,
        "detail": null
      },
      "warrantyClaims": {
        "score": 72,
        "detail": null
      },
      "sellerReliability": {
        "score": 91,
        "detail": null
      },
      "priceStability": {
        "score": 73,
        "detail": null
      }
    }
  },
  {
    "id": "dj19",
    "name": "Chicken Meat",
    "brand": "Generic",
    "category": "Grocery",
    "price": 829,
    "originalPrice": 964,
    "discount": 14,
    "rating": 3.2,
    "reviewCount": 75114,
    "inStock": true,
    "isPrime": false,
    "delivery": "Get it by Tomorrow, 6 PM",
    "deliveryFree": true,
    "trustScore": 76,
    "trustLabel": "Genuine",
    "buyNowOrWait": "wait",
    "waitReason": "Sale expected in the next 2 weeks based on historical patterns.",
    "spikePriceMonths": [
      0
    ],
    "priceHistory": [
      1041,
      829,
      829,
      861,
      829,
      900,
      841,
      829,
      829,
      829,
      829,
      829
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/groceries/chicken-meat/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/groceries/chicken-meat/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/groceries/chicken-meat/1.webp",
      "https://cdn.dummyjson.com/product-images/groceries/chicken-meat/2.webp"
    ],
    "description": "Fresh and tender chicken meat, suitable for various culinary preparations.",
    "features": [
      "Fresh and tender chicken meat, suitable for various culinary preparations",
      "100% natural — clean-label formulation you can trust",
      "High in dietary fibre to support a balanced diet",
      "No added sugar — convenient for busy households",
      "Shelf life: 12 months when stored in a cool, dry place"
    ],
    "specs": {
      "Weight": "1 kg",
      "Serving Size": "50g",
      "Calories": "45 kcal per serving",
      "Shelf Life": "12 months",
      "Storage": "Freeze after opening",
      "Country of Origin": "India"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Third-party Seller",
    "soldByRating": 4.2,
    "sellerSince": "2011",
    "fulfillment": "Fulfilled by Amazon",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 62,
        "detail": null
      },
      "returnRate": {
        "score": 74,
        "detail": null
      },
      "warrantyClaims": {
        "score": 75,
        "detail": null
      },
      "sellerReliability": {
        "score": 84,
        "detail": null
      },
      "priceStability": {
        "score": 75,
        "detail": null
      }
    }
  },
  {
    "id": "dj20",
    "name": "Cooking Oil",
    "brand": "Generic",
    "category": "Grocery",
    "price": 414,
    "originalPrice": 455,
    "discount": 9,
    "rating": 4.8,
    "reviewCount": 69782,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Day after Tomorrow",
    "deliveryFree": true,
    "trustScore": 94,
    "trustLabel": "Genuine",
    "buyNowOrWait": "wait",
    "waitReason": "Sale expected in the next 2 weeks based on historical patterns.",
    "spikePriceMonths": [
      10
    ],
    "priceHistory": [
      414,
      414,
      414,
      414,
      414,
      414,
      414,
      414,
      455,
      414,
      515,
      414
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/groceries/cooking-oil/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/groceries/cooking-oil/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/groceries/cooking-oil/1.webp"
    ],
    "description": "Versatile cooking oil suitable for frying, sautéing, and various culinary applications.",
    "features": [
      "Versatile cooking oil suitable for frying, sautéing, and various culinary applications",
      "No artificial preservatives — clean-label formulation you can trust",
      "Packed with vitamins & minerals to support a balanced diet",
      "No added sugar — convenient for busy households",
      "Shelf life: 6 months when stored in a cool, dry place"
    ],
    "specs": {
      "Weight": "5 kg",
      "Serving Size": "one piece",
      "Calories": "45 kcal per serving",
      "Shelf Life": "6 months",
      "Storage": "Refrigerate after opening",
      "Country of Origin": "India"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Third-party Seller",
    "soldByRating": 4.4,
    "sellerSince": "2013",
    "fulfillment": "Fulfilled by Amazon",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 86,
        "detail": null
      },
      "returnRate": {
        "score": 86,
        "detail": null
      },
      "warrantyClaims": {
        "score": 90,
        "detail": null
      },
      "sellerReliability": {
        "score": 96,
        "detail": null
      },
      "priceStability": {
        "score": 86,
        "detail": null
      }
    }
  },
  {
    "id": "dj21",
    "name": "Cucumber",
    "brand": "Generic",
    "category": "Grocery",
    "price": 124,
    "originalPrice": 124,
    "discount": 0,
    "rating": 4.1,
    "reviewCount": 9543,
    "inStock": true,
    "isPrime": false,
    "delivery": "Get it by Tomorrow, 6 PM",
    "deliveryFree": true,
    "trustScore": 93,
    "trustLabel": "Genuine",
    "buyNowOrWait": "wait",
    "waitReason": "Sale expected in the next 2 weeks based on historical patterns.",
    "spikePriceMonths": [],
    "priceHistory": [
      124,
      124,
      124,
      107,
      124,
      124,
      124,
      124,
      124,
      124,
      127,
      124
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/groceries/cucumber/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/groceries/cucumber/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/groceries/cucumber/1.webp"
    ],
    "description": "Crisp and hydrating cucumbers, ideal for salads, snacks, or as a refreshing side.",
    "features": [
      "Crisp and hydrating cucumbers, ideal for salads, snacks, or as a refreshing side",
      "Organic origin — clean-label formulation you can trust",
      "Rich in essential nutrients to support a balanced diet",
      "Ready-to-cook — convenient for busy households",
      "Shelf life: 12 months when stored in a cool, dry place"
    ],
    "specs": {
      "Weight": "4 kg",
      "Serving Size": "30g",
      "Calories": "120 kcal per serving",
      "Shelf Life": "12 months",
      "Storage": "Freeze after opening",
      "Country of Origin": "USA"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Third-party Seller",
    "soldByRating": 3.8,
    "sellerSince": "2016",
    "fulfillment": "Seller fulfilled",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 86,
        "detail": null
      },
      "returnRate": {
        "score": 96,
        "detail": null
      },
      "warrantyClaims": {
        "score": 90,
        "detail": null
      },
      "sellerReliability": {
        "score": 96,
        "detail": null
      },
      "priceStability": {
        "score": 86,
        "detail": null
      }
    }
  },
  {
    "id": "dj22",
    "name": "Dog Food",
    "brand": "Generic",
    "category": "Grocery",
    "price": 912,
    "originalPrice": 1013,
    "discount": 10,
    "rating": 4.6,
    "reviewCount": 42838,
    "inStock": true,
    "isPrime": false,
    "delivery": "Get it by Day after Tomorrow",
    "deliveryFree": true,
    "trustScore": 94,
    "trustLabel": "Genuine",
    "buyNowOrWait": "wait",
    "waitReason": "Sale expected in the next 2 weeks based on historical patterns.",
    "spikePriceMonths": [
      9
    ],
    "priceHistory": [
      912,
      990,
      912,
      912,
      1005,
      912,
      912,
      912,
      912,
      1034,
      1012,
      912
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/groceries/dog-food/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/groceries/dog-food/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/groceries/dog-food/1.webp"
    ],
    "description": "Specially formulated dog food designed to provide essential nutrients for your canine companion.",
    "features": [
      "Specially formulated dog food designed to provide essential nutrients for your canine companion",
      "Naturally sourced — clean-label formulation you can trust",
      "Rich in essential nutrients to support a balanced diet",
      "Minimal preparation needed — convenient for busy households",
      "Shelf life: 6 months when stored in a cool, dry place"
    ],
    "specs": {
      "Weight": "10 kg",
      "Serving Size": "30g",
      "Calories": "80 kcal per serving",
      "Shelf Life": "6 months",
      "Storage": "Refrigerate after opening",
      "Country of Origin": "USA"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Third-party Seller",
    "soldByRating": 3.9,
    "sellerSince": "2013",
    "fulfillment": "Fulfilled by Amazon",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 86,
        "detail": null
      },
      "returnRate": {
        "score": 94,
        "detail": null
      },
      "warrantyClaims": {
        "score": 89,
        "detail": null
      },
      "sellerReliability": {
        "score": 96,
        "detail": null
      },
      "priceStability": {
        "score": 96,
        "detail": null
      }
    }
  },
  {
    "id": "dj23",
    "name": "Eggs",
    "brand": "Generic",
    "category": "Grocery",
    "price": 248,
    "originalPrice": 279,
    "discount": 11,
    "rating": 2.5,
    "reviewCount": 58284,
    "inStock": true,
    "isPrime": false,
    "delivery": "Get it by Tomorrow, 6 PM",
    "deliveryFree": true,
    "trustScore": 77,
    "trustLabel": "Genuine",
    "buyNowOrWait": "wait",
    "waitReason": "Sale expected in the next 2 weeks based on historical patterns.",
    "spikePriceMonths": [],
    "priceHistory": [
      248,
      248,
      248,
      248,
      248,
      248,
      248,
      248,
      248,
      248,
      248,
      248
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/groceries/eggs/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/groceries/eggs/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/groceries/eggs/1.webp"
    ],
    "description": "Fresh eggs, a versatile ingredient for baking, cooking, or breakfast.",
    "features": [
      "Fresh eggs, a versatile ingredient for baking, cooking, or breakfast",
      "Naturally sourced — clean-label formulation you can trust",
      "High in dietary fibre to support a balanced diet",
      "No added sugar — convenient for busy households",
      "Shelf life: 12 months when stored in a cool, dry place"
    ],
    "specs": {
      "Weight": "2 kg",
      "Serving Size": "50g",
      "Calories": "350 kcal per serving",
      "Shelf Life": "12 months",
      "Storage": "Room temperature",
      "Country of Origin": "India"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Third-party Seller",
    "soldByRating": 4.8,
    "sellerSince": "2017",
    "fulfillment": "Fulfilled by Amazon",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 81,
        "detail": null
      },
      "returnRate": {
        "score": 87,
        "detail": null
      },
      "warrantyClaims": {
        "score": 80,
        "detail": null
      },
      "sellerReliability": {
        "score": 84,
        "detail": null
      },
      "priceStability": {
        "score": 75,
        "detail": null
      }
    }
  },
  {
    "id": "dj24",
    "name": "Fish Steak",
    "brand": "Generic",
    "category": "Grocery",
    "price": 1244,
    "originalPrice": 1296,
    "discount": 4,
    "rating": 3.8,
    "reviewCount": 2564,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Day after Tomorrow",
    "deliveryFree": true,
    "trustScore": 39,
    "trustLabel": "Suspicious",
    "buyNowOrWait": "buy",
    "waitReason": null,
    "spikePriceMonths": [
      2
    ],
    "priceHistory": [
      1244,
      1290,
      1401,
      1075,
      1244,
      1244,
      1116,
      1244,
      1244,
      1244,
      1244,
      1244
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/groceries/fish-steak/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/groceries/fish-steak/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/groceries/fish-steak/1.webp"
    ],
    "description": "Quality fish steak, suitable for grilling, baking, or pan-searing.",
    "features": [
      "Quality fish steak, suitable for grilling, baking, or pan-searing",
      "100% natural — clean-label formulation you can trust",
      "Rich in essential nutrients to support a balanced diet",
      "Ready-to-cook — convenient for busy households",
      "Shelf life: 24 months when stored in a cool, dry place"
    ],
    "specs": {
      "Weight": "6 kg",
      "Serving Size": "30g",
      "Calories": "120 kcal per serving",
      "Shelf Life": "3 months",
      "Storage": "Cool, dry place",
      "Country of Origin": "Italy"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Third-party Seller",
    "soldByRating": 2.7,
    "sellerSince": "2022",
    "fulfillment": "Seller fulfilled",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 35,
        "detail": null
      },
      "returnRate": {
        "score": 38,
        "detail": null
      },
      "warrantyClaims": {
        "score": 38,
        "detail": null
      },
      "sellerReliability": {
        "score": 51,
        "detail": null
      },
      "priceStability": {
        "score": 31,
        "detail": null
      }
    }
  },
  {
    "id": "dj25",
    "name": "Green Bell Pepper",
    "brand": "Generic",
    "category": "Grocery",
    "price": 107,
    "originalPrice": 107,
    "discount": 0,
    "rating": 3.3,
    "reviewCount": 56985,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Tomorrow, 6 PM",
    "deliveryFree": true,
    "trustScore": 79,
    "trustLabel": "Genuine",
    "buyNowOrWait": "buy",
    "waitReason": null,
    "spikePriceMonths": [],
    "priceHistory": [
      107,
      107,
      107,
      107,
      107,
      97,
      107,
      107,
      107,
      107,
      107,
      107
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/groceries/green-bell-pepper/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/groceries/green-bell-pepper/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/groceries/green-bell-pepper/1.webp"
    ],
    "description": "Fresh and vibrant green bell pepper, perfect for adding color and flavor to your dishes.",
    "features": [
      "Fresh and vibrant green bell pepper, perfect for adding color and flavor to your dishes",
      "100% natural — clean-label formulation you can trust",
      "Source of natural antioxidants to support a balanced diet",
      "No added sugar — convenient for busy households",
      "Shelf life: 18 months when stored in a cool, dry place"
    ],
    "specs": {
      "Weight": "2 kg",
      "Serving Size": "200ml",
      "Calories": "350 kcal per serving",
      "Shelf Life": "24 months",
      "Storage": "Freeze after opening",
      "Country of Origin": "Sri Lanka"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Third-party Seller",
    "soldByRating": 4.8,
    "sellerSince": "2015",
    "fulfillment": "Seller fulfilled",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 74,
        "detail": null
      },
      "returnRate": {
        "score": 78,
        "detail": null
      },
      "warrantyClaims": {
        "score": 76,
        "detail": null
      },
      "sellerReliability": {
        "score": 93,
        "detail": null
      },
      "priceStability": {
        "score": 77,
        "detail": null
      }
    }
  },
  {
    "id": "dj26",
    "name": "Green Chili Pepper",
    "brand": "Generic",
    "category": "Grocery",
    "price": 82,
    "originalPrice": 83,
    "discount": 1,
    "rating": 3.7,
    "reviewCount": 8216,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Tomorrow, 6 PM",
    "deliveryFree": true,
    "trustScore": 88,
    "trustLabel": "Genuine",
    "buyNowOrWait": "buy",
    "waitReason": null,
    "spikePriceMonths": [],
    "priceHistory": [
      82,
      82,
      82,
      73,
      82,
      82,
      82,
      82,
      82,
      82,
      82,
      82
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/groceries/green-chili-pepper/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/groceries/green-chili-pepper/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/groceries/green-chili-pepper/1.webp"
    ],
    "description": "Spicy green chili pepper, ideal for adding heat to your favorite recipes.",
    "features": [
      "Spicy green chili pepper, ideal for adding heat to your favorite recipes",
      "Naturally sourced — clean-label formulation you can trust",
      "Packed with vitamins & minerals to support a balanced diet",
      "Ready-to-cook — convenient for busy households",
      "Shelf life: 6 months when stored in a cool, dry place"
    ],
    "specs": {
      "Weight": "7 kg",
      "Serving Size": "one piece",
      "Calories": "120 kcal per serving",
      "Shelf Life": "18 months",
      "Storage": "Freeze after opening",
      "Country of Origin": "Sri Lanka"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Third-party Seller",
    "soldByRating": 4.6,
    "sellerSince": "2011",
    "fulfillment": "Seller fulfilled",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 92,
        "detail": null
      },
      "returnRate": {
        "score": 84,
        "detail": null
      },
      "warrantyClaims": {
        "score": 87,
        "detail": null
      },
      "sellerReliability": {
        "score": 93,
        "detail": null
      },
      "priceStability": {
        "score": 94,
        "detail": null
      }
    }
  },
  {
    "id": "dj27",
    "name": "Honey Jar",
    "brand": "Generic",
    "category": "Grocery",
    "price": 580,
    "originalPrice": 674,
    "discount": 14,
    "rating": 4,
    "reviewCount": 12501,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Day after Tomorrow",
    "deliveryFree": true,
    "trustScore": 86,
    "trustLabel": "Genuine",
    "buyNowOrWait": "wait",
    "waitReason": "Sale expected in the next 2 weeks based on historical patterns.",
    "spikePriceMonths": [
      7
    ],
    "priceHistory": [
      580,
      585,
      580,
      580,
      603,
      580,
      580,
      787,
      580,
      580,
      580,
      580
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/groceries/honey-jar/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/groceries/honey-jar/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/groceries/honey-jar/1.webp"
    ],
    "description": "Pure and natural honey in a convenient jar, perfect for sweetening beverages or drizzling over food.",
    "features": [
      "Pure and natural honey in a convenient jar, perfect for sweetening beverages or drizzling over food",
      "No artificial preservatives — clean-label formulation you can trust",
      "Packed with vitamins & minerals to support a balanced diet",
      "Ready-to-cook — convenient for busy households",
      "Shelf life: 6 months when stored in a cool, dry place"
    ],
    "specs": {
      "Weight": "2 kg",
      "Serving Size": "one piece",
      "Calories": "120 kcal per serving",
      "Shelf Life": "18 months",
      "Storage": "Refrigerate after opening",
      "Country of Origin": "USA"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Third-party Seller",
    "soldByRating": 4.3,
    "sellerSince": "2018",
    "fulfillment": "Fulfilled by Amazon",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 93,
        "detail": null
      },
      "returnRate": {
        "score": 91,
        "detail": null
      },
      "warrantyClaims": {
        "score": 88,
        "detail": null
      },
      "sellerReliability": {
        "score": 88,
        "detail": null
      },
      "priceStability": {
        "score": 78,
        "detail": null
      }
    }
  },
  {
    "id": "dj28",
    "name": "Ice Cream",
    "brand": "Generic",
    "category": "Grocery",
    "price": 456,
    "originalPrice": 501,
    "discount": 9,
    "rating": 3.4,
    "reviewCount": 26306,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Day after Tomorrow",
    "deliveryFree": true,
    "trustScore": 36,
    "trustLabel": "Suspicious",
    "buyNowOrWait": "wait",
    "waitReason": "Sale expected in the next 2 weeks based on historical patterns.",
    "spikePriceMonths": [
      2,
      6
    ],
    "priceHistory": [
      456,
      456,
      521,
      456,
      456,
      456,
      568,
      456,
      456,
      456,
      456,
      456
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/groceries/ice-cream/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/groceries/ice-cream/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/groceries/ice-cream/1.webp",
      "https://cdn.dummyjson.com/product-images/groceries/ice-cream/2.webp"
    ],
    "description": "Creamy and delicious ice cream, available in various flavors for a delightful treat.",
    "features": [
      "Creamy and delicious ice cream, available in various flavors for a delightful treat",
      "100% natural — clean-label formulation you can trust",
      "Rich in essential nutrients to support a balanced diet",
      "Ready-to-eat — convenient for busy households",
      "Shelf life: 24 months when stored in a cool, dry place"
    ],
    "specs": {
      "Weight": "1 kg",
      "Serving Size": "30g",
      "Calories": "250 kcal per serving",
      "Shelf Life": "3 months",
      "Storage": "Refrigerate after opening",
      "Country of Origin": "Thailand"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Third-party Seller",
    "soldByRating": 2.5,
    "sellerSince": "2021",
    "fulfillment": "Seller fulfilled",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 26,
        "detail": null
      },
      "returnRate": {
        "score": 29,
        "detail": null
      },
      "warrantyClaims": {
        "score": 38,
        "detail": null
      },
      "sellerReliability": {
        "score": 42,
        "detail": null
      },
      "priceStability": {
        "score": 25,
        "detail": null
      }
    }
  },
  {
    "id": "dj29",
    "name": "Juice",
    "brand": "Generic",
    "category": "Grocery",
    "price": 331,
    "originalPrice": 376,
    "discount": 12,
    "rating": 3.9,
    "reviewCount": 42031,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Day after Tomorrow",
    "deliveryFree": true,
    "trustScore": 82,
    "trustLabel": "Genuine",
    "buyNowOrWait": "wait",
    "waitReason": "Sale expected in the next 2 weeks based on historical patterns.",
    "spikePriceMonths": [
      5
    ],
    "priceHistory": [
      331,
      331,
      331,
      331,
      331,
      374,
      331,
      331,
      331,
      331,
      331,
      331
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/groceries/juice/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/groceries/juice/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/groceries/juice/1.webp"
    ],
    "description": "Refreshing fruit juice, packed with vitamins and great for staying hydrated.",
    "features": [
      "Refreshing fruit juice, packed with vitamins and great for staying hydrated",
      "100% natural — clean-label formulation you can trust",
      "Rich in essential nutrients to support a balanced diet",
      "Minimal preparation needed — convenient for busy households",
      "Shelf life: 24 months when stored in a cool, dry place"
    ],
    "specs": {
      "Weight": "1 kg",
      "Serving Size": "30g",
      "Calories": "80 kcal per serving",
      "Shelf Life": "3 months",
      "Storage": "Refrigerate after opening",
      "Country of Origin": "India"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Third-party Seller",
    "soldByRating": 4.3,
    "sellerSince": "2020",
    "fulfillment": "Fulfilled by Amazon",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 88,
        "detail": null
      },
      "returnRate": {
        "score": 91,
        "detail": null
      },
      "warrantyClaims": {
        "score": 80,
        "detail": null
      },
      "sellerReliability": {
        "score": 83,
        "detail": null
      },
      "priceStability": {
        "score": 76,
        "detail": null
      }
    }
  },
  {
    "id": "dj30",
    "name": "Kiwi",
    "brand": "Generic",
    "category": "Grocery",
    "price": 207,
    "originalPrice": 244,
    "discount": 15,
    "rating": 4.9,
    "reviewCount": 16687,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Day after Tomorrow",
    "deliveryFree": true,
    "trustScore": 90,
    "trustLabel": "Genuine",
    "buyNowOrWait": "wait",
    "waitReason": "Sale expected in the next 2 weeks based on historical patterns.",
    "spikePriceMonths": [
      5
    ],
    "priceHistory": [
      207,
      207,
      231,
      207,
      207,
      256,
      207,
      207,
      207,
      213,
      207,
      207
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/groceries/kiwi/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/groceries/kiwi/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/groceries/kiwi/1.webp"
    ],
    "description": "Nutrient-rich kiwi, perfect for snacking or adding a tropical twist to your dishes.",
    "features": [
      "Nutrient-rich kiwi, perfect for snacking or adding a tropical twist to your dishes",
      "100% natural — clean-label formulation you can trust",
      "High in dietary fibre to support a balanced diet",
      "Ready-to-cook — convenient for busy households",
      "Shelf life: 6 months when stored in a cool, dry place"
    ],
    "specs": {
      "Weight": "5 kg",
      "Serving Size": "100g",
      "Calories": "250 kcal per serving",
      "Shelf Life": "18 months",
      "Storage": "Cool, dry place",
      "Country of Origin": "USA"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Third-party Seller",
    "soldByRating": 4.5,
    "sellerSince": "2011",
    "fulfillment": "Fulfilled by Amazon",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 96,
        "detail": null
      },
      "returnRate": {
        "score": 90,
        "detail": null
      },
      "warrantyClaims": {
        "score": 96,
        "detail": null
      },
      "sellerReliability": {
        "score": 96,
        "detail": null
      },
      "priceStability": {
        "score": 91,
        "detail": null
      }
    }
  },
  {
    "id": "dj31",
    "name": "Lemon",
    "brand": "Generic",
    "category": "Grocery",
    "price": 66,
    "originalPrice": 73,
    "discount": 10,
    "rating": 3.5,
    "reviewCount": 43552,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Day after Tomorrow",
    "deliveryFree": true,
    "trustScore": 78,
    "trustLabel": "Genuine",
    "buyNowOrWait": "buy",
    "waitReason": null,
    "spikePriceMonths": [],
    "priceHistory": [
      66,
      66,
      66,
      66,
      66,
      66,
      66,
      67,
      66,
      66,
      66,
      66
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/groceries/lemon/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/groceries/lemon/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/groceries/lemon/1.webp"
    ],
    "description": "Zesty and tangy lemons, versatile for cooking, baking, or making refreshing beverages.",
    "features": [
      "Zesty and tangy lemons, versatile for cooking, baking, or making refreshing beverages",
      "100% natural — clean-label formulation you can trust",
      "Rich in essential nutrients to support a balanced diet",
      "Minimal preparation needed — convenient for busy households",
      "Shelf life: 18 months when stored in a cool, dry place"
    ],
    "specs": {
      "Weight": "3 kg",
      "Serving Size": "100g",
      "Calories": "80 kcal per serving",
      "Shelf Life": "24 months",
      "Storage": "Cool, dry place",
      "Country of Origin": "Italy"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Third-party Seller",
    "soldByRating": 4.6,
    "sellerSince": "2020",
    "fulfillment": "Fulfilled by Amazon",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 78,
        "detail": null
      },
      "returnRate": {
        "score": 85,
        "detail": null
      },
      "warrantyClaims": {
        "score": 78,
        "detail": null
      },
      "sellerReliability": {
        "score": 87,
        "detail": null
      },
      "priceStability": {
        "score": 80,
        "detail": null
      }
    }
  },
  {
    "id": "dj32",
    "name": "Milk",
    "brand": "Generic",
    "category": "Grocery",
    "price": 290,
    "originalPrice": 337,
    "discount": 14,
    "rating": 2.6,
    "reviewCount": 47813,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Tomorrow, 6 PM",
    "deliveryFree": true,
    "trustScore": 74,
    "trustLabel": "Mixed",
    "buyNowOrWait": "buy",
    "waitReason": null,
    "spikePriceMonths": [
      1,
      6,
      7
    ],
    "priceHistory": [
      290,
      386,
      290,
      314,
      296,
      290,
      348,
      328,
      319,
      290,
      290,
      290
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/groceries/milk/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/groceries/milk/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/groceries/milk/1.webp"
    ],
    "description": "Fresh and nutritious milk, a staple for various recipes and daily consumption.",
    "features": [
      "Fresh and nutritious milk, a staple for various recipes and daily consumption",
      "No artificial preservatives — clean-label formulation you can trust",
      "Source of natural antioxidants to support a balanced diet",
      "Minimal preparation needed — convenient for busy households",
      "Shelf life: 6 months when stored in a cool, dry place"
    ],
    "specs": {
      "Weight": "5 kg",
      "Serving Size": "200ml",
      "Calories": "350 kcal per serving",
      "Shelf Life": "6 months",
      "Storage": "Freeze after opening",
      "Country of Origin": "Sri Lanka"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Third-party Seller",
    "soldByRating": 4.1,
    "sellerSince": "2016",
    "fulfillment": "Fulfilled by Amazon",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 65,
        "detail": null
      },
      "returnRate": {
        "score": 85,
        "detail": null
      },
      "warrantyClaims": {
        "score": 65,
        "detail": null
      },
      "sellerReliability": {
        "score": 84,
        "detail": null
      },
      "priceStability": {
        "score": 74,
        "detail": null
      }
    }
  },
  {
    "id": "dj33",
    "name": "Mulberry",
    "brand": "Generic",
    "category": "Grocery",
    "price": 414,
    "originalPrice": 476,
    "discount": 13,
    "rating": 5,
    "reviewCount": 14931,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Tomorrow, 6 PM",
    "deliveryFree": true,
    "trustScore": 90,
    "trustLabel": "Genuine",
    "buyNowOrWait": "wait",
    "waitReason": "Sale expected in the next 2 weeks based on historical patterns.",
    "spikePriceMonths": [
      7
    ],
    "priceHistory": [
      414,
      414,
      414,
      414,
      414,
      414,
      414,
      489,
      414,
      414,
      414,
      414
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/groceries/mulberry/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/groceries/mulberry/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/groceries/mulberry/1.webp"
    ],
    "description": "Sweet and juicy mulberries, perfect for snacking or adding to desserts and cereals.",
    "features": [
      "Sweet and juicy mulberries, perfect for snacking or adding to desserts and cereals",
      "100% natural — clean-label formulation you can trust",
      "Rich in essential nutrients to support a balanced diet",
      "Ready-to-cook — convenient for busy households",
      "Shelf life: 18 months when stored in a cool, dry place"
    ],
    "specs": {
      "Weight": "5 kg",
      "Serving Size": "100g",
      "Calories": "120 kcal per serving",
      "Shelf Life": "24 months",
      "Storage": "Room temperature",
      "Country of Origin": "Thailand"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Third-party Seller",
    "soldByRating": 4.5,
    "sellerSince": "2011",
    "fulfillment": "Fulfilled by Amazon",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 81,
        "detail": null
      },
      "returnRate": {
        "score": 92,
        "detail": null
      },
      "warrantyClaims": {
        "score": 96,
        "detail": null
      },
      "sellerReliability": {
        "score": 96,
        "detail": null
      },
      "priceStability": {
        "score": 86,
        "detail": null
      }
    }
  },
  {
    "id": "dj34",
    "name": "Nescafe Coffee",
    "brand": "Generic",
    "category": "Grocery",
    "price": 663,
    "originalPrice": 677,
    "discount": 2,
    "rating": 4.8,
    "reviewCount": 14538,
    "inStock": true,
    "isPrime": false,
    "delivery": "Get it by Tomorrow, 6 PM",
    "deliveryFree": true,
    "trustScore": 92,
    "trustLabel": "Genuine",
    "buyNowOrWait": "buy",
    "waitReason": null,
    "spikePriceMonths": [],
    "priceHistory": [
      562,
      663,
      663,
      663,
      663,
      648,
      663,
      663,
      663,
      663,
      663,
      663
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/groceries/nescafe-coffee/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/groceries/nescafe-coffee/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/groceries/nescafe-coffee/1.webp"
    ],
    "description": "Quality coffee from Nescafe, available in various blends for a rich and satisfying cup.",
    "features": [
      "Quality coffee from Nescafe, available in various blends for a rich and satisfying cup",
      "No artificial preservatives — clean-label formulation you can trust",
      "Source of natural antioxidants to support a balanced diet",
      "Ready-to-cook — convenient for busy households",
      "Shelf life: 12 months when stored in a cool, dry place"
    ],
    "specs": {
      "Weight": "6 kg",
      "Serving Size": "200ml",
      "Calories": "120 kcal per serving",
      "Shelf Life": "12 months",
      "Storage": "Freeze after opening",
      "Country of Origin": "Italy"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Third-party Seller",
    "soldByRating": 3.8,
    "sellerSince": "2019",
    "fulfillment": "Fulfilled by Amazon",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 96,
        "detail": null
      },
      "returnRate": {
        "score": 92,
        "detail": null
      },
      "warrantyClaims": {
        "score": 85,
        "detail": null
      },
      "sellerReliability": {
        "score": 96,
        "detail": null
      },
      "priceStability": {
        "score": 91,
        "detail": null
      }
    }
  },
  {
    "id": "dj35",
    "name": "Potatoes",
    "brand": "Generic",
    "category": "Grocery",
    "price": 190,
    "originalPrice": 200,
    "discount": 5,
    "rating": 4.8,
    "reviewCount": 8584,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Tomorrow, 6 PM",
    "deliveryFree": true,
    "trustScore": 96,
    "trustLabel": "Genuine",
    "buyNowOrWait": "buy",
    "waitReason": null,
    "spikePriceMonths": [],
    "priceHistory": [
      190,
      190,
      190,
      190,
      190,
      190,
      190,
      190,
      190,
      198,
      190,
      190
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/groceries/potatoes/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/groceries/potatoes/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/groceries/potatoes/1.webp"
    ],
    "description": "Versatile and starchy potatoes, great for roasting, mashing, or as a side dish.",
    "features": [
      "Versatile and starchy potatoes, great for roasting, mashing, or as a side dish",
      "Naturally sourced — clean-label formulation you can trust",
      "Rich in essential nutrients to support a balanced diet",
      "Ready-to-cook — convenient for busy households",
      "Shelf life: 6 months when stored in a cool, dry place"
    ],
    "specs": {
      "Weight": "9 kg",
      "Serving Size": "30g",
      "Calories": "120 kcal per serving",
      "Shelf Life": "6 months",
      "Storage": "Room temperature",
      "Country of Origin": "Sri Lanka"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Third-party Seller",
    "soldByRating": 4.3,
    "sellerSince": "2010",
    "fulfillment": "Seller fulfilled",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 88,
        "detail": null
      },
      "returnRate": {
        "score": 96,
        "detail": null
      },
      "warrantyClaims": {
        "score": 92,
        "detail": null
      },
      "sellerReliability": {
        "score": 96,
        "detail": null
      },
      "priceStability": {
        "score": 86,
        "detail": null
      }
    }
  },
  {
    "id": "dj36",
    "name": "Protein Powder",
    "brand": "Generic",
    "category": "Grocery",
    "price": 1659,
    "originalPrice": 1803,
    "discount": 8,
    "rating": 4.2,
    "reviewCount": 66293,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Day after Tomorrow",
    "deliveryFree": true,
    "trustScore": 93,
    "trustLabel": "Genuine",
    "buyNowOrWait": "wait",
    "waitReason": "Sale expected in the next 2 weeks based on historical patterns.",
    "spikePriceMonths": [
      4
    ],
    "priceHistory": [
      1517,
      1659,
      1659,
      1659,
      1952,
      1659,
      1659,
      1659,
      1538,
      1659,
      1659,
      1659
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/groceries/protein-powder/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/groceries/protein-powder/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/groceries/protein-powder/1.webp"
    ],
    "description": "Nutrient-packed protein powder, ideal for supplementing your diet with essential proteins.",
    "features": [
      "Nutrient-packed protein powder, ideal for supplementing your diet with essential proteins",
      "Organic origin — clean-label formulation you can trust",
      "High in dietary fibre to support a balanced diet",
      "No added sugar — convenient for busy households",
      "Shelf life: 18 months when stored in a cool, dry place"
    ],
    "specs": {
      "Weight": "9 kg",
      "Serving Size": "50g",
      "Calories": "45 kcal per serving",
      "Shelf Life": "18 months",
      "Storage": "Refrigerate after opening",
      "Country of Origin": "Thailand"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Third-party Seller",
    "soldByRating": 4.9,
    "sellerSince": "2015",
    "fulfillment": "Seller fulfilled",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 89,
        "detail": null
      },
      "returnRate": {
        "score": 96,
        "detail": null
      },
      "warrantyClaims": {
        "score": 96,
        "detail": null
      },
      "sellerReliability": {
        "score": 96,
        "detail": null
      },
      "priceStability": {
        "score": 96,
        "detail": null
      }
    }
  },
  {
    "id": "dj37",
    "name": "Red Onions",
    "brand": "Generic",
    "category": "Grocery",
    "price": 165,
    "originalPrice": 183,
    "discount": 10,
    "rating": 4.2,
    "reviewCount": 2228,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Tomorrow, 6 PM",
    "deliveryFree": true,
    "trustScore": 89,
    "trustLabel": "Genuine",
    "buyNowOrWait": "wait",
    "waitReason": "Sale expected in the next 2 weeks based on historical patterns.",
    "spikePriceMonths": [
      0
    ],
    "priceHistory": [
      191,
      165,
      165,
      165,
      165,
      165,
      165,
      165,
      151,
      170,
      165,
      165
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/groceries/red-onions/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/groceries/red-onions/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/groceries/red-onions/1.webp"
    ],
    "description": "Flavorful and aromatic red onions, perfect for adding depth to your savory dishes.",
    "features": [
      "Flavorful and aromatic red onions, perfect for adding depth to your savory dishes",
      "No artificial preservatives — clean-label formulation you can trust",
      "Rich in essential nutrients to support a balanced diet",
      "Ready-to-cook — convenient for busy households",
      "Shelf life: 6 months when stored in a cool, dry place"
    ],
    "specs": {
      "Weight": "9 kg",
      "Serving Size": "30g",
      "Calories": "120 kcal per serving",
      "Shelf Life": "6 months",
      "Storage": "Room temperature",
      "Country of Origin": "USA"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Third-party Seller",
    "soldByRating": 4.8,
    "sellerSince": "2021",
    "fulfillment": "Seller fulfilled",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 79,
        "detail": null
      },
      "returnRate": {
        "score": 88,
        "detail": null
      },
      "warrantyClaims": {
        "score": 96,
        "detail": null
      },
      "sellerReliability": {
        "score": 89,
        "detail": null
      },
      "priceStability": {
        "score": 84,
        "detail": null
      }
    }
  },
  {
    "id": "dj38",
    "name": "Rice",
    "brand": "Generic",
    "category": "Grocery",
    "price": 497,
    "originalPrice": 546,
    "discount": 9,
    "rating": 3.2,
    "reviewCount": 40433,
    "inStock": true,
    "isPrime": false,
    "delivery": "Get it by Day after Tomorrow",
    "deliveryFree": true,
    "trustScore": 77,
    "trustLabel": "Genuine",
    "buyNowOrWait": "buy",
    "waitReason": null,
    "spikePriceMonths": [],
    "priceHistory": [
      497,
      497,
      497,
      497,
      497,
      497,
      497,
      497,
      497,
      497,
      497,
      497
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/groceries/rice/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/groceries/rice/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/groceries/rice/1.webp"
    ],
    "description": "High-quality rice, a staple for various cuisines and a versatile base for many dishes.",
    "features": [
      "High-quality rice, a staple for various cuisines and a versatile base for many dishes",
      "100% natural — clean-label formulation you can trust",
      "High in dietary fibre to support a balanced diet",
      "Minimal preparation needed — convenient for busy households",
      "Shelf life: 12 months when stored in a cool, dry place"
    ],
    "specs": {
      "Weight": "5 kg",
      "Serving Size": "100g",
      "Calories": "80 kcal per serving",
      "Shelf Life": "12 months",
      "Storage": "Refrigerate after opening",
      "Country of Origin": "Sri Lanka"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Third-party Seller",
    "soldByRating": 4.9,
    "sellerSince": "2019",
    "fulfillment": "Fulfilled by Amazon",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 72,
        "detail": null
      },
      "returnRate": {
        "score": 81,
        "detail": null
      },
      "warrantyClaims": {
        "score": 83,
        "detail": null
      },
      "sellerReliability": {
        "score": 90,
        "detail": null
      },
      "priceStability": {
        "score": 79,
        "detail": null
      }
    }
  },
  {
    "id": "dj39",
    "name": "Soft Drinks",
    "brand": "Generic",
    "category": "Grocery",
    "price": 165,
    "originalPrice": 199,
    "discount": 17,
    "rating": 4.8,
    "reviewCount": 57508,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Day after Tomorrow",
    "deliveryFree": true,
    "trustScore": 96,
    "trustLabel": "Genuine",
    "buyNowOrWait": "buy",
    "waitReason": null,
    "spikePriceMonths": [],
    "priceHistory": [
      165,
      165,
      165,
      165,
      165,
      165,
      165,
      165,
      165,
      165,
      165,
      165
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/groceries/soft-drinks/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/groceries/soft-drinks/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/groceries/soft-drinks/1.webp"
    ],
    "description": "Assorted soft drinks in various flavors, perfect for refreshing beverages.",
    "features": [
      "Assorted soft drinks in various flavors, perfect for refreshing beverages",
      "Organic origin — clean-label formulation you can trust",
      "High in dietary fibre to support a balanced diet",
      "No added sugar — convenient for busy households",
      "Shelf life: 18 months when stored in a cool, dry place"
    ],
    "specs": {
      "Weight": "9 kg",
      "Serving Size": "50g",
      "Calories": "350 kcal per serving",
      "Shelf Life": "24 months",
      "Storage": "Refrigerate after opening",
      "Country of Origin": "Sri Lanka"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Third-party Seller",
    "soldByRating": 4.7,
    "sellerSince": "2016",
    "fulfillment": "Fulfilled by Amazon",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 88,
        "detail": null
      },
      "returnRate": {
        "score": 91,
        "detail": null
      },
      "warrantyClaims": {
        "score": 88,
        "detail": null
      },
      "sellerReliability": {
        "score": 96,
        "detail": null
      },
      "priceStability": {
        "score": 96,
        "detail": null
      }
    }
  },
  {
    "id": "dj40",
    "name": "Strawberry",
    "brand": "Generic",
    "category": "Grocery",
    "price": 331,
    "originalPrice": 334,
    "discount": 1,
    "rating": 3.1,
    "reviewCount": 21005,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Tomorrow, 6 PM",
    "deliveryFree": true,
    "trustScore": 82,
    "trustLabel": "Genuine",
    "buyNowOrWait": "wait",
    "waitReason": "Sale expected in the next 2 weeks based on historical patterns.",
    "spikePriceMonths": [],
    "priceHistory": [
      331,
      331,
      331,
      331,
      328,
      331,
      331,
      331,
      352,
      331,
      331,
      331
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/groceries/strawberry/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/groceries/strawberry/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/groceries/strawberry/1.webp"
    ],
    "description": "Sweet and succulent strawberries, great for snacking, desserts, or blending into smoothies.",
    "features": [
      "Sweet and succulent strawberries, great for snacking, desserts, or blending into smoothies",
      "Naturally sourced — clean-label formulation you can trust",
      "Packed with vitamins & minerals to support a balanced diet",
      "Ready-to-eat — convenient for busy households",
      "Shelf life: 24 months when stored in a cool, dry place"
    ],
    "specs": {
      "Weight": "1 kg",
      "Serving Size": "one piece",
      "Calories": "250 kcal per serving",
      "Shelf Life": "3 months",
      "Storage": "Freeze after opening",
      "Country of Origin": "Thailand"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Third-party Seller",
    "soldByRating": 4.1,
    "sellerSince": "2021",
    "fulfillment": "Seller fulfilled",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 84,
        "detail": null
      },
      "returnRate": {
        "score": 79,
        "detail": null
      },
      "warrantyClaims": {
        "score": 80,
        "detail": null
      },
      "sellerReliability": {
        "score": 85,
        "detail": null
      },
      "priceStability": {
        "score": 89,
        "detail": null
      }
    }
  },
  {
    "id": "dj41",
    "name": "Tissue Paper Box",
    "brand": "Generic",
    "category": "Grocery",
    "price": 207,
    "originalPrice": 238,
    "discount": 13,
    "rating": 2.7,
    "reviewCount": 34309,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Tomorrow, 6 PM",
    "deliveryFree": true,
    "trustScore": 39,
    "trustLabel": "Suspicious",
    "buyNowOrWait": "buy",
    "waitReason": null,
    "spikePriceMonths": [
      0,
      1,
      2,
      9,
      10
    ],
    "priceHistory": [
      268,
      258,
      233,
      207,
      207,
      207,
      207,
      207,
      207,
      257,
      273,
      207
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/groceries/tissue-paper-box/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/groceries/tissue-paper-box/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/groceries/tissue-paper-box/1.webp",
      "https://cdn.dummyjson.com/product-images/groceries/tissue-paper-box/2.webp"
    ],
    "description": "Convenient tissue paper box for everyday use, providing soft and absorbent tissues.",
    "features": [
      "Convenient tissue paper box for everyday use, providing soft and absorbent tissues",
      "Organic origin — clean-label formulation you can trust",
      "Packed with vitamins & minerals to support a balanced diet",
      "Ready-to-eat — convenient for busy households",
      "Shelf life: 18 months when stored in a cool, dry place"
    ],
    "specs": {
      "Weight": "1 kg",
      "Serving Size": "one piece",
      "Calories": "80 kcal per serving",
      "Shelf Life": "24 months",
      "Storage": "Room temperature",
      "Country of Origin": "Sri Lanka"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Third-party Seller",
    "soldByRating": 2.6,
    "sellerSince": "2024",
    "fulfillment": "Seller fulfilled",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 44,
        "detail": null
      },
      "returnRate": {
        "score": 32,
        "detail": null
      },
      "warrantyClaims": {
        "score": 43,
        "detail": null
      },
      "sellerReliability": {
        "score": 44,
        "detail": null
      },
      "priceStability": {
        "score": 36,
        "detail": null
      }
    }
  },
  {
    "id": "dj42",
    "name": "Water",
    "brand": "Generic",
    "category": "Grocery",
    "price": 82,
    "originalPrice": 96,
    "discount": 15,
    "rating": 5,
    "reviewCount": 64588,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Tomorrow, 6 PM",
    "deliveryFree": true,
    "trustScore": 96,
    "trustLabel": "Genuine",
    "buyNowOrWait": "wait",
    "waitReason": "Sale expected in the next 2 weeks based on historical patterns.",
    "spikePriceMonths": [
      10
    ],
    "priceHistory": [
      82,
      82,
      82,
      82,
      82,
      82,
      82,
      82,
      82,
      82,
      96,
      82
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/groceries/water/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/groceries/water/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/groceries/water/1.webp"
    ],
    "description": "Pure and refreshing bottled water, essential for staying hydrated throughout the day.",
    "features": [
      "Pure and refreshing bottled water, essential for staying hydrated throughout the day",
      "Naturally sourced — clean-label formulation you can trust",
      "Packed with vitamins & minerals to support a balanced diet",
      "No added sugar — convenient for busy households",
      "Shelf life: 6 months when stored in a cool, dry place"
    ],
    "specs": {
      "Weight": "4 kg",
      "Serving Size": "200ml",
      "Calories": "45 kcal per serving",
      "Shelf Life": "6 months",
      "Storage": "Freeze after opening",
      "Country of Origin": "USA"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Third-party Seller",
    "soldByRating": 4.2,
    "sellerSince": "2019",
    "fulfillment": "Seller fulfilled",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 91,
        "detail": null
      },
      "returnRate": {
        "score": 96,
        "detail": null
      },
      "warrantyClaims": {
        "score": 96,
        "detail": null
      },
      "sellerReliability": {
        "score": 96,
        "detail": null
      },
      "priceStability": {
        "score": 96,
        "detail": null
      }
    }
  },
  {
    "id": "dj43",
    "name": "Decoration Swing",
    "brand": "Generic",
    "category": "Home & Kitchen",
    "price": 4979,
    "originalPrice": 5532,
    "discount": 10,
    "rating": 3.2,
    "reviewCount": 38363,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Tomorrow, 6 PM",
    "deliveryFree": true,
    "trustScore": 80,
    "trustLabel": "Genuine",
    "buyNowOrWait": "wait",
    "waitReason": "Sale expected in the next 2 weeks based on historical patterns.",
    "spikePriceMonths": [],
    "priceHistory": [
      4979,
      4979,
      4979,
      4979,
      4979,
      4979,
      4979,
      4979,
      4979,
      4979,
      4979,
      4979
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/home-decoration/decoration-swing/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/home-decoration/decoration-swing/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/home-decoration/decoration-swing/1.webp",
      "https://cdn.dummyjson.com/product-images/home-decoration/decoration-swing/2.webp"
    ],
    "description": "The Decoration Swing is a charming addition to your home decor. Crafted with intricate details, it adds a touch of elegance and whimsy to any room.",
    "features": [
      "The Decoration Swing is a charming addition to your home decor",
      "Crafted from premium resin for a stylish, lasting finish",
      "Artisan-crafted — every piece has its own unique character",
      "Versatile design complements minimalist interior styles",
      "Easy care: dust with a soft brush"
    ],
    "specs": {
      "Material": "Ceramic",
      "Dimensions": "24W × 15H × 26D cm",
      "Weight": "4 kg",
      "Finish": "Natural Wax",
      "Indoor/Outdoor": "Indoor only",
      "Warranty": "1 week warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Third-party Seller",
    "soldByRating": 3.8,
    "sellerSince": "2012",
    "fulfillment": "Seller fulfilled",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 79,
        "detail": null
      },
      "returnRate": {
        "score": 71,
        "detail": null
      },
      "warrantyClaims": {
        "score": 75,
        "detail": null
      },
      "sellerReliability": {
        "score": 88,
        "detail": null
      },
      "priceStability": {
        "score": 71,
        "detail": null
      }
    }
  },
  {
    "id": "dj44",
    "name": "Family Tree Photo Frame",
    "brand": "Generic",
    "category": "Home & Kitchen",
    "price": 2489,
    "originalPrice": 2928,
    "discount": 15,
    "rating": 4.5,
    "reviewCount": 40241,
    "inStock": true,
    "isPrime": false,
    "delivery": "Get it by Day after Tomorrow",
    "deliveryFree": true,
    "trustScore": 88,
    "trustLabel": "Genuine",
    "buyNowOrWait": "wait",
    "waitReason": "Sale expected in the next 2 weeks based on historical patterns.",
    "spikePriceMonths": [
      0
    ],
    "priceHistory": [
      2816,
      2489,
      2489,
      2489,
      2489,
      2489,
      2489,
      2489,
      2489,
      2489,
      2489,
      2489
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/home-decoration/family-tree-photo-frame/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/home-decoration/family-tree-photo-frame/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/home-decoration/family-tree-photo-frame/1.webp"
    ],
    "description": "The Family Tree Photo Frame is a sentimental and stylish way to display your cherished family memories. With multiple photo slots, it tells the story of your loved ones.",
    "features": [
      "The Family Tree Photo Frame is a sentimental and stylish way to display your cherished family memories",
      "Crafted from solid wood for a stylish, lasting finish",
      "Artisan-crafted — every piece has its own unique character",
      "Versatile design complements minimalist interior styles",
      "Easy care: wipe clean with a dry cloth"
    ],
    "specs": {
      "Material": "Resin",
      "Dimensions": "17W × 15H × 29D cm",
      "Weight": "1 kg",
      "Finish": "Matte",
      "Indoor/Outdoor": "Indoor only",
      "Warranty": "1 month warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Third-party Seller",
    "soldByRating": 4.1,
    "sellerSince": "2011",
    "fulfillment": "Fulfilled by Amazon",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 85,
        "detail": null
      },
      "returnRate": {
        "score": 96,
        "detail": null
      },
      "warrantyClaims": {
        "score": 83,
        "detail": null
      },
      "sellerReliability": {
        "score": 89,
        "detail": null
      },
      "priceStability": {
        "score": 94,
        "detail": null
      }
    }
  },
  {
    "id": "dj45",
    "name": "House Showpiece Plant",
    "brand": "Generic",
    "category": "Home & Kitchen",
    "price": 3319,
    "originalPrice": 3569,
    "discount": 7,
    "rating": 4.7,
    "reviewCount": 43959,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Day after Tomorrow",
    "deliveryFree": true,
    "trustScore": 90,
    "trustLabel": "Genuine",
    "buyNowOrWait": "wait",
    "waitReason": "Sale expected in the next 2 weeks based on historical patterns.",
    "spikePriceMonths": [],
    "priceHistory": [
      3253,
      3319,
      3319,
      3319,
      3319,
      3319,
      3319,
      3319,
      3319,
      3319,
      3319,
      3319
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/home-decoration/house-showpiece-plant/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/home-decoration/house-showpiece-plant/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/home-decoration/house-showpiece-plant/1.webp",
      "https://cdn.dummyjson.com/product-images/home-decoration/house-showpiece-plant/2.webp"
    ],
    "description": "The House Showpiece Plant is an artificial plant that brings a touch of nature to your home without the need for maintenance. It adds greenery and style to any space.",
    "features": [
      "The House Showpiece Plant is an artificial plant that brings a touch of nature to your home without the need for maintenance",
      "Crafted from premium resin for a stylish, lasting finish",
      "Hand-carved — every piece has its own unique character",
      "Versatile design complements minimalist interior styles",
      "Easy care: dust with a soft brush"
    ],
    "specs": {
      "Material": "Resin",
      "Dimensions": "9W × 15H × 17D cm",
      "Weight": "8 kg",
      "Finish": "Matte",
      "Indoor/Outdoor": "Indoor only",
      "Warranty": "1 year warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Third-party Seller",
    "soldByRating": 4.4,
    "sellerSince": "2019",
    "fulfillment": "Seller fulfilled",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 96,
        "detail": null
      },
      "returnRate": {
        "score": 93,
        "detail": null
      },
      "warrantyClaims": {
        "score": 84,
        "detail": null
      },
      "sellerReliability": {
        "score": 96,
        "detail": null
      },
      "priceStability": {
        "score": 96,
        "detail": null
      }
    }
  },
  {
    "id": "dj46",
    "name": "Plant Pot",
    "brand": "Generic",
    "category": "Home & Kitchen",
    "price": 1244,
    "originalPrice": 1338,
    "discount": 7,
    "rating": 3,
    "reviewCount": 35790,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Tomorrow, 6 PM",
    "deliveryFree": true,
    "trustScore": 77,
    "trustLabel": "Genuine",
    "buyNowOrWait": "wait",
    "waitReason": "Sale expected in the next 2 weeks based on historical patterns.",
    "spikePriceMonths": [
      1
    ],
    "priceHistory": [
      1244,
      1452,
      1244,
      1244,
      1244,
      1244,
      1244,
      1332,
      1244,
      1244,
      1244,
      1244
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/home-decoration/plant-pot/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/home-decoration/plant-pot/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/home-decoration/plant-pot/1.webp",
      "https://cdn.dummyjson.com/product-images/home-decoration/plant-pot/2.webp"
    ],
    "description": "The Plant Pot is a stylish container for your favorite plants. With a sleek design, it complements your indoor or outdoor garden, adding a modern touch to your plant display.",
    "features": [
      "The Plant Pot is a stylish container for your favorite plants",
      "Crafted from premium resin for a stylish, lasting finish",
      "Machine-finished with hand detailing — every piece has its own unique character",
      "Versatile design complements minimalist interior styles",
      "Easy care: dust with a soft brush"
    ],
    "specs": {
      "Material": "Resin",
      "Dimensions": "10W × 20H × 13D cm",
      "Weight": "9 kg",
      "Finish": "Antique Distressed",
      "Indoor/Outdoor": "Indoor & Covered Outdoor",
      "Warranty": "Lifetime warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Third-party Seller",
    "soldByRating": 4.3,
    "sellerSince": "2013",
    "fulfillment": "Seller fulfilled",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 73,
        "detail": null
      },
      "returnRate": {
        "score": 78,
        "detail": null
      },
      "warrantyClaims": {
        "score": 84,
        "detail": null
      },
      "sellerReliability": {
        "score": 88,
        "detail": null
      },
      "priceStability": {
        "score": 70,
        "detail": null
      }
    }
  },
  {
    "id": "dj47",
    "name": "Table Lamp",
    "brand": "Generic",
    "category": "Home & Kitchen",
    "price": 4149,
    "originalPrice": 4461,
    "discount": 7,
    "rating": 3.6,
    "reviewCount": 52710,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Day after Tomorrow",
    "deliveryFree": true,
    "trustScore": 42,
    "trustLabel": "Suspicious",
    "buyNowOrWait": "wait",
    "waitReason": "Sale expected in the next 2 weeks based on historical patterns.",
    "spikePriceMonths": [],
    "priceHistory": [
      4149,
      4149,
      4125,
      4149,
      4149,
      4637,
      4149,
      4149,
      4149,
      4149,
      4149,
      4149
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/home-decoration/table-lamp/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/home-decoration/table-lamp/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/home-decoration/table-lamp/1.webp"
    ],
    "description": "The Table Lamp is a functional and decorative lighting solution for your living space. With a modern design, it provides both ambient and task lighting, enhancing the atmosphere.",
    "features": [
      "The Table Lamp is a functional and decorative lighting solution for your living space",
      "Crafted from high-quality ceramics for a stylish, lasting finish",
      "Hand-carved — every piece has its own unique character",
      "Versatile design complements bohemian interior styles",
      "Easy care: spot clean only"
    ],
    "specs": {
      "Material": "Metal",
      "Dimensions": "9W × 13H × 6D cm",
      "Weight": "4 kg",
      "Finish": "Glossy",
      "Indoor/Outdoor": "Indoor only",
      "Warranty": "3 months warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Third-party Seller",
    "soldByRating": 2.3,
    "sellerSince": "2021",
    "fulfillment": "Seller fulfilled",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 41,
        "detail": null
      },
      "returnRate": {
        "score": 52,
        "detail": null
      },
      "warrantyClaims": {
        "score": 42,
        "detail": null
      },
      "sellerReliability": {
        "score": 51,
        "detail": null
      },
      "priceStability": {
        "score": 50,
        "detail": null
      }
    }
  },
  {
    "id": "dj48",
    "name": "Bamboo Spatula",
    "brand": "Generic",
    "category": "Home & Kitchen",
    "price": 663,
    "originalPrice": 684,
    "discount": 3,
    "rating": 3.3,
    "reviewCount": 46124,
    "inStock": true,
    "isPrime": false,
    "delivery": "Get it by Day after Tomorrow",
    "deliveryFree": true,
    "trustScore": 82,
    "trustLabel": "Genuine",
    "buyNowOrWait": "wait",
    "waitReason": "Sale expected in the next 2 weeks based on historical patterns.",
    "spikePriceMonths": [],
    "priceHistory": [
      622,
      663,
      663,
      702,
      663,
      663,
      719,
      663,
      663,
      663,
      663,
      663
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/kitchen-accessories/bamboo-spatula/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/kitchen-accessories/bamboo-spatula/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/kitchen-accessories/bamboo-spatula/1.webp"
    ],
    "description": "The Bamboo Spatula is a versatile kitchen tool made from eco-friendly bamboo. Ideal for flipping, stirring, and serving various dishes.",
    "features": [
      "The Bamboo Spatula is a versatile kitchen tool made from eco-friendly bamboo",
      "Made from BPA-free plastic — safe and built to last",
      "Generous capacity — ideal for everyday cooking, meal prep and entertaining",
      "Top-rack dishwasher safe for convenient cleaning",
      "Compatible with oven up to 250°C"
    ],
    "specs": {
      "Material": "Cast Iron",
      "Capacity": "500ml",
      "Dimensions": "21W × 23H × 26D cm",
      "Weight": "3 kg",
      "Dishwasher Safe": "Top Rack Only",
      "Induction Compatible": "Yes",
      "Warranty": "1 month warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Third-party Seller",
    "soldByRating": 4.7,
    "sellerSince": "2011",
    "fulfillment": "Fulfilled by Amazon",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 80,
        "detail": null
      },
      "returnRate": {
        "score": 74,
        "detail": null
      },
      "warrantyClaims": {
        "score": 74,
        "detail": null
      },
      "sellerReliability": {
        "score": 92,
        "detail": null
      },
      "priceStability": {
        "score": 84,
        "detail": null
      }
    }
  },
  {
    "id": "dj49",
    "name": "Black Aluminium Cup",
    "brand": "Generic",
    "category": "Home & Kitchen",
    "price": 497,
    "originalPrice": 592,
    "discount": 16,
    "rating": 4.5,
    "reviewCount": 72019,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Tomorrow, 6 PM",
    "deliveryFree": true,
    "trustScore": 89,
    "trustLabel": "Genuine",
    "buyNowOrWait": "buy",
    "waitReason": null,
    "spikePriceMonths": [
      0,
      10
    ],
    "priceHistory": [
      688,
      497,
      497,
      497,
      497,
      497,
      497,
      497,
      532,
      497,
      611,
      497
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/kitchen-accessories/black-aluminium-cup/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/kitchen-accessories/black-aluminium-cup/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/kitchen-accessories/black-aluminium-cup/1.webp",
      "https://cdn.dummyjson.com/product-images/kitchen-accessories/black-aluminium-cup/2.webp"
    ],
    "description": "The Black Aluminium Cup is a stylish and durable cup suitable for both hot and cold beverages. Its sleek black design adds a modern touch to your drinkware collection.",
    "features": [
      "The Black Aluminium Cup is a stylish and durable cup suitable for both hot and cold beverages",
      "Made from premium-grade polypropylene — safe and built to last",
      "Generous capacity — ideal for everyday cooking, meal prep and entertaining",
      "Top-rack dishwasher safe for convenient cleaning",
      "Compatible with microwave and oven"
    ],
    "specs": {
      "Material": "BPA-free Polypropylene",
      "Capacity": "5L",
      "Dimensions": "6W × 5H × 10D cm",
      "Weight": "7 kg",
      "Dishwasher Safe": "No",
      "Induction Compatible": "Yes",
      "Warranty": "1 year warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Third-party Seller",
    "soldByRating": 3.8,
    "sellerSince": "2013",
    "fulfillment": "Fulfilled by Amazon",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 88,
        "detail": null
      },
      "returnRate": {
        "score": 81,
        "detail": null
      },
      "warrantyClaims": {
        "score": 96,
        "detail": null
      },
      "sellerReliability": {
        "score": 94,
        "detail": null
      },
      "priceStability": {
        "score": 91,
        "detail": null
      }
    }
  },
  {
    "id": "dj50",
    "name": "Black Whisk",
    "brand": "Generic",
    "category": "Home & Kitchen",
    "price": 829,
    "originalPrice": 921,
    "discount": 10,
    "rating": 3.9,
    "reviewCount": 25293,
    "inStock": true,
    "isPrime": false,
    "delivery": "Get it by Day after Tomorrow",
    "deliveryFree": true,
    "trustScore": 44,
    "trustLabel": "Suspicious",
    "buyNowOrWait": "buy",
    "waitReason": null,
    "spikePriceMonths": [
      2,
      10
    ],
    "priceHistory": [
      829,
      862,
      950,
      829,
      829,
      922,
      829,
      829,
      829,
      829,
      950,
      829
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/kitchen-accessories/black-whisk/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/kitchen-accessories/black-whisk/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/kitchen-accessories/black-whisk/1.webp"
    ],
    "description": "The Black Whisk is a kitchen essential for whisking and beating ingredients. Its ergonomic handle and sleek design make it a practical and stylish tool.",
    "features": [
      "The Black Whisk is a kitchen essential for whisking and beating ingredients",
      "Made from food-safe silicone — safe and built to last",
      "Generous capacity — ideal for everyday cooking, meal prep and entertaining",
      "Easy-wipe for convenient cleaning",
      "Compatible with all cooktop types"
    ],
    "specs": {
      "Material": "Silicone",
      "Capacity": "3L",
      "Dimensions": "13W × 6H × 21D cm",
      "Weight": "1 kg",
      "Dishwasher Safe": "Top Rack Only",
      "Induction Compatible": "Yes",
      "Warranty": "3 months warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Third-party Seller",
    "soldByRating": 2.2,
    "sellerSince": "2022",
    "fulfillment": "Seller fulfilled",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 54,
        "detail": null
      },
      "returnRate": {
        "score": 37,
        "detail": null
      },
      "warrantyClaims": {
        "score": 53,
        "detail": null
      },
      "sellerReliability": {
        "score": 53,
        "detail": null
      },
      "priceStability": {
        "score": 45,
        "detail": null
      }
    }
  },
  {
    "id": "dj51",
    "name": "Boxed Blender",
    "brand": "Generic",
    "category": "Home & Kitchen",
    "price": 3319,
    "originalPrice": 3569,
    "discount": 7,
    "rating": 4.6,
    "reviewCount": 20465,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Tomorrow, 6 PM",
    "deliveryFree": true,
    "trustScore": 88,
    "trustLabel": "Genuine",
    "buyNowOrWait": "wait",
    "waitReason": "Sale expected in the next 2 weeks based on historical patterns.",
    "spikePriceMonths": [],
    "priceHistory": [
      3568,
      3319,
      3319,
      3319,
      3319,
      3319,
      3319,
      3319,
      3319,
      3319,
      3319,
      3319
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/kitchen-accessories/boxed-blender/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/kitchen-accessories/boxed-blender/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/kitchen-accessories/boxed-blender/1.webp",
      "https://cdn.dummyjson.com/product-images/kitchen-accessories/boxed-blender/2.webp"
    ],
    "description": "The Boxed Blender is a powerful and compact blender perfect for smoothies, shakes, and more. Its convenient design and multiple functions make it a versatile kitchen appliance.",
    "features": [
      "The Boxed Blender is a powerful and compact blender perfect for smoothies, shakes, and more",
      "Made from food-grade stainless steel — safe and built to last",
      "Generous capacity — ideal for everyday cooking, meal prep and entertaining",
      "Hand-wash only for convenient cleaning",
      "Compatible with all cooktop types"
    ],
    "specs": {
      "Material": "Food-grade Stainless Steel",
      "Capacity": "1.5L",
      "Dimensions": "9W × 19H × 18D cm",
      "Weight": "1 kg",
      "Dishwasher Safe": "Yes",
      "Induction Compatible": "No",
      "Warranty": "5 year warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Third-party Seller",
    "soldByRating": 4.6,
    "sellerSince": "2014",
    "fulfillment": "Seller fulfilled",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 87,
        "detail": null
      },
      "returnRate": {
        "score": 78,
        "detail": null
      },
      "warrantyClaims": {
        "score": 87,
        "detail": null
      },
      "sellerReliability": {
        "score": 91,
        "detail": null
      },
      "priceStability": {
        "score": 92,
        "detail": null
      }
    }
  },
  {
    "id": "dj52",
    "name": "Carbon Steel Wok",
    "brand": "Generic",
    "category": "Home & Kitchen",
    "price": 2489,
    "originalPrice": 2676,
    "discount": 7,
    "rating": 4.1,
    "reviewCount": 23125,
    "inStock": true,
    "isPrime": false,
    "delivery": "Get it by Tomorrow, 6 PM",
    "deliveryFree": true,
    "trustScore": 94,
    "trustLabel": "Genuine",
    "buyNowOrWait": "wait",
    "waitReason": "Sale expected in the next 2 weeks based on historical patterns.",
    "spikePriceMonths": [],
    "priceHistory": [
      2489,
      2489,
      2489,
      2762,
      2489,
      2489,
      2489,
      2489,
      2489,
      2489,
      2489,
      2489
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/kitchen-accessories/carbon-steel-wok/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/kitchen-accessories/carbon-steel-wok/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/kitchen-accessories/carbon-steel-wok/1.webp"
    ],
    "description": "The Carbon Steel Wok is a versatile cooking pan suitable for stir-frying, sautéing, and deep frying. Its sturdy construction ensures even heat distribution for delicious meals.",
    "features": [
      "The Carbon Steel Wok is a versatile cooking pan suitable for stir-frying, sautéing, and deep frying",
      "Made from durable cast iron — safe and built to last",
      "Generous capacity — ideal for everyday cooking, meal prep and entertaining",
      "Hand-wash only for convenient cleaning",
      "Compatible with all cooktop types"
    ],
    "specs": {
      "Material": "Anodised Aluminium",
      "Capacity": "5L",
      "Dimensions": "28W × 8H × 10D cm",
      "Weight": "2 kg",
      "Dishwasher Safe": "Yes",
      "Induction Compatible": "No",
      "Warranty": "2 year warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Third-party Seller",
    "soldByRating": 4.3,
    "sellerSince": "2016",
    "fulfillment": "Fulfilled by Amazon",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 92,
        "detail": null
      },
      "returnRate": {
        "score": 96,
        "detail": null
      },
      "warrantyClaims": {
        "score": 96,
        "detail": null
      },
      "sellerReliability": {
        "score": 96,
        "detail": null
      },
      "priceStability": {
        "score": 93,
        "detail": null
      }
    }
  },
  {
    "id": "dj53",
    "name": "Chopping Board",
    "brand": "Generic",
    "category": "Home & Kitchen",
    "price": 1078,
    "originalPrice": 1172,
    "discount": 8,
    "rating": 3.7,
    "reviewCount": 11853,
    "inStock": true,
    "isPrime": false,
    "delivery": "Get it by Tomorrow, 6 PM",
    "deliveryFree": true,
    "trustScore": 82,
    "trustLabel": "Genuine",
    "buyNowOrWait": "buy",
    "waitReason": null,
    "spikePriceMonths": [],
    "priceHistory": [
      1078,
      1078,
      1078,
      1078,
      1078,
      1078,
      1078,
      1078,
      1078,
      1078,
      1078,
      1078
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/kitchen-accessories/chopping-board/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/kitchen-accessories/chopping-board/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/kitchen-accessories/chopping-board/1.webp"
    ],
    "description": "The Chopping Board is an essential kitchen accessory for food preparation. Made from durable material, it provides a safe and hygienic surface for cutting and chopping.",
    "features": [
      "The Chopping Board is an essential kitchen accessory for food preparation",
      "Made from premium-grade polypropylene — safe and built to last",
      "Generous capacity — ideal for everyday cooking, meal prep and entertaining",
      "Top-rack dishwasher safe for convenient cleaning",
      "Compatible with gas, induction, and electric stovetops"
    ],
    "specs": {
      "Material": "BPA-free Polypropylene",
      "Capacity": "500ml",
      "Dimensions": "16W × 7H × 9D cm",
      "Weight": "2 kg",
      "Dishwasher Safe": "No",
      "Induction Compatible": "Yes",
      "Warranty": "3 months warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Third-party Seller",
    "soldByRating": 4.8,
    "sellerSince": "2014",
    "fulfillment": "Seller fulfilled",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 69,
        "detail": null
      },
      "returnRate": {
        "score": 86,
        "detail": null
      },
      "warrantyClaims": {
        "score": 79,
        "detail": null
      },
      "sellerReliability": {
        "score": 94,
        "detail": null
      },
      "priceStability": {
        "score": 78,
        "detail": null
      }
    }
  },
  {
    "id": "dj54",
    "name": "Citrus Squeezer Yellow",
    "brand": "Generic",
    "category": "Home & Kitchen",
    "price": 746,
    "originalPrice": 848,
    "discount": 12,
    "rating": 4.6,
    "reviewCount": 54096,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Day after Tomorrow",
    "deliveryFree": true,
    "trustScore": 95,
    "trustLabel": "Genuine",
    "buyNowOrWait": "buy",
    "waitReason": null,
    "spikePriceMonths": [
      0,
      5
    ],
    "priceHistory": [
      853,
      746,
      746,
      746,
      746,
      844,
      746,
      746,
      746,
      746,
      746,
      746
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/kitchen-accessories/citrus-squeezer-yellow/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/kitchen-accessories/citrus-squeezer-yellow/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/kitchen-accessories/citrus-squeezer-yellow/1.webp"
    ],
    "description": "The Citrus Squeezer in Yellow is a handy tool for extracting juice from citrus fruits. Its vibrant color adds a cheerful touch to your kitchen gadgets.",
    "features": [
      "The Citrus Squeezer in Yellow is a handy tool for extracting juice from citrus fruits",
      "Made from food-safe silicone — safe and built to last",
      "Generous capacity — ideal for everyday cooking, meal prep and entertaining",
      "Top-rack dishwasher safe for convenient cleaning",
      "Compatible with oven up to 250°C"
    ],
    "specs": {
      "Material": "Silicone",
      "Capacity": "500ml",
      "Dimensions": "17W × 27H × 16D cm",
      "Weight": "2 kg",
      "Dishwasher Safe": "Top Rack Only",
      "Induction Compatible": "Yes",
      "Warranty": "2 year warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Third-party Seller",
    "soldByRating": 4.4,
    "sellerSince": "2013",
    "fulfillment": "Seller fulfilled",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 87,
        "detail": null
      },
      "returnRate": {
        "score": 96,
        "detail": null
      },
      "warrantyClaims": {
        "score": 92,
        "detail": null
      },
      "sellerReliability": {
        "score": 96,
        "detail": null
      },
      "priceStability": {
        "score": 96,
        "detail": null
      }
    }
  },
  {
    "id": "dj55",
    "name": "Egg Slicer",
    "brand": "Generic",
    "category": "Home & Kitchen",
    "price": 580,
    "originalPrice": 682,
    "discount": 15,
    "rating": 3.1,
    "reviewCount": 70682,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Tomorrow, 6 PM",
    "deliveryFree": true,
    "trustScore": 40,
    "trustLabel": "Suspicious",
    "buyNowOrWait": "buy",
    "waitReason": null,
    "spikePriceMonths": [
      0,
      2
    ],
    "priceHistory": [
      745,
      591,
      695,
      580,
      580,
      580,
      580,
      580,
      572,
      580,
      580,
      580
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/kitchen-accessories/egg-slicer/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/kitchen-accessories/egg-slicer/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/kitchen-accessories/egg-slicer/1.webp"
    ],
    "description": "The Egg Slicer is a convenient tool for slicing boiled eggs evenly. It's perfect for salads, sandwiches, and other dishes where sliced eggs are desired.",
    "features": [
      "The Egg Slicer is a convenient tool for slicing boiled eggs evenly",
      "Made from food-safe silicone — safe and built to last",
      "Generous capacity — ideal for everyday cooking, meal prep and entertaining",
      "Easy-wipe for convenient cleaning",
      "Compatible with microwave and oven"
    ],
    "specs": {
      "Material": "Silicone",
      "Capacity": "3L",
      "Dimensions": "11W × 19H × 13D cm",
      "Weight": "2 kg",
      "Dishwasher Safe": "Yes",
      "Induction Compatible": "Yes",
      "Warranty": "1 week warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Third-party Seller",
    "soldByRating": 2.5,
    "sellerSince": "2021",
    "fulfillment": "Seller fulfilled",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 41,
        "detail": null
      },
      "returnRate": {
        "score": 33,
        "detail": null
      },
      "warrantyClaims": {
        "score": 32,
        "detail": null
      },
      "sellerReliability": {
        "score": 54,
        "detail": null
      },
      "priceStability": {
        "score": 45,
        "detail": null
      }
    }
  },
  {
    "id": "dj56",
    "name": "Electric Stove",
    "brand": "Generic",
    "category": "Home & Kitchen",
    "price": 4149,
    "originalPrice": 4824,
    "discount": 14,
    "rating": 4.1,
    "reviewCount": 28081,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Tomorrow, 6 PM",
    "deliveryFree": true,
    "trustScore": 84,
    "trustLabel": "Genuine",
    "buyNowOrWait": "wait",
    "waitReason": "Sale expected in the next 2 weeks based on historical patterns.",
    "spikePriceMonths": [
      8
    ],
    "priceHistory": [
      4149,
      4149,
      4149,
      4149,
      4149,
      4149,
      4149,
      4149,
      5253,
      4149,
      4492,
      4149
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/kitchen-accessories/electric-stove/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/kitchen-accessories/electric-stove/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/kitchen-accessories/electric-stove/1.webp",
      "https://cdn.dummyjson.com/product-images/kitchen-accessories/electric-stove/2.webp"
    ],
    "description": "The Electric Stove provides a portable and efficient cooking solution. Ideal for small kitchens or as an additional cooking surface for various culinary needs.",
    "features": [
      "The Electric Stove provides a portable and efficient cooking solution",
      "Made from food-grade stainless steel — safe and built to last",
      "Generous capacity — ideal for everyday cooking, meal prep and entertaining",
      "Dishwasher-safe for convenient cleaning",
      "Compatible with all cooktop types"
    ],
    "specs": {
      "Material": "Food-grade Stainless Steel",
      "Capacity": "3L",
      "Dimensions": "24W × 23H × 27D cm",
      "Weight": "5 kg",
      "Dishwasher Safe": "Yes",
      "Induction Compatible": "Yes",
      "Warranty": "2 year warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Third-party Seller",
    "soldByRating": 4.9,
    "sellerSince": "2014",
    "fulfillment": "Fulfilled by Amazon",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 94,
        "detail": null
      },
      "returnRate": {
        "score": 94,
        "detail": null
      },
      "warrantyClaims": {
        "score": 80,
        "detail": null
      },
      "sellerReliability": {
        "score": 86,
        "detail": null
      },
      "priceStability": {
        "score": 74,
        "detail": null
      }
    }
  },
  {
    "id": "dj57",
    "name": "Fine Mesh Strainer",
    "brand": "Generic",
    "category": "Home & Kitchen",
    "price": 829,
    "originalPrice": 864,
    "discount": 4,
    "rating": 3,
    "reviewCount": 8840,
    "inStock": true,
    "isPrime": false,
    "delivery": "Get it by Day after Tomorrow",
    "deliveryFree": true,
    "trustScore": 80,
    "trustLabel": "Genuine",
    "buyNowOrWait": "buy",
    "waitReason": null,
    "spikePriceMonths": [],
    "priceHistory": [
      829,
      829,
      829,
      829,
      829,
      829,
      829,
      829,
      829,
      890,
      829,
      829
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/kitchen-accessories/fine-mesh-strainer/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/kitchen-accessories/fine-mesh-strainer/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/kitchen-accessories/fine-mesh-strainer/1.webp"
    ],
    "description": "The Fine Mesh Strainer is a versatile tool for straining liquids and sifting dry ingredients. Its fine mesh ensures efficient filtering for smooth cooking and baking.",
    "features": [
      "The Fine Mesh Strainer is a versatile tool for straining liquids and sifting dry ingredients",
      "Made from BPA-free plastic — safe and built to last",
      "Generous capacity — ideal for everyday cooking, meal prep and entertaining",
      "Dishwasher-safe for convenient cleaning",
      "Compatible with gas, induction, and electric stovetops"
    ],
    "specs": {
      "Material": "Cast Iron",
      "Capacity": "2L",
      "Dimensions": "18W × 13H × 16D cm",
      "Weight": "9 kg",
      "Dishwasher Safe": "Top Rack Only",
      "Induction Compatible": "Yes",
      "Warranty": "1 month warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Third-party Seller",
    "soldByRating": 4.6,
    "sellerSince": "2015",
    "fulfillment": "Seller fulfilled",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 94,
        "detail": null
      },
      "returnRate": {
        "score": 85,
        "detail": null
      },
      "warrantyClaims": {
        "score": 87,
        "detail": null
      },
      "sellerReliability": {
        "score": 92,
        "detail": null
      },
      "priceStability": {
        "score": 79,
        "detail": null
      }
    }
  },
  {
    "id": "dj58",
    "name": "Fork",
    "brand": "Generic",
    "category": "Home & Kitchen",
    "price": 331,
    "originalPrice": 360,
    "discount": 8,
    "rating": 3.1,
    "reviewCount": 10167,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Day after Tomorrow",
    "deliveryFree": true,
    "trustScore": 81,
    "trustLabel": "Genuine",
    "buyNowOrWait": "wait",
    "waitReason": "Sale expected in the next 2 weeks based on historical patterns.",
    "spikePriceMonths": [
      0,
      3
    ],
    "priceHistory": [
      408,
      331,
      331,
      417,
      336,
      327,
      331,
      331,
      301,
      331,
      327,
      331
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/kitchen-accessories/fork/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/kitchen-accessories/fork/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/kitchen-accessories/fork/1.webp"
    ],
    "description": "The Fork is a classic utensil for various dining and serving purposes. Its durable and ergonomic design makes it a reliable choice for everyday use.",
    "features": [
      "The Fork is a classic utensil for various dining and serving purposes",
      "Made from BPA-free plastic — safe and built to last",
      "Generous capacity — ideal for everyday cooking, meal prep and entertaining",
      "Top-rack dishwasher safe for convenient cleaning",
      "Compatible with gas, induction, and electric stovetops"
    ],
    "specs": {
      "Material": "Cast Iron",
      "Capacity": "500ml",
      "Dimensions": "12W × 26H × 23D cm",
      "Weight": "9 kg",
      "Dishwasher Safe": "Yes",
      "Induction Compatible": "Yes",
      "Warranty": "No warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Third-party Seller",
    "soldByRating": 4.4,
    "sellerSince": "2015",
    "fulfillment": "Seller fulfilled",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 69,
        "detail": null
      },
      "returnRate": {
        "score": 78,
        "detail": null
      },
      "warrantyClaims": {
        "score": 83,
        "detail": null
      },
      "sellerReliability": {
        "score": 88,
        "detail": null
      },
      "priceStability": {
        "score": 77,
        "detail": null
      }
    }
  },
  {
    "id": "dj59",
    "name": "Glass",
    "brand": "Generic",
    "category": "Home & Kitchen",
    "price": 414,
    "originalPrice": 450,
    "discount": 8,
    "rating": 4,
    "reviewCount": 36245,
    "inStock": true,
    "isPrime": false,
    "delivery": "Get it by Tomorrow, 6 PM",
    "deliveryFree": true,
    "trustScore": 83,
    "trustLabel": "Genuine",
    "buyNowOrWait": "buy",
    "waitReason": null,
    "spikePriceMonths": [],
    "priceHistory": [
      386,
      414,
      414,
      414,
      414,
      414,
      414,
      414,
      435,
      443,
      414,
      414
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/kitchen-accessories/glass/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/kitchen-accessories/glass/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/kitchen-accessories/glass/1.webp"
    ],
    "description": "The Glass is a versatile and elegant drinking vessel suitable for a variety of beverages. Its clear design allows you to enjoy the colors and textures of your drinks.",
    "features": [
      "The Glass is a versatile and elegant drinking vessel suitable for a variety of beverages",
      "Made from food-grade stainless steel — safe and built to last",
      "Generous capacity — ideal for everyday cooking, meal prep and entertaining",
      "Hand-wash only for convenient cleaning",
      "Compatible with all cooktop types"
    ],
    "specs": {
      "Material": "Food-grade Stainless Steel",
      "Capacity": "5L",
      "Dimensions": "20W × 25H × 27D cm",
      "Weight": "10 kg",
      "Dishwasher Safe": "No",
      "Induction Compatible": "Yes",
      "Warranty": "3 year warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Third-party Seller",
    "soldByRating": 3.9,
    "sellerSince": "2012",
    "fulfillment": "Fulfilled by Amazon",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 84,
        "detail": null
      },
      "returnRate": {
        "score": 76,
        "detail": null
      },
      "warrantyClaims": {
        "score": 87,
        "detail": null
      },
      "sellerReliability": {
        "score": 91,
        "detail": null
      },
      "priceStability": {
        "score": 83,
        "detail": null
      }
    }
  },
  {
    "id": "dj60",
    "name": "Grater Black",
    "brand": "Generic",
    "category": "Home & Kitchen",
    "price": 912,
    "originalPrice": 950,
    "discount": 4,
    "rating": 3.2,
    "reviewCount": 46852,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Tomorrow, 6 PM",
    "deliveryFree": true,
    "trustScore": 85,
    "trustLabel": "Genuine",
    "buyNowOrWait": "wait",
    "waitReason": "Sale expected in the next 2 weeks based on historical patterns.",
    "spikePriceMonths": [],
    "priceHistory": [
      912,
      912,
      912,
      869,
      912,
      912,
      912,
      912,
      912,
      912,
      912,
      912
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/kitchen-accessories/grater-black/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/kitchen-accessories/grater-black/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/kitchen-accessories/grater-black/1.webp"
    ],
    "description": "The Grater in Black is a handy kitchen tool for grating cheese, vegetables, and more. Its sleek design and sharp blades make food preparation efficient and easy.",
    "features": [
      "The Grater in Black is a handy kitchen tool for grating cheese, vegetables, and more",
      "Made from food-safe silicone — safe and built to last",
      "Generous capacity — ideal for everyday cooking, meal prep and entertaining",
      "Hand-wash only for convenient cleaning",
      "Compatible with oven up to 250°C"
    ],
    "specs": {
      "Material": "Silicone",
      "Capacity": "1.5L",
      "Dimensions": "6W × 23H × 25D cm",
      "Weight": "3 kg",
      "Dishwasher Safe": "No",
      "Induction Compatible": "No",
      "Warranty": "1 month warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Third-party Seller",
    "soldByRating": 3.9,
    "sellerSince": "2010",
    "fulfillment": "Fulfilled by Amazon",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 75,
        "detail": null
      },
      "returnRate": {
        "score": 82,
        "detail": null
      },
      "warrantyClaims": {
        "score": 90,
        "detail": null
      },
      "sellerReliability": {
        "score": 90,
        "detail": null
      },
      "priceStability": {
        "score": 78,
        "detail": null
      }
    }
  },
  {
    "id": "dj61",
    "name": "Hand Blender",
    "brand": "Generic",
    "category": "Home & Kitchen",
    "price": 2904,
    "originalPrice": 3499,
    "discount": 17,
    "rating": 3.9,
    "reviewCount": 40620,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Tomorrow, 6 PM",
    "deliveryFree": true,
    "trustScore": 82,
    "trustLabel": "Genuine",
    "buyNowOrWait": "buy",
    "waitReason": null,
    "spikePriceMonths": [
      0,
      1
    ],
    "priceHistory": [
      3439,
      3929,
      2904,
      2904,
      2904,
      2904,
      2904,
      2904,
      3244,
      2904,
      2904,
      2904
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/kitchen-accessories/hand-blender/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/kitchen-accessories/hand-blender/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/kitchen-accessories/hand-blender/1.webp"
    ],
    "description": "The Hand Blender is a versatile kitchen appliance for blending, pureeing, and mixing. Its compact design and powerful motor make it a convenient tool for various recipes.",
    "features": [
      "The Hand Blender is a versatile kitchen appliance for blending, pureeing, and mixing",
      "Made from food-grade stainless steel — safe and built to last",
      "Generous capacity — ideal for everyday cooking, meal prep and entertaining",
      "Top-rack dishwasher safe for convenient cleaning",
      "Compatible with oven up to 250°C"
    ],
    "specs": {
      "Material": "Food-grade Stainless Steel",
      "Capacity": "500ml",
      "Dimensions": "27W × 21H × 24D cm",
      "Weight": "5 kg",
      "Dishwasher Safe": "Yes",
      "Induction Compatible": "Yes",
      "Warranty": "1 year warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Third-party Seller",
    "soldByRating": 4.9,
    "sellerSince": "2016",
    "fulfillment": "Fulfilled by Amazon",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 84,
        "detail": null
      },
      "returnRate": {
        "score": 81,
        "detail": null
      },
      "warrantyClaims": {
        "score": 74,
        "detail": null
      },
      "sellerReliability": {
        "score": 90,
        "detail": null
      },
      "priceStability": {
        "score": 87,
        "detail": null
      }
    }
  },
  {
    "id": "dj62",
    "name": "Ice Cube Tray",
    "brand": "Generic",
    "category": "Home & Kitchen",
    "price": 497,
    "originalPrice": 502,
    "discount": 1,
    "rating": 4.7,
    "reviewCount": 3677,
    "inStock": true,
    "isPrime": false,
    "delivery": "Get it by Tomorrow, 6 PM",
    "deliveryFree": true,
    "trustScore": 88,
    "trustLabel": "Genuine",
    "buyNowOrWait": "buy",
    "waitReason": null,
    "spikePriceMonths": [
      4
    ],
    "priceHistory": [
      464,
      497,
      497,
      497,
      564,
      497,
      497,
      477,
      497,
      497,
      497,
      497
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/kitchen-accessories/ice-cube-tray/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/kitchen-accessories/ice-cube-tray/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/kitchen-accessories/ice-cube-tray/1.webp"
    ],
    "description": "The Ice Cube Tray is a practical accessory for making ice cubes in various shapes. Perfect for keeping your drinks cool and adding a fun element to your beverages.",
    "features": [
      "The Ice Cube Tray is a practical accessory for making ice cubes in various shapes",
      "Made from food-grade stainless steel — safe and built to last",
      "Generous capacity — ideal for everyday cooking, meal prep and entertaining",
      "Top-rack dishwasher safe for convenient cleaning",
      "Compatible with gas, induction, and electric stovetops"
    ],
    "specs": {
      "Material": "Food-grade Stainless Steel",
      "Capacity": "500ml",
      "Dimensions": "27W × 18H × 5D cm",
      "Weight": "3 kg",
      "Dishwasher Safe": "No",
      "Induction Compatible": "Yes",
      "Warranty": "1 month warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Third-party Seller",
    "soldByRating": 4.6,
    "sellerSince": "2011",
    "fulfillment": "Fulfilled by Amazon",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 80,
        "detail": null
      },
      "returnRate": {
        "score": 83,
        "detail": null
      },
      "warrantyClaims": {
        "score": 93,
        "detail": null
      },
      "sellerReliability": {
        "score": 96,
        "detail": null
      },
      "priceStability": {
        "score": 88,
        "detail": null
      }
    }
  },
  {
    "id": "dj63",
    "name": "Kitchen Sieve",
    "brand": "Generic",
    "category": "Home & Kitchen",
    "price": 663,
    "originalPrice": 819,
    "discount": 19,
    "rating": 3.1,
    "reviewCount": 43548,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Day after Tomorrow",
    "deliveryFree": true,
    "trustScore": 37,
    "trustLabel": "Suspicious",
    "buyNowOrWait": "buy",
    "waitReason": null,
    "spikePriceMonths": [
      5
    ],
    "priceHistory": [
      663,
      718,
      724,
      663,
      663,
      822,
      663,
      663,
      663,
      712,
      714,
      663
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/kitchen-accessories/kitchen-sieve/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/kitchen-accessories/kitchen-sieve/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/kitchen-accessories/kitchen-sieve/1.webp"
    ],
    "description": "The Kitchen Sieve is a versatile tool for sifting and straining dry and wet ingredients. Its fine mesh design ensures smooth results in your cooking and baking.",
    "features": [
      "The Kitchen Sieve is a versatile tool for sifting and straining dry and wet ingredients",
      "Made from premium-grade polypropylene — safe and built to last",
      "Generous capacity — ideal for everyday cooking, meal prep and entertaining",
      "Dishwasher-safe for convenient cleaning",
      "Compatible with oven up to 250°C"
    ],
    "specs": {
      "Material": "BPA-free Polypropylene",
      "Capacity": "3L",
      "Dimensions": "21W × 7H × 8D cm",
      "Weight": "5 kg",
      "Dishwasher Safe": "Yes",
      "Induction Compatible": "Yes",
      "Warranty": "1 month warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Third-party Seller",
    "soldByRating": 2.3,
    "sellerSince": "2022",
    "fulfillment": "Seller fulfilled",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 32,
        "detail": null
      },
      "returnRate": {
        "score": 40,
        "detail": null
      },
      "warrantyClaims": {
        "score": 30,
        "detail": null
      },
      "sellerReliability": {
        "score": 50,
        "detail": null
      },
      "priceStability": {
        "score": 37,
        "detail": null
      }
    }
  },
  {
    "id": "dj64",
    "name": "Knife",
    "brand": "Generic",
    "category": "Home & Kitchen",
    "price": 1244,
    "originalPrice": 1536,
    "discount": 19,
    "rating": 3.3,
    "reviewCount": 30316,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Day after Tomorrow",
    "deliveryFree": true,
    "trustScore": 82,
    "trustLabel": "Genuine",
    "buyNowOrWait": "buy",
    "waitReason": null,
    "spikePriceMonths": [
      9
    ],
    "priceHistory": [
      1244,
      1244,
      1244,
      1261,
      1244,
      1244,
      1244,
      1244,
      1244,
      1510,
      1244,
      1244
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/kitchen-accessories/knife/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/kitchen-accessories/knife/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/kitchen-accessories/knife/1.webp"
    ],
    "description": "The Knife is an essential kitchen tool for chopping, slicing, and dicing. Its sharp blade and ergonomic handle make it a reliable choice for food preparation.",
    "features": [
      "The Knife is an essential kitchen tool for chopping, slicing, and dicing",
      "Made from BPA-free plastic — safe and built to last",
      "Generous capacity — ideal for everyday cooking, meal prep and entertaining",
      "Easy-wipe for convenient cleaning",
      "Compatible with all cooktop types"
    ],
    "specs": {
      "Material": "Cast Iron",
      "Capacity": "1.5L",
      "Dimensions": "25W × 19H × 21D cm",
      "Weight": "3 kg",
      "Dishwasher Safe": "Yes",
      "Induction Compatible": "Yes",
      "Warranty": "1 month warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Third-party Seller",
    "soldByRating": 4,
    "sellerSince": "2011",
    "fulfillment": "Seller fulfilled",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 74,
        "detail": null
      },
      "returnRate": {
        "score": 90,
        "detail": null
      },
      "warrantyClaims": {
        "score": 90,
        "detail": null
      },
      "sellerReliability": {
        "score": 94,
        "detail": null
      },
      "priceStability": {
        "score": 92,
        "detail": null
      }
    }
  },
  {
    "id": "dj65",
    "name": "Lunch Box",
    "brand": "Generic",
    "category": "Home & Kitchen",
    "price": 1078,
    "originalPrice": 1198,
    "discount": 10,
    "rating": 4.9,
    "reviewCount": 71582,
    "inStock": true,
    "isPrime": false,
    "delivery": "Get it by Day after Tomorrow",
    "deliveryFree": true,
    "trustScore": 91,
    "trustLabel": "Genuine",
    "buyNowOrWait": "wait",
    "waitReason": "Sale expected in the next 2 weeks based on historical patterns.",
    "spikePriceMonths": [
      7,
      8
    ],
    "priceHistory": [
      1078,
      1078,
      1078,
      1078,
      1024,
      1078,
      1078,
      1212,
      1379,
      1078,
      1078,
      1078
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/kitchen-accessories/lunch-box/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/kitchen-accessories/lunch-box/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/kitchen-accessories/lunch-box/1.webp"
    ],
    "description": "The Lunch Box is a convenient and portable container for packing and carrying your meals. With compartments for different foods, it's perfect for on-the-go dining.",
    "features": [
      "The Lunch Box is a convenient and portable container for packing and carrying your meals",
      "Made from food-grade stainless steel — safe and built to last",
      "Generous capacity — ideal for everyday cooking, meal prep and entertaining",
      "Easy-wipe for convenient cleaning",
      "Compatible with microwave and oven"
    ],
    "specs": {
      "Material": "Food-grade Stainless Steel",
      "Capacity": "3L",
      "Dimensions": "12W × 19H × 8D cm",
      "Weight": "9 kg",
      "Dishwasher Safe": "Top Rack Only",
      "Induction Compatible": "No",
      "Warranty": "5 year warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Third-party Seller",
    "soldByRating": 4.9,
    "sellerSince": "2019",
    "fulfillment": "Seller fulfilled",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 78,
        "detail": null
      },
      "returnRate": {
        "score": 83,
        "detail": null
      },
      "warrantyClaims": {
        "score": 94,
        "detail": null
      },
      "sellerReliability": {
        "score": 92,
        "detail": null
      },
      "priceStability": {
        "score": 96,
        "detail": null
      }
    }
  },
  {
    "id": "dj66",
    "name": "Microwave Oven",
    "brand": "Generic",
    "category": "Home & Kitchen",
    "price": 7469,
    "originalPrice": 8488,
    "discount": 12,
    "rating": 4.8,
    "reviewCount": 4591,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Tomorrow, 6 PM",
    "deliveryFree": true,
    "trustScore": 92,
    "trustLabel": "Genuine",
    "buyNowOrWait": "buy",
    "waitReason": null,
    "spikePriceMonths": [],
    "priceHistory": [
      7469,
      7469,
      7469,
      7469,
      7469,
      7469,
      7469,
      7469,
      7469,
      7469,
      7469,
      7469
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/kitchen-accessories/microwave-oven/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/kitchen-accessories/microwave-oven/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/kitchen-accessories/microwave-oven/1.webp",
      "https://cdn.dummyjson.com/product-images/kitchen-accessories/microwave-oven/2.webp"
    ],
    "description": "The Microwave Oven is a versatile kitchen appliance for quick and efficient cooking, reheating, and defrosting. Its compact size makes it suitable for various kitchen setups.",
    "features": [
      "The Microwave Oven is a versatile kitchen appliance for quick and efficient cooking, reheating, and defrosting",
      "Made from premium-grade polypropylene — safe and built to last",
      "Generous capacity — ideal for everyday cooking, meal prep and entertaining",
      "Dishwasher-safe for convenient cleaning",
      "Compatible with gas, induction, and electric stovetops"
    ],
    "specs": {
      "Material": "BPA-free Polypropylene",
      "Capacity": "3L",
      "Dimensions": "16W × 27H × 13D cm",
      "Weight": "9 kg",
      "Dishwasher Safe": "No",
      "Induction Compatible": "Yes",
      "Warranty": "1 year warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Third-party Seller",
    "soldByRating": 4.8,
    "sellerSince": "2020",
    "fulfillment": "Seller fulfilled",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 85,
        "detail": null
      },
      "returnRate": {
        "score": 96,
        "detail": null
      },
      "warrantyClaims": {
        "score": 92,
        "detail": null
      },
      "sellerReliability": {
        "score": 96,
        "detail": null
      },
      "priceStability": {
        "score": 83,
        "detail": null
      }
    }
  },
  {
    "id": "dj67",
    "name": "Mug Tree Stand",
    "brand": "Generic",
    "category": "Home & Kitchen",
    "price": 1327,
    "originalPrice": 1458,
    "discount": 9,
    "rating": 2.6,
    "reviewCount": 15221,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Tomorrow, 6 PM",
    "deliveryFree": true,
    "trustScore": 77,
    "trustLabel": "Genuine",
    "buyNowOrWait": "wait",
    "waitReason": "Sale expected in the next 2 weeks based on historical patterns.",
    "spikePriceMonths": [
      4,
      6
    ],
    "priceHistory": [
      1327,
      1327,
      1327,
      1327,
      1629,
      1327,
      1575,
      1327,
      1327,
      1327,
      1327,
      1327
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/kitchen-accessories/mug-tree-stand/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/kitchen-accessories/mug-tree-stand/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/kitchen-accessories/mug-tree-stand/1.webp",
      "https://cdn.dummyjson.com/product-images/kitchen-accessories/mug-tree-stand/2.webp"
    ],
    "description": "The Mug Tree Stand is a stylish and space-saving solution for organizing your mugs. Keep your favorite mugs easily accessible and neatly displayed in your kitchen.",
    "features": [
      "The Mug Tree Stand is a stylish and space-saving solution for organizing your mugs",
      "Made from BPA-free plastic — safe and built to last",
      "Generous capacity — ideal for everyday cooking, meal prep and entertaining",
      "Dishwasher-safe for convenient cleaning",
      "Compatible with gas, induction, and electric stovetops"
    ],
    "specs": {
      "Material": "Cast Iron",
      "Capacity": "3L",
      "Dimensions": "19W × 27H × 27D cm",
      "Weight": "3 kg",
      "Dishwasher Safe": "Yes",
      "Induction Compatible": "Yes",
      "Warranty": "1 year warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Third-party Seller",
    "soldByRating": 4.8,
    "sellerSince": "2019",
    "fulfillment": "Fulfilled by Amazon",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 90,
        "detail": null
      },
      "returnRate": {
        "score": 72,
        "detail": null
      },
      "warrantyClaims": {
        "score": 75,
        "detail": null
      },
      "sellerReliability": {
        "score": 86,
        "detail": null
      },
      "priceStability": {
        "score": 81,
        "detail": null
      }
    }
  },
  {
    "id": "dj68",
    "name": "Pan",
    "brand": "Generic",
    "category": "Home & Kitchen",
    "price": 2074,
    "originalPrice": 2138,
    "discount": 3,
    "rating": 2.8,
    "reviewCount": 49480,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Tomorrow, 6 PM",
    "deliveryFree": true,
    "trustScore": 77,
    "trustLabel": "Genuine",
    "buyNowOrWait": "wait",
    "waitReason": "Sale expected in the next 2 weeks based on historical patterns.",
    "spikePriceMonths": [
      9
    ],
    "priceHistory": [
      2074,
      2074,
      2074,
      2074,
      2074,
      2074,
      2074,
      1998,
      2074,
      2332,
      2074,
      2074
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/kitchen-accessories/pan/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/kitchen-accessories/pan/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/kitchen-accessories/pan/1.webp"
    ],
    "description": "The Pan is a versatile and essential cookware item for frying, sautéing, and cooking various dishes. Its non-stick coating ensures easy food release and cleanup.",
    "features": [
      "The Pan is a versatile and essential cookware item for frying, sautéing, and cooking various dishes",
      "Made from BPA-free plastic — safe and built to last",
      "Generous capacity — ideal for everyday cooking, meal prep and entertaining",
      "Easy-wipe for convenient cleaning",
      "Compatible with oven up to 250°C"
    ],
    "specs": {
      "Material": "Cast Iron",
      "Capacity": "3L",
      "Dimensions": "17W × 22H × 26D cm",
      "Weight": "8 kg",
      "Dishwasher Safe": "No",
      "Induction Compatible": "No",
      "Warranty": "1 year warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Third-party Seller",
    "soldByRating": 4.6,
    "sellerSince": "2011",
    "fulfillment": "Fulfilled by Amazon",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 83,
        "detail": null
      },
      "returnRate": {
        "score": 70,
        "detail": null
      },
      "warrantyClaims": {
        "score": 79,
        "detail": null
      },
      "sellerReliability": {
        "score": 86,
        "detail": null
      },
      "priceStability": {
        "score": 72,
        "detail": null
      }
    }
  },
  {
    "id": "dj69",
    "name": "Plate",
    "brand": "Generic",
    "category": "Home & Kitchen",
    "price": 331,
    "originalPrice": 356,
    "discount": 7,
    "rating": 3.7,
    "reviewCount": 51077,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Day after Tomorrow",
    "deliveryFree": true,
    "trustScore": 88,
    "trustLabel": "Genuine",
    "buyNowOrWait": "buy",
    "waitReason": null,
    "spikePriceMonths": [],
    "priceHistory": [
      331,
      354,
      331,
      331,
      331,
      331,
      331,
      352,
      336,
      331,
      331,
      331
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/kitchen-accessories/plate/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/kitchen-accessories/plate/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/kitchen-accessories/plate/1.webp"
    ],
    "description": "The Plate is a classic and essential dishware item for serving meals. Its durable and stylish design makes it suitable for everyday use or special occasions.",
    "features": [
      "The Plate is a classic and essential dishware item for serving meals",
      "Made from food-safe silicone — safe and built to last",
      "Generous capacity — ideal for everyday cooking, meal prep and entertaining",
      "Top-rack dishwasher safe for convenient cleaning",
      "Compatible with oven up to 250°C"
    ],
    "specs": {
      "Material": "Silicone",
      "Capacity": "5L",
      "Dimensions": "16W × 5H × 13D cm",
      "Weight": "4 kg",
      "Dishwasher Safe": "Yes",
      "Induction Compatible": "Yes",
      "Warranty": "1 year warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Third-party Seller",
    "soldByRating": 4.7,
    "sellerSince": "2018",
    "fulfillment": "Seller fulfilled",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 88,
        "detail": null
      },
      "returnRate": {
        "score": 78,
        "detail": null
      },
      "warrantyClaims": {
        "score": 92,
        "detail": null
      },
      "sellerReliability": {
        "score": 96,
        "detail": null
      },
      "priceStability": {
        "score": 93,
        "detail": null
      }
    }
  },
  {
    "id": "dj70",
    "name": "Red Tongs",
    "brand": "Generic",
    "category": "Home & Kitchen",
    "price": 580,
    "originalPrice": 682,
    "discount": 15,
    "rating": 4.4,
    "reviewCount": 27721,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Day after Tomorrow",
    "deliveryFree": true,
    "trustScore": 91,
    "trustLabel": "Genuine",
    "buyNowOrWait": "wait",
    "waitReason": "Sale expected in the next 2 weeks based on historical patterns.",
    "spikePriceMonths": [
      7
    ],
    "priceHistory": [
      580,
      633,
      580,
      580,
      580,
      580,
      580,
      714,
      580,
      580,
      580,
      580
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/kitchen-accessories/red-tongs/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/kitchen-accessories/red-tongs/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/kitchen-accessories/red-tongs/1.webp"
    ],
    "description": "The Red Tongs are versatile kitchen tongs suitable for various cooking and serving tasks. Their vibrant color adds a pop of excitement to your kitchen utensils.",
    "features": [
      "The Red Tongs are versatile kitchen tongs suitable for various cooking and serving tasks",
      "Made from BPA-free plastic — safe and built to last",
      "Generous capacity — ideal for everyday cooking, meal prep and entertaining",
      "Dishwasher-safe for convenient cleaning",
      "Compatible with all cooktop types"
    ],
    "specs": {
      "Material": "Cast Iron",
      "Capacity": "3L",
      "Dimensions": "23W × 26H × 17D cm",
      "Weight": "7 kg",
      "Dishwasher Safe": "Top Rack Only",
      "Induction Compatible": "Yes",
      "Warranty": "No warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Third-party Seller",
    "soldByRating": 4.3,
    "sellerSince": "2013",
    "fulfillment": "Seller fulfilled",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 96,
        "detail": null
      },
      "returnRate": {
        "score": 88,
        "detail": null
      },
      "warrantyClaims": {
        "score": 96,
        "detail": null
      },
      "sellerReliability": {
        "score": 92,
        "detail": null
      },
      "priceStability": {
        "score": 86,
        "detail": null
      }
    }
  },
  {
    "id": "dj71",
    "name": "Silver Pot With Glass Cap",
    "brand": "Generic",
    "category": "Home & Kitchen",
    "price": 3319,
    "originalPrice": 3531,
    "discount": 6,
    "rating": 3.2,
    "reviewCount": 58326,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Tomorrow, 6 PM",
    "deliveryFree": true,
    "trustScore": 85,
    "trustLabel": "Genuine",
    "buyNowOrWait": "wait",
    "waitReason": "Sale expected in the next 2 weeks based on historical patterns.",
    "spikePriceMonths": [],
    "priceHistory": [
      3319,
      3319,
      3319,
      3319,
      3319,
      3319,
      3319,
      3319,
      3319,
      3319,
      3319,
      3319
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/kitchen-accessories/silver-pot-with-glass-cap/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/kitchen-accessories/silver-pot-with-glass-cap/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/kitchen-accessories/silver-pot-with-glass-cap/1.webp"
    ],
    "description": "The Silver Pot with Glass Cap is a stylish and functional cookware item for boiling, simmering, and preparing delicious meals. Its glass cap allows you to monitor cooking progress.",
    "features": [
      "The Silver Pot with Glass Cap is a stylish and functional cookware item for boiling, simmering, and preparing delicious meals",
      "Made from food-safe silicone — safe and built to last",
      "Generous capacity — ideal for everyday cooking, meal prep and entertaining",
      "Easy-wipe for convenient cleaning",
      "Compatible with microwave and oven"
    ],
    "specs": {
      "Material": "Silicone",
      "Capacity": "1.5L",
      "Dimensions": "21W × 12H × 7D cm",
      "Weight": "7 kg",
      "Dishwasher Safe": "Yes",
      "Induction Compatible": "No",
      "Warranty": "1 year warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Third-party Seller",
    "soldByRating": 4.9,
    "sellerSince": "2015",
    "fulfillment": "Seller fulfilled",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 88,
        "detail": null
      },
      "returnRate": {
        "score": 86,
        "detail": null
      },
      "warrantyClaims": {
        "score": 77,
        "detail": null
      },
      "sellerReliability": {
        "score": 96,
        "detail": null
      },
      "priceStability": {
        "score": 88,
        "detail": null
      }
    }
  },
  {
    "id": "dj72",
    "name": "Slotted Turner",
    "brand": "Generic",
    "category": "Home & Kitchen",
    "price": 746,
    "originalPrice": 857,
    "discount": 13,
    "rating": 3.4,
    "reviewCount": 11784,
    "inStock": true,
    "isPrime": false,
    "delivery": "Get it by Tomorrow, 6 PM",
    "deliveryFree": true,
    "trustScore": 78,
    "trustLabel": "Genuine",
    "buyNowOrWait": "wait",
    "waitReason": "Sale expected in the next 2 weeks based on historical patterns.",
    "spikePriceMonths": [
      5,
      9
    ],
    "priceHistory": [
      705,
      746,
      746,
      746,
      746,
      981,
      746,
      746,
      746,
      923,
      746,
      746
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/kitchen-accessories/slotted-turner/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/kitchen-accessories/slotted-turner/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/kitchen-accessories/slotted-turner/1.webp"
    ],
    "description": "The Slotted Turner is a kitchen utensil designed for flipping and turning food items. Its slotted design allows excess liquid to drain, making it ideal for frying and sautéing.",
    "features": [
      "The Slotted Turner is a kitchen utensil designed for flipping and turning food items",
      "Made from food-grade stainless steel — safe and built to last",
      "Generous capacity — ideal for everyday cooking, meal prep and entertaining",
      "Hand-wash only for convenient cleaning",
      "Compatible with gas, induction, and electric stovetops"
    ],
    "specs": {
      "Material": "Food-grade Stainless Steel",
      "Capacity": "5L",
      "Dimensions": "14W × 27H × 15D cm",
      "Weight": "8 kg",
      "Dishwasher Safe": "No",
      "Induction Compatible": "Yes",
      "Warranty": "Lifetime warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Third-party Seller",
    "soldByRating": 3.9,
    "sellerSince": "2021",
    "fulfillment": "Fulfilled by Amazon",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 76,
        "detail": null
      },
      "returnRate": {
        "score": 81,
        "detail": null
      },
      "warrantyClaims": {
        "score": 80,
        "detail": null
      },
      "sellerReliability": {
        "score": 93,
        "detail": null
      },
      "priceStability": {
        "score": 70,
        "detail": null
      }
    }
  },
  {
    "id": "dj73",
    "name": "Spice Rack",
    "brand": "Generic",
    "category": "Home & Kitchen",
    "price": 1659,
    "originalPrice": 1885,
    "discount": 12,
    "rating": 4.9,
    "reviewCount": 16464,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Tomorrow, 6 PM",
    "deliveryFree": true,
    "trustScore": 93,
    "trustLabel": "Genuine",
    "buyNowOrWait": "wait",
    "waitReason": "Sale expected in the next 2 weeks based on historical patterns.",
    "spikePriceMonths": [
      6
    ],
    "priceHistory": [
      1659,
      1659,
      1659,
      1659,
      1659,
      1659,
      2021,
      1659,
      1823,
      1659,
      1659,
      1659
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/kitchen-accessories/spice-rack/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/kitchen-accessories/spice-rack/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/kitchen-accessories/spice-rack/1.webp"
    ],
    "description": "The Spice Rack is a convenient organizer for your spices and seasonings. Keep your kitchen essentials within reach and neatly arranged with this stylish spice rack.",
    "features": [
      "The Spice Rack is a convenient organizer for your spices and seasonings",
      "Made from premium-grade polypropylene — safe and built to last",
      "Generous capacity — ideal for everyday cooking, meal prep and entertaining",
      "Dishwasher-safe for convenient cleaning",
      "Compatible with gas, induction, and electric stovetops"
    ],
    "specs": {
      "Material": "BPA-free Polypropylene",
      "Capacity": "3L",
      "Dimensions": "28W × 22H × 26D cm",
      "Weight": "7 kg",
      "Dishwasher Safe": "No",
      "Induction Compatible": "No",
      "Warranty": "1 week warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Third-party Seller",
    "soldByRating": 4.2,
    "sellerSince": "2020",
    "fulfillment": "Seller fulfilled",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 96,
        "detail": null
      },
      "returnRate": {
        "score": 86,
        "detail": null
      },
      "warrantyClaims": {
        "score": 89,
        "detail": null
      },
      "sellerReliability": {
        "score": 96,
        "detail": null
      },
      "priceStability": {
        "score": 89,
        "detail": null
      }
    }
  },
  {
    "id": "dj74",
    "name": "Spoon",
    "brand": "Generic",
    "category": "Home & Kitchen",
    "price": 414,
    "originalPrice": 422,
    "discount": 2,
    "rating": 4,
    "reviewCount": 17907,
    "inStock": true,
    "isPrime": false,
    "delivery": "Get it by Tomorrow, 6 PM",
    "deliveryFree": true,
    "trustScore": 85,
    "trustLabel": "Genuine",
    "buyNowOrWait": "buy",
    "waitReason": null,
    "spikePriceMonths": [
      4,
      7,
      8
    ],
    "priceHistory": [
      414,
      414,
      414,
      414,
      470,
      414,
      414,
      465,
      492,
      414,
      414,
      414
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/kitchen-accessories/spoon/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/kitchen-accessories/spoon/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/kitchen-accessories/spoon/1.webp"
    ],
    "description": "The Spoon is a versatile kitchen utensil for stirring, serving, and tasting. Its ergonomic design and durable construction make it an essential tool for every kitchen.",
    "features": [
      "The Spoon is a versatile kitchen utensil for stirring, serving, and tasting",
      "Made from premium-grade polypropylene — safe and built to last",
      "Generous capacity — ideal for everyday cooking, meal prep and entertaining",
      "Hand-wash only for convenient cleaning",
      "Compatible with gas, induction, and electric stovetops"
    ],
    "specs": {
      "Material": "BPA-free Polypropylene",
      "Capacity": "5L",
      "Dimensions": "8W × 26H × 28D cm",
      "Weight": "6 kg",
      "Dishwasher Safe": "No",
      "Induction Compatible": "Yes",
      "Warranty": "1 year warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Third-party Seller",
    "soldByRating": 4.1,
    "sellerSince": "2013",
    "fulfillment": "Fulfilled by Amazon",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 79,
        "detail": null
      },
      "returnRate": {
        "score": 79,
        "detail": null
      },
      "warrantyClaims": {
        "score": 84,
        "detail": null
      },
      "sellerReliability": {
        "score": 89,
        "detail": null
      },
      "priceStability": {
        "score": 86,
        "detail": null
      }
    }
  },
  {
    "id": "dj75",
    "name": "Tray",
    "brand": "Generic",
    "category": "Home & Kitchen",
    "price": 1410,
    "originalPrice": 1516,
    "discount": 7,
    "rating": 4.6,
    "reviewCount": 29748,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Day after Tomorrow",
    "deliveryFree": true,
    "trustScore": 91,
    "trustLabel": "Genuine",
    "buyNowOrWait": "buy",
    "waitReason": null,
    "spikePriceMonths": [
      3
    ],
    "priceHistory": [
      1410,
      1410,
      1410,
      1674,
      1410,
      1329,
      1410,
      1410,
      1281,
      1410,
      1410,
      1410
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/kitchen-accessories/tray/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/kitchen-accessories/tray/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/kitchen-accessories/tray/1.webp"
    ],
    "description": "The Tray is a functional and decorative item for serving snacks, appetizers, or drinks. Its stylish design makes it a versatile accessory for entertaining guests.",
    "features": [
      "The Tray is a functional and decorative item for serving snacks, appetizers, or drinks",
      "Made from premium-grade polypropylene — safe and built to last",
      "Generous capacity — ideal for everyday cooking, meal prep and entertaining",
      "Top-rack dishwasher safe for convenient cleaning",
      "Compatible with all cooktop types"
    ],
    "specs": {
      "Material": "BPA-free Polypropylene",
      "Capacity": "500ml",
      "Dimensions": "13W × 8H × 10D cm",
      "Weight": "10 kg",
      "Dishwasher Safe": "Top Rack Only",
      "Induction Compatible": "Yes",
      "Warranty": "5 year warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Third-party Seller",
    "soldByRating": 4.9,
    "sellerSince": "2020",
    "fulfillment": "Fulfilled by Amazon",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 96,
        "detail": null
      },
      "returnRate": {
        "score": 81,
        "detail": null
      },
      "warrantyClaims": {
        "score": 96,
        "detail": null
      },
      "sellerReliability": {
        "score": 96,
        "detail": null
      },
      "priceStability": {
        "score": 95,
        "detail": null
      }
    }
  },
  {
    "id": "dj76",
    "name": "Wooden Rolling Pin",
    "brand": "Generic",
    "category": "Home & Kitchen",
    "price": 995,
    "originalPrice": 1106,
    "discount": 10,
    "rating": 2.9,
    "reviewCount": 1792,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Day after Tomorrow",
    "deliveryFree": true,
    "trustScore": 83,
    "trustLabel": "Genuine",
    "buyNowOrWait": "buy",
    "waitReason": null,
    "spikePriceMonths": [
      6,
      8
    ],
    "priceHistory": [
      1074,
      995,
      995,
      995,
      1104,
      995,
      1127,
      995,
      1147,
      995,
      995,
      995
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/kitchen-accessories/wooden-rolling-pin/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/kitchen-accessories/wooden-rolling-pin/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/kitchen-accessories/wooden-rolling-pin/1.webp"
    ],
    "description": "The Wooden Rolling Pin is a classic kitchen tool for rolling out dough for baking. Its smooth surface and sturdy handles make it easy to achieve uniform thickness.",
    "features": [
      "The Wooden Rolling Pin is a classic kitchen tool for rolling out dough for baking",
      "Made from durable cast iron — safe and built to last",
      "Generous capacity — ideal for everyday cooking, meal prep and entertaining",
      "Dishwasher-safe for convenient cleaning",
      "Compatible with gas, induction, and electric stovetops"
    ],
    "specs": {
      "Material": "Anodised Aluminium",
      "Capacity": "2L",
      "Dimensions": "6W × 11H × 20D cm",
      "Weight": "7 kg",
      "Dishwasher Safe": "Yes",
      "Induction Compatible": "Yes",
      "Warranty": "3 year warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Third-party Seller",
    "soldByRating": 4.8,
    "sellerSince": "2015",
    "fulfillment": "Seller fulfilled",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 92,
        "detail": null
      },
      "returnRate": {
        "score": 90,
        "detail": null
      },
      "warrantyClaims": {
        "score": 78,
        "detail": null
      },
      "sellerReliability": {
        "score": 89,
        "detail": null
      },
      "priceStability": {
        "score": 75,
        "detail": null
      }
    }
  },
  {
    "id": "dj77",
    "name": "Yellow Peeler",
    "brand": "Generic",
    "category": "Home & Kitchen",
    "price": 497,
    "originalPrice": 565,
    "discount": 12,
    "rating": 4.2,
    "reviewCount": 66170,
    "inStock": true,
    "isPrime": false,
    "delivery": "Get it by Tomorrow, 6 PM",
    "deliveryFree": true,
    "trustScore": 86,
    "trustLabel": "Genuine",
    "buyNowOrWait": "buy",
    "waitReason": null,
    "spikePriceMonths": [
      7,
      10
    ],
    "priceHistory": [
      497,
      497,
      497,
      497,
      497,
      497,
      497,
      642,
      497,
      497,
      598,
      497
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/kitchen-accessories/yellow-peeler/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/kitchen-accessories/yellow-peeler/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/kitchen-accessories/yellow-peeler/1.webp"
    ],
    "description": "The Yellow Peeler is a handy tool for peeling fruits and vegetables with ease. Its bright yellow color adds a cheerful touch to your kitchen gadgets.",
    "features": [
      "The Yellow Peeler is a handy tool for peeling fruits and vegetables with ease",
      "Made from food-grade stainless steel — safe and built to last",
      "Generous capacity — ideal for everyday cooking, meal prep and entertaining",
      "Easy-wipe for convenient cleaning",
      "Compatible with microwave and oven"
    ],
    "specs": {
      "Material": "Food-grade Stainless Steel",
      "Capacity": "3L",
      "Dimensions": "10W × 6H × 18D cm",
      "Weight": "7 kg",
      "Dishwasher Safe": "No",
      "Induction Compatible": "Yes",
      "Warranty": "1 month warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Third-party Seller",
    "soldByRating": 4.6,
    "sellerSince": "2017",
    "fulfillment": "Seller fulfilled",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 90,
        "detail": null
      },
      "returnRate": {
        "score": 76,
        "detail": null
      },
      "warrantyClaims": {
        "score": 84,
        "detail": null
      },
      "sellerReliability": {
        "score": 96,
        "detail": null
      },
      "priceStability": {
        "score": 78,
        "detail": null
      }
    }
  },
  {
    "id": "dj78",
    "name": "Apple MacBook Pro 14 Inch Space Grey",
    "brand": "Apple",
    "category": "Electronics > Computers",
    "price": 165999,
    "originalPrice": 174736,
    "discount": 5,
    "rating": 3.7,
    "reviewCount": 56114,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Tomorrow, 6 PM",
    "deliveryFree": true,
    "trustScore": 86,
    "trustLabel": "Genuine",
    "buyNowOrWait": "wait",
    "waitReason": "Sale expected in the next 2 weeks based on historical patterns.",
    "spikePriceMonths": [
      10
    ],
    "priceHistory": [
      165999,
      165999,
      165999,
      165999,
      165999,
      165999,
      151968,
      165999,
      154065,
      165999,
      189531,
      165999
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/laptops/apple-macbook-pro-14-inch-space-grey/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/laptops/apple-macbook-pro-14-inch-space-grey/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/laptops/apple-macbook-pro-14-inch-space-grey/1.webp",
      "https://cdn.dummyjson.com/product-images/laptops/apple-macbook-pro-14-inch-space-grey/2.webp"
    ],
    "description": "The MacBook Pro 14 Inch in Space Grey is a powerful and sleek laptop, featuring Apple's M1 Pro chip for exceptional performance and a stunning Retina display.",
    "features": [
      "The MacBook Pro 14 Inch in Space Grey is a powerful and sleek laptop, featuring Apple's M1 Pro chip for exceptional performance and a stunning Retina display",
      "Powered by AMD Ryzen 7 processor for fast, responsive computing",
      "32GB DDR5 RAM — runs multiple apps without slowdown",
      "1TB high-speed solid-state storage for instant boot and fast file access",
      "14\" 4K display — vibrant visuals for work and entertainment"
    ],
    "specs": {
      "Brand": "Apple",
      "Display": "14\" 4K OLED",
      "Processor": "AMD Ryzen 7 6800H",
      "RAM": "32GB DDR5",
      "Storage": "1TB NVMe SSD",
      "Graphics": "NVIDIA GeForce RTX 3050",
      "Battery": "86Whr — up to 6 hours",
      "OS": "DOS (No OS)",
      "Weight": "2 kg",
      "Warranty": "3 year warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Apple",
    "soldByRating": 4.7,
    "sellerSince": "2016",
    "fulfillment": "Fulfilled by Amazon",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 96,
        "detail": null
      },
      "returnRate": {
        "score": 81,
        "detail": null
      },
      "warrantyClaims": {
        "score": 81,
        "detail": null
      },
      "sellerReliability": {
        "score": 95,
        "detail": null
      },
      "priceStability": {
        "score": 93,
        "detail": null
      }
    }
  },
  {
    "id": "dj79",
    "name": "Asus Zenbook Pro Dual Screen Laptop",
    "brand": "Asus",
    "category": "Electronics > Computers",
    "price": 149399,
    "originalPrice": 167864,
    "discount": 11,
    "rating": 4,
    "reviewCount": 67613,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Tomorrow, 6 PM",
    "deliveryFree": true,
    "trustScore": 89,
    "trustLabel": "Genuine",
    "buyNowOrWait": "buy",
    "waitReason": null,
    "spikePriceMonths": [],
    "priceHistory": [
      138058,
      149399,
      149399,
      149399,
      138212,
      149399,
      149399,
      149399,
      149399,
      149399,
      149399,
      149399
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/laptops/asus-zenbook-pro-dual-screen-laptop/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/laptops/asus-zenbook-pro-dual-screen-laptop/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/laptops/asus-zenbook-pro-dual-screen-laptop/1.webp",
      "https://cdn.dummyjson.com/product-images/laptops/asus-zenbook-pro-dual-screen-laptop/2.webp"
    ],
    "description": "The Asus Zenbook Pro Dual Screen Laptop is a high-performance device with dual screens, providing productivity and versatility for creative professionals.",
    "features": [
      "The Asus Zenbook Pro Dual Screen Laptop is a high-performance device with dual screens, providing productivity and versatility for creative professionals",
      "Powered by AMD Ryzen 7 processor for fast, responsive computing",
      "8GB DDR4 RAM — runs multiple apps without slowdown",
      "512GB high-speed solid-state storage for instant boot and fast file access",
      "14\" Full HD display — vibrant visuals for work and entertainment"
    ],
    "specs": {
      "Brand": "Asus",
      "Display": "14\" Full HD TN",
      "Processor": "AMD Ryzen 7 6800H",
      "RAM": "8GB DDR4",
      "Storage": "1TB NVMe SSD",
      "Graphics": "NVIDIA GeForce RTX 4060",
      "Battery": "86Whr — up to 12 hours",
      "OS": "Windows 11 Home",
      "Weight": "2.6 kg",
      "Warranty": "3 year warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Asus",
    "soldByRating": 4.7,
    "sellerSince": "2021",
    "fulfillment": "Seller fulfilled",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 86,
        "detail": null
      },
      "returnRate": {
        "score": 86,
        "detail": null
      },
      "warrantyClaims": {
        "score": 89,
        "detail": null
      },
      "sellerReliability": {
        "score": 96,
        "detail": null
      },
      "priceStability": {
        "score": 86,
        "detail": null
      }
    }
  },
  {
    "id": "dj80",
    "name": "Huawei Matebook X Pro",
    "brand": "Huawei",
    "category": "Electronics > Computers",
    "price": 116199,
    "originalPrice": 127691,
    "discount": 9,
    "rating": 5,
    "reviewCount": 59683,
    "inStock": true,
    "isPrime": false,
    "delivery": "Get it by Tomorrow, 6 PM",
    "deliveryFree": true,
    "trustScore": 93,
    "trustLabel": "Genuine",
    "buyNowOrWait": "buy",
    "waitReason": null,
    "spikePriceMonths": [
      0
    ],
    "priceHistory": [
      146272,
      116199,
      116199,
      116199,
      116199,
      116199,
      116199,
      120491,
      116199,
      116199,
      106296,
      116199
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/laptops/huawei-matebook-x-pro/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/laptops/huawei-matebook-x-pro/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/laptops/huawei-matebook-x-pro/1.webp",
      "https://cdn.dummyjson.com/product-images/laptops/huawei-matebook-x-pro/2.webp"
    ],
    "description": "The Huawei Matebook X Pro is a slim and stylish laptop with a high-resolution touchscreen display, offering a premium experience for users on the go.",
    "features": [
      "The Huawei Matebook X Pro is a slim and stylish laptop with a high-resolution touchscreen display, offering a premium experience for users on the go",
      "Powered by Intel Core i5 processor for fast, responsive computing",
      "8GB DDR4 RAM — runs multiple apps without slowdown",
      "512GB high-speed solid-state storage for instant boot and fast file access",
      "14\" Full HD display — vibrant visuals for work and entertainment"
    ],
    "specs": {
      "Brand": "Huawei",
      "Display": "14\" Full HD TN",
      "Processor": "Intel Core i5-1235U",
      "RAM": "8GB DDR4",
      "Storage": "1TB NVMe SSD",
      "Graphics": "NVIDIA GeForce RTX 4060",
      "Battery": "86Whr — up to 6 hours",
      "OS": "Windows 10 Home",
      "Weight": "2.4 kg",
      "Warranty": "No warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Huawei",
    "soldByRating": 4.9,
    "sellerSince": "2017",
    "fulfillment": "Fulfilled by Amazon",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 96,
        "detail": null
      },
      "returnRate": {
        "score": 96,
        "detail": null
      },
      "warrantyClaims": {
        "score": 87,
        "detail": null
      },
      "sellerReliability": {
        "score": 96,
        "detail": null
      },
      "priceStability": {
        "score": 89,
        "detail": null
      }
    }
  },
  {
    "id": "dj81",
    "name": "Lenovo Yoga 920",
    "brand": "Lenovo",
    "category": "Electronics > Computers",
    "price": 91299,
    "originalPrice": 98171,
    "discount": 7,
    "rating": 2.9,
    "reviewCount": 37088,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Tomorrow, 6 PM",
    "deliveryFree": true,
    "trustScore": 33,
    "trustLabel": "Suspicious",
    "buyNowOrWait": "buy",
    "waitReason": null,
    "spikePriceMonths": [],
    "priceHistory": [
      91299,
      91299,
      95571,
      91299,
      91299,
      91299,
      91299,
      91299,
      91299,
      91299,
      91299,
      91299
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/laptops/lenovo-yoga-920/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/laptops/lenovo-yoga-920/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/laptops/lenovo-yoga-920/1.webp",
      "https://cdn.dummyjson.com/product-images/laptops/lenovo-yoga-920/2.webp"
    ],
    "description": "The Lenovo Yoga 920 is a 2-in-1 convertible laptop with a flexible hinge, allowing you to use it as a laptop or tablet, offering versatility and portability.",
    "features": [
      "The Lenovo Yoga 920 is a 2-in-1 convertible laptop with a flexible hinge, allowing you to use it as a laptop or tablet, offering versatility and portability",
      "Powered by Intel Core i7 processor for fast, responsive computing",
      "32GB DDR5 RAM — runs multiple apps without slowdown",
      "256GB high-speed solid-state storage for instant boot and fast file access",
      "13.3\" 4K display — vibrant visuals for work and entertainment"
    ],
    "specs": {
      "Brand": "Lenovo",
      "Display": "13.3\" 4K OLED",
      "Processor": "Intel Core i7-1255U",
      "RAM": "32GB DDR5",
      "Storage": "256GB SSD",
      "Graphics": "NVIDIA GeForce RTX 4060",
      "Battery": "56Whr — up to 10 hours",
      "OS": "DOS (No OS)",
      "Weight": "1.8 kg",
      "Warranty": "6 months warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Lenovo",
    "soldByRating": 2,
    "sellerSince": "2022",
    "fulfillment": "Seller fulfilled",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 28,
        "detail": null
      },
      "returnRate": {
        "score": 29,
        "detail": null
      },
      "warrantyClaims": {
        "score": 38,
        "detail": null
      },
      "sellerReliability": {
        "score": 42,
        "detail": null
      },
      "priceStability": {
        "score": 36,
        "detail": null
      }
    }
  },
  {
    "id": "dj82",
    "name": "New DELL XPS 13 9300 Laptop",
    "brand": "Dell",
    "category": "Electronics > Computers",
    "price": 124499,
    "originalPrice": 141476,
    "discount": 12,
    "rating": 2.7,
    "reviewCount": 53273,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Day after Tomorrow",
    "deliveryFree": true,
    "trustScore": 39,
    "trustLabel": "Suspicious",
    "buyNowOrWait": "wait",
    "waitReason": "Sale expected in the next 2 weeks based on historical patterns.",
    "spikePriceMonths": [],
    "priceHistory": [
      117644,
      124499,
      118642,
      136749,
      134302,
      124499,
      124499,
      124499,
      124499,
      124499,
      124499,
      124499
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/laptops/new-dell-xps-13-9300-laptop/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/laptops/new-dell-xps-13-9300-laptop/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/laptops/new-dell-xps-13-9300-laptop/1.webp",
      "https://cdn.dummyjson.com/product-images/laptops/new-dell-xps-13-9300-laptop/2.webp"
    ],
    "description": "The New DELL XPS 13 9300 Laptop is a compact and powerful device, featuring a virtually borderless InfinityEdge display and high-end performance for various tasks.",
    "features": [
      "The New DELL XPS 13 9300 Laptop is a compact and powerful device, featuring a virtually borderless InfinityEdge display and high-end performance for various tasks",
      "Powered by AMD Ryzen 7 processor for fast, responsive computing",
      "16GB DDR4 RAM — runs multiple apps without slowdown",
      "1TB high-speed solid-state storage for instant boot and fast file access",
      "16\" QHD display — vibrant visuals for work and entertainment"
    ],
    "specs": {
      "Brand": "Dell",
      "Display": "16\" QHD",
      "Processor": "AMD Ryzen 7 6800H",
      "RAM": "16GB DDR4",
      "Storage": "1TB NVMe SSD",
      "Graphics": "Intel Iris Xe",
      "Battery": "86Whr — up to 6 hours",
      "OS": "Windows 10 Home",
      "Weight": "2.6 kg",
      "Warranty": "2 year warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Dell",
    "soldByRating": 2.8,
    "sellerSince": "2023",
    "fulfillment": "Seller fulfilled",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 48,
        "detail": null
      },
      "returnRate": {
        "score": 40,
        "detail": null
      },
      "warrantyClaims": {
        "score": 43,
        "detail": null
      },
      "sellerReliability": {
        "score": 50,
        "detail": null
      },
      "priceStability": {
        "score": 43,
        "detail": null
      }
    }
  },
  {
    "id": "dj83",
    "name": "Blue & Black Check Shirt",
    "brand": "Fashion Trends",
    "category": "Fashion",
    "price": 2489,
    "originalPrice": 2928,
    "discount": 15,
    "rating": 3.6,
    "reviewCount": 38484,
    "inStock": true,
    "isPrime": false,
    "delivery": "Get it by Tomorrow, 6 PM",
    "deliveryFree": true,
    "trustScore": 91,
    "trustLabel": "Genuine",
    "buyNowOrWait": "wait",
    "waitReason": "Sale expected in the next 2 weeks based on historical patterns.",
    "spikePriceMonths": [
      9
    ],
    "priceHistory": [
      2489,
      2489,
      2489,
      2489,
      2489,
      2489,
      2681,
      2489,
      2489,
      2986,
      2504,
      2489
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/mens-shirts/blue-&-black-check-shirt/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/mens-shirts/blue-&-black-check-shirt/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/mens-shirts/blue-&-black-check-shirt/1.webp",
      "https://cdn.dummyjson.com/product-images/mens-shirts/blue-&-black-check-shirt/2.webp"
    ],
    "description": "The Blue & Black Check Shirt is a stylish and comfortable men's shirt featuring a classic check pattern. Made from high-quality fabric, it's suitable for both casual and semi-formal occasions.",
    "features": [
      "The Blue & Black Check Shirt is a stylish and comfortable men's shirt featuring a classic check pattern",
      "Fabric: stretch poplin — breathable and comfortable all day",
      "Regular fit silhouette — versatile enough for office to weekend wear",
      "Dry-clean only — low-maintenance care",
      "Available in a wide colour range — from neutral classics to seasonal bold tones"
    ],
    "specs": {
      "Fabric": "Stretch Poplin",
      "Fit": "Regular Fit",
      "Collar": "Cutaway Collar",
      "Closure": "Button Placket",
      "Care": "Hand Wash Cold",
      "Sizes": "XS / S / M / L / XL / XXL / XXXL",
      "Weight": "220g"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Fashion Trends",
    "soldByRating": 3.9,
    "sellerSince": "2015",
    "fulfillment": "Seller fulfilled",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 79,
        "detail": null
      },
      "returnRate": {
        "score": 91,
        "detail": null
      },
      "warrantyClaims": {
        "score": 96,
        "detail": null
      },
      "sellerReliability": {
        "score": 96,
        "detail": null
      },
      "priceStability": {
        "score": 87,
        "detail": null
      }
    }
  },
  {
    "id": "dj84",
    "name": "Gigabyte Aorus Men Tshirt",
    "brand": "Gigabyte",
    "category": "Fashion",
    "price": 2074,
    "originalPrice": 2095,
    "discount": 1,
    "rating": 3.2,
    "reviewCount": 65667,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Day after Tomorrow",
    "deliveryFree": true,
    "trustScore": 84,
    "trustLabel": "Genuine",
    "buyNowOrWait": "buy",
    "waitReason": null,
    "spikePriceMonths": [],
    "priceHistory": [
      2074,
      2074,
      2074,
      2074,
      1870,
      2074,
      2074,
      2074,
      1732,
      2074,
      2107,
      2074
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/mens-shirts/gigabyte-aorus-men-tshirt/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/mens-shirts/gigabyte-aorus-men-tshirt/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/mens-shirts/gigabyte-aorus-men-tshirt/1.webp",
      "https://cdn.dummyjson.com/product-images/mens-shirts/gigabyte-aorus-men-tshirt/2.webp"
    ],
    "description": "The Gigabyte Aorus Men Tshirt is a cool and casual shirt for gaming enthusiasts. With the Aorus logo and sleek design, it's perfect for expressing your gaming style.",
    "features": [
      "The Gigabyte Aorus Men Tshirt is a cool and casual shirt for gaming enthusiasts",
      "Fabric: Oxford weave cotton — breathable and comfortable all day",
      "Regular fit silhouette — versatile enough for office to weekend wear",
      "Hand wash cold — low-maintenance care",
      "Available in a wide colour range — from neutral classics to seasonal bold tones"
    ],
    "specs": {
      "Fabric": "Oxford Weave Cotton",
      "Fit": "Regular Fit",
      "Collar": "Mandarin Collar",
      "Closure": "Zip Placket",
      "Care": "Machine Wash 40°C",
      "Sizes": "XS / S / M / L / XL / XXL / XXXL",
      "Weight": "284g"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Gigabyte",
    "soldByRating": 4,
    "sellerSince": "2017",
    "fulfillment": "Seller fulfilled",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 83,
        "detail": null
      },
      "returnRate": {
        "score": 79,
        "detail": null
      },
      "warrantyClaims": {
        "score": 83,
        "detail": null
      },
      "sellerReliability": {
        "score": 86,
        "detail": null
      },
      "priceStability": {
        "score": 86,
        "detail": null
      }
    }
  },
  {
    "id": "dj85",
    "name": "Man Plaid Shirt",
    "brand": "Classic Wear",
    "category": "Fashion",
    "price": 2904,
    "originalPrice": 3630,
    "discount": 20,
    "rating": 3.5,
    "reviewCount": 3528,
    "inStock": true,
    "isPrime": false,
    "delivery": "Get it by Tomorrow, 6 PM",
    "deliveryFree": true,
    "trustScore": 86,
    "trustLabel": "Genuine",
    "buyNowOrWait": "wait",
    "waitReason": "Sale expected in the next 2 weeks based on historical patterns.",
    "spikePriceMonths": [
      7
    ],
    "priceHistory": [
      3230,
      2904,
      2904,
      2904,
      2904,
      2904,
      2904,
      3301,
      2904,
      2904,
      2904,
      2904
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/mens-shirts/man-plaid-shirt/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/mens-shirts/man-plaid-shirt/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/mens-shirts/man-plaid-shirt/1.webp",
      "https://cdn.dummyjson.com/product-images/mens-shirts/man-plaid-shirt/2.webp"
    ],
    "description": "The Man Plaid Shirt is a timeless and versatile men's shirt with a classic plaid pattern. Its comfortable fit and casual style make it a wardrobe essential for various occasions.",
    "features": [
      "The Man Plaid Shirt is a timeless and versatile men's shirt with a classic plaid pattern",
      "Fabric: Oxford weave cotton — breathable and comfortable all day",
      "Regular fit silhouette — versatile enough for office to weekend wear",
      "Machine washable at 30°C — low-maintenance care",
      "Available in a wide colour range — from neutral classics to seasonal bold tones"
    ],
    "specs": {
      "Fabric": "Oxford Weave Cotton",
      "Fit": "Regular Fit",
      "Collar": "Classic Spread Collar",
      "Closure": "Hidden Placket",
      "Care": "Dry Clean Only",
      "Sizes": "XS / S / M / L / XL / XXL / XXXL",
      "Weight": "184g"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Classic Wear",
    "soldByRating": 3.8,
    "sellerSince": "2011",
    "fulfillment": "Seller fulfilled",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 87,
        "detail": null
      },
      "returnRate": {
        "score": 86,
        "detail": null
      },
      "warrantyClaims": {
        "score": 91,
        "detail": null
      },
      "sellerReliability": {
        "score": 88,
        "detail": null
      },
      "priceStability": {
        "score": 94,
        "detail": null
      }
    }
  },
  {
    "id": "dj86",
    "name": "Man Short Sleeve Shirt",
    "brand": "Casual Comfort",
    "category": "Fashion",
    "price": 1659,
    "originalPrice": 1784,
    "discount": 7,
    "rating": 2.9,
    "reviewCount": 56202,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Day after Tomorrow",
    "deliveryFree": true,
    "trustScore": 82,
    "trustLabel": "Genuine",
    "buyNowOrWait": "buy",
    "waitReason": null,
    "spikePriceMonths": [],
    "priceHistory": [
      1468,
      1496,
      1659,
      1659,
      1506,
      1565,
      1659,
      1659,
      1659,
      1659,
      1659,
      1659
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/mens-shirts/man-short-sleeve-shirt/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/mens-shirts/man-short-sleeve-shirt/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/mens-shirts/man-short-sleeve-shirt/1.webp",
      "https://cdn.dummyjson.com/product-images/mens-shirts/man-short-sleeve-shirt/2.webp"
    ],
    "description": "The Man Short Sleeve Shirt is a breezy and stylish option for warm days. With a comfortable fit and short sleeves, it's perfect for a laid-back yet polished look.",
    "features": [
      "The Man Short Sleeve Shirt is a breezy and stylish option for warm days",
      "Fabric: Oxford weave cotton — breathable and comfortable all day",
      "Slim fit silhouette — versatile enough for office to weekend wear",
      "Dry-clean only — low-maintenance care",
      "Available in a wide colour range — from neutral classics to seasonal bold tones"
    ],
    "specs": {
      "Fabric": "Oxford Weave Cotton",
      "Fit": "Slim Fit",
      "Collar": "Cutaway Collar",
      "Closure": "Snap Buttons",
      "Care": "Machine Wash 40°C",
      "Sizes": "XS / S / M / L / XL / XXL / XXXL",
      "Weight": "267g"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Casual Comfort",
    "soldByRating": 4.3,
    "sellerSince": "2011",
    "fulfillment": "Seller fulfilled",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 94,
        "detail": null
      },
      "returnRate": {
        "score": 74,
        "detail": null
      },
      "warrantyClaims": {
        "score": 90,
        "detail": null
      },
      "sellerReliability": {
        "score": 86,
        "detail": null
      },
      "priceStability": {
        "score": 72,
        "detail": null
      }
    }
  },
  {
    "id": "dj87",
    "name": "Men Check Shirt",
    "brand": "Urban Chic",
    "category": "Fashion",
    "price": 2323,
    "originalPrice": 2610,
    "discount": 11,
    "rating": 2.7,
    "reviewCount": 5714,
    "inStock": true,
    "isPrime": false,
    "delivery": "Get it by Tomorrow, 6 PM",
    "deliveryFree": true,
    "trustScore": 73,
    "trustLabel": "Mixed",
    "buyNowOrWait": "wait",
    "waitReason": "Sale expected in the next 2 weeks based on historical patterns.",
    "spikePriceMonths": [],
    "priceHistory": [
      2193,
      2558,
      2323,
      2323,
      2323,
      2323,
      2323,
      2323,
      2323,
      2323,
      2323,
      2323
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/mens-shirts/men-check-shirt/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/mens-shirts/men-check-shirt/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/mens-shirts/men-check-shirt/1.webp",
      "https://cdn.dummyjson.com/product-images/mens-shirts/men-check-shirt/2.webp"
    ],
    "description": "The Men Check Shirt is a classic and versatile shirt featuring a stylish check pattern. Suitable for various occasions, it adds a smart and polished touch to your wardrobe.",
    "features": [
      "The Men Check Shirt is a classic and versatile shirt featuring a stylish check pattern",
      "Fabric: 100% pure cotton — breathable and comfortable all day",
      "Regular fit silhouette — versatile enough for office to weekend wear",
      "Machine washable at 30°C — low-maintenance care",
      "Available in a wide colour range — from neutral classics to seasonal bold tones"
    ],
    "specs": {
      "Fabric": "100% Cotton",
      "Fit": "Regular Fit",
      "Collar": "Classic Spread Collar",
      "Closure": "Button Placket",
      "Care": "Hand Wash Cold",
      "Sizes": "XS / S / M / L / XL / XXL / XXXL",
      "Weight": "242g"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Urban Chic",
    "soldByRating": 4,
    "sellerSince": "2015",
    "fulfillment": "Seller fulfilled",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 66,
        "detail": null
      },
      "returnRate": {
        "score": 75,
        "detail": null
      },
      "warrantyClaims": {
        "score": 68,
        "detail": null
      },
      "sellerReliability": {
        "score": 74,
        "detail": null
      },
      "priceStability": {
        "score": 72,
        "detail": null
      }
    }
  },
  {
    "id": "dj88",
    "name": "Nike Air Jordan 1 Red And Black",
    "brand": "Nike",
    "category": "Fashion",
    "price": 12449,
    "originalPrice": 12968,
    "discount": 4,
    "rating": 4.8,
    "reviewCount": 72251,
    "inStock": true,
    "isPrime": false,
    "delivery": "Get it by Day after Tomorrow",
    "deliveryFree": true,
    "trustScore": 90,
    "trustLabel": "Genuine",
    "buyNowOrWait": "wait",
    "waitReason": "Sale expected in the next 2 weeks based on historical patterns.",
    "spikePriceMonths": [],
    "priceHistory": [
      12449,
      12449,
      12449,
      12449,
      12449,
      12449,
      12449,
      12449,
      12449,
      12449,
      12449,
      12449
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/mens-shoes/nike-air-jordan-1-red-and-black/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/mens-shoes/nike-air-jordan-1-red-and-black/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/mens-shoes/nike-air-jordan-1-red-and-black/1.webp",
      "https://cdn.dummyjson.com/product-images/mens-shoes/nike-air-jordan-1-red-and-black/2.webp"
    ],
    "description": "The Nike Air Jordan 1 in Red and Black is an iconic basketball sneaker known for its stylish design and high-performance features, making it a favorite among sneaker enthusiasts and athletes.",
    "features": [
      "The Nike Air Jordan 1 in Red and Black is an iconic basketball sneaker known for its stylish design and high-performance features, making it a favorite among sneaker enthusiasts and athletes",
      "Upper: genuine leather — durable and refined",
      "EVA foam midsole for all-day support",
      "Outsole: durable synthetic — excellent traction on all surfaces",
      "Lace-up closure — easy to put on and take off"
    ],
    "specs": {
      "Upper Material": "Genuine Leather",
      "Sole Material": "Natural Gum Rubber",
      "Closure": "Buckle",
      "Toe Shape": "Round Toe",
      "Heel Height": "3 cm",
      "Sizes": "UK 6 – UK 12",
      "Weight": "472g (per pair)",
      "Warranty": "1 year warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Nike",
    "soldByRating": 4.1,
    "sellerSince": "2011",
    "fulfillment": "Fulfilled by Amazon",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 81,
        "detail": null
      },
      "returnRate": {
        "score": 90,
        "detail": null
      },
      "warrantyClaims": {
        "score": 94,
        "detail": null
      },
      "sellerReliability": {
        "score": 96,
        "detail": null
      },
      "priceStability": {
        "score": 87,
        "detail": null
      }
    }
  },
  {
    "id": "dj89",
    "name": "Nike Baseball Cleats",
    "brand": "Nike",
    "category": "Fashion",
    "price": 6639,
    "originalPrice": 8096,
    "discount": 18,
    "rating": 3.9,
    "reviewCount": 37159,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Tomorrow, 6 PM",
    "deliveryFree": true,
    "trustScore": 46,
    "trustLabel": "Suspicious",
    "buyNowOrWait": "wait",
    "waitReason": "Sale expected in the next 2 weeks based on historical patterns.",
    "spikePriceMonths": [
      0,
      2,
      3,
      8,
      9
    ],
    "priceHistory": [
      7942,
      6639,
      7833,
      7503,
      6639,
      6639,
      6639,
      6639,
      7842,
      8520,
      6639,
      6639
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/mens-shoes/nike-baseball-cleats/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/mens-shoes/nike-baseball-cleats/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/mens-shoes/nike-baseball-cleats/1.webp",
      "https://cdn.dummyjson.com/product-images/mens-shoes/nike-baseball-cleats/2.webp"
    ],
    "description": "Nike Baseball Cleats are designed for maximum traction and performance on the baseball field. They provide stability and support for players during games and practices.",
    "features": [
      "Nike Baseball Cleats are designed for maximum traction and performance on the baseball field",
      "Upper: patent leather — durable and refined",
      "Cushioned memory foam insole for all-day support",
      "Outsole: thermoplastic rubber (TPR) — excellent traction on all surfaces",
      "Velcro strap — easy to put on and take off"
    ],
    "specs": {
      "Upper Material": "Patent Leather",
      "Sole Material": "Rubber",
      "Closure": "Slip-on",
      "Toe Shape": "Pointed Toe",
      "Heel Height": "1 cm",
      "Sizes": "UK 6 – UK 12",
      "Weight": "396g (per pair)",
      "Warranty": "6 months warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Nike",
    "soldByRating": 2.9,
    "sellerSince": "2021",
    "fulfillment": "Seller fulfilled",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 38,
        "detail": null
      },
      "returnRate": {
        "score": 53,
        "detail": null
      },
      "warrantyClaims": {
        "score": 48,
        "detail": null
      },
      "sellerReliability": {
        "score": 58,
        "detail": null
      },
      "priceStability": {
        "score": 48,
        "detail": null
      }
    }
  },
  {
    "id": "dj90",
    "name": "Puma Future Rider Trainers",
    "brand": "Puma",
    "category": "Fashion",
    "price": 7469,
    "originalPrice": 7780,
    "discount": 4,
    "rating": 4.9,
    "reviewCount": 9259,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Day after Tomorrow",
    "deliveryFree": true,
    "trustScore": 96,
    "trustLabel": "Genuine",
    "buyNowOrWait": "wait",
    "waitReason": "Sale expected in the next 2 weeks based on historical patterns.",
    "spikePriceMonths": [
      1,
      5
    ],
    "priceHistory": [
      7469,
      8939,
      7469,
      7469,
      7469,
      8887,
      7469,
      7469,
      7469,
      7469,
      7469,
      7469
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/mens-shoes/puma-future-rider-trainers/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/mens-shoes/puma-future-rider-trainers/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/mens-shoes/puma-future-rider-trainers/1.webp",
      "https://cdn.dummyjson.com/product-images/mens-shoes/puma-future-rider-trainers/2.webp"
    ],
    "description": "The Puma Future Rider Trainers offer a blend of retro style and modern comfort. Perfect for casual wear, these trainers provide a fashionable and comfortable option for everyday use.",
    "features": [
      "The Puma Future Rider Trainers offer a blend of retro style and modern comfort",
      "Upper: premium mesh — durable and refined",
      "EVA foam midsole for all-day support",
      "Outsole: non-slip rubber — excellent traction on all surfaces",
      "Buckle closure — easy to put on and take off"
    ],
    "specs": {
      "Upper Material": "Premium Mesh",
      "Sole Material": "Natural Gum Rubber",
      "Closure": "Lace-up",
      "Toe Shape": "Almond Toe",
      "Heel Height": "2 cm",
      "Sizes": "UK 6 – UK 12",
      "Weight": "457g (per pair)",
      "Warranty": "5 year warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Puma",
    "soldByRating": 4.1,
    "sellerSince": "2018",
    "fulfillment": "Fulfilled by Amazon",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 96,
        "detail": null
      },
      "returnRate": {
        "score": 96,
        "detail": null
      },
      "warrantyClaims": {
        "score": 89,
        "detail": null
      },
      "sellerReliability": {
        "score": 96,
        "detail": null
      },
      "priceStability": {
        "score": 96,
        "detail": null
      }
    }
  },
  {
    "id": "dj91",
    "name": "Sports Sneakers Off White & Red",
    "brand": "Off White",
    "category": "Fashion",
    "price": 9959,
    "originalPrice": 10483,
    "discount": 5,
    "rating": 4.8,
    "reviewCount": 15365,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Day after Tomorrow",
    "deliveryFree": true,
    "trustScore": 96,
    "trustLabel": "Genuine",
    "buyNowOrWait": "wait",
    "waitReason": "Sale expected in the next 2 weeks based on historical patterns.",
    "spikePriceMonths": [
      7
    ],
    "priceHistory": [
      9959,
      9959,
      8924,
      9959,
      9959,
      9959,
      9959,
      11192,
      9959,
      9959,
      9959,
      9959
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/mens-shoes/sports-sneakers-off-white-&-red/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/mens-shoes/sports-sneakers-off-white-&-red/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/mens-shoes/sports-sneakers-off-white-&-red/1.webp",
      "https://cdn.dummyjson.com/product-images/mens-shoes/sports-sneakers-off-white-&-red/2.webp"
    ],
    "description": "The Sports Sneakers in Off White and Red combine style and functionality, making them a fashionable choice for sports enthusiasts. The red and off-white color combination adds a bold and energetic touch.",
    "features": [
      "The Sports Sneakers in Off White and Red combine style and functionality, making them a fashionable choice for sports enthusiasts",
      "Upper: patent leather — durable and refined",
      "Contour comfort footbed for all-day support",
      "Outsole: non-slip rubber — excellent traction on all surfaces",
      "Slip-on — easy to put on and take off"
    ],
    "specs": {
      "Upper Material": "Patent Leather",
      "Sole Material": "Synthetic",
      "Closure": "Lace-up",
      "Toe Shape": "Square Toe",
      "Heel Height": "2 cm",
      "Sizes": "UK 6 – UK 12",
      "Weight": "476g (per pair)",
      "Warranty": "1 week warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Off White",
    "soldByRating": 4.8,
    "sellerSince": "2011",
    "fulfillment": "Fulfilled by Amazon",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 92,
        "detail": null
      },
      "returnRate": {
        "score": 96,
        "detail": null
      },
      "warrantyClaims": {
        "score": 96,
        "detail": null
      },
      "sellerReliability": {
        "score": 96,
        "detail": null
      },
      "priceStability": {
        "score": 96,
        "detail": null
      }
    }
  },
  {
    "id": "dj92",
    "name": "Sports Sneakers Off White Red",
    "brand": "Off White",
    "category": "Fashion",
    "price": 9129,
    "originalPrice": 9129,
    "discount": 0,
    "rating": 4.7,
    "reviewCount": 59261,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Day after Tomorrow",
    "deliveryFree": true,
    "trustScore": 91,
    "trustLabel": "Genuine",
    "buyNowOrWait": "wait",
    "waitReason": "Sale expected in the next 2 weeks based on historical patterns.",
    "spikePriceMonths": [
      0
    ],
    "priceHistory": [
      10238,
      9129,
      9129,
      9129,
      9129,
      9129,
      8665,
      9129,
      9129,
      9129,
      9642,
      9129
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/mens-shoes/sports-sneakers-off-white-red/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/mens-shoes/sports-sneakers-off-white-red/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/mens-shoes/sports-sneakers-off-white-red/1.webp",
      "https://cdn.dummyjson.com/product-images/mens-shoes/sports-sneakers-off-white-red/2.webp"
    ],
    "description": "Another variant of the Sports Sneakers in Off White Red, featuring a unique design. These sneakers offer style and comfort for casual occasions.",
    "features": [
      "Another variant of the Sports Sneakers in Off White Red, featuring a unique design",
      "Upper: suede leather — durable and refined",
      "Cushioned memory foam insole for all-day support",
      "Outsole: durable synthetic — excellent traction on all surfaces",
      "Buckle closure — easy to put on and take off"
    ],
    "specs": {
      "Upper Material": "Suede Leather",
      "Sole Material": "Rubber",
      "Closure": "Buckle",
      "Toe Shape": "Almond Toe",
      "Heel Height": "2 cm",
      "Sizes": "UK 6 – UK 12",
      "Weight": "481g (per pair)",
      "Warranty": "3 months warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Off White",
    "soldByRating": 3.9,
    "sellerSince": "2014",
    "fulfillment": "Fulfilled by Amazon",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 80,
        "detail": null
      },
      "returnRate": {
        "score": 90,
        "detail": null
      },
      "warrantyClaims": {
        "score": 96,
        "detail": null
      },
      "sellerReliability": {
        "score": 96,
        "detail": null
      },
      "priceStability": {
        "score": 92,
        "detail": null
      }
    }
  },
  {
    "id": "dj93",
    "name": "Brown Leather Belt Watch",
    "brand": "Fashion Timepieces",
    "category": "Fashion",
    "price": 7469,
    "originalPrice": 7946,
    "discount": 6,
    "rating": 4.2,
    "reviewCount": 67361,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Tomorrow, 6 PM",
    "deliveryFree": true,
    "trustScore": 95,
    "trustLabel": "Genuine",
    "buyNowOrWait": "wait",
    "waitReason": "Sale expected in the next 2 weeks based on historical patterns.",
    "spikePriceMonths": [],
    "priceHistory": [
      7469,
      7469,
      7469,
      7382,
      7469,
      7469,
      7469,
      7469,
      7469,
      7469,
      7469,
      7469
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/mens-watches/brown-leather-belt-watch/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/mens-watches/brown-leather-belt-watch/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/mens-watches/brown-leather-belt-watch/1.webp",
      "https://cdn.dummyjson.com/product-images/mens-watches/brown-leather-belt-watch/2.webp"
    ],
    "description": "The Brown Leather Belt Watch is a stylish timepiece with a classic design. Featuring a genuine leather strap and a sleek dial, it adds a touch of sophistication to your look.",
    "features": [
      "The Brown Leather Belt Watch is a stylish timepiece with a classic design",
      "Case: rose gold plated — 17mm diameter",
      "Chronograph movement — high accuracy",
      "3 ATM water resistant — suitable for daily wear and light water activities",
      "Strap: silicone sport strap — comfortable and stylish"
    ],
    "specs": {
      "Case Material": "PVD Black Coated",
      "Case Diameter": "38mm",
      "Case Thickness": "11mm",
      "Movement": "Mechanical Hand-wind",
      "Water Resistance": "3 ATM (30m)",
      "Strap Material": "Silicone",
      "Warranty": "1 year warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Fashion Timepieces",
    "soldByRating": 4,
    "sellerSince": "2018",
    "fulfillment": "Fulfilled by Amazon",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 94,
        "detail": null
      },
      "returnRate": {
        "score": 96,
        "detail": null
      },
      "warrantyClaims": {
        "score": 92,
        "detail": null
      },
      "sellerReliability": {
        "score": 96,
        "detail": null
      },
      "priceStability": {
        "score": 86,
        "detail": null
      }
    }
  },
  {
    "id": "dj94",
    "name": "Longines Master Collection",
    "brand": "Longines",
    "category": "Fashion",
    "price": 124499,
    "originalPrice": 149999,
    "discount": 17,
    "rating": 3.9,
    "reviewCount": 72190,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Tomorrow, 6 PM",
    "deliveryFree": true,
    "trustScore": 89,
    "trustLabel": "Genuine",
    "buyNowOrWait": "wait",
    "waitReason": "Sale expected in the next 2 weeks based on historical patterns.",
    "spikePriceMonths": [
      0
    ],
    "priceHistory": [
      157929,
      124499,
      124499,
      124499,
      124499,
      124499,
      124499,
      124499,
      127148,
      124654,
      124499,
      124499
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/mens-watches/longines-master-collection/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/mens-watches/longines-master-collection/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/mens-watches/longines-master-collection/1.webp",
      "https://cdn.dummyjson.com/product-images/mens-watches/longines-master-collection/2.webp"
    ],
    "description": "The Longines Master Collection is an elegant and refined watch known for its precision and craftsmanship. With a timeless design, it's a symbol of luxury and sophistication.",
    "features": [
      "The Longines Master Collection is an elegant and refined watch known for its precision and craftsmanship",
      "Case: matte black PVD — 15mm diameter",
      "Chronograph movement — ±30 sec/month",
      "3 ATM water resistant — suitable for daily wear and light water activities",
      "Strap: genuine leather — comfortable and stylish"
    ],
    "specs": {
      "Case Material": "IP Gold-plated",
      "Case Diameter": "42mm",
      "Case Thickness": "11mm",
      "Movement": "Solar Quartz",
      "Water Resistance": "3 ATM (30m)",
      "Strap Material": "Genuine Leather",
      "Warranty": "1 week warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Longines",
    "soldByRating": 4.9,
    "sellerSince": "2010",
    "fulfillment": "Seller fulfilled",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 92,
        "detail": null
      },
      "returnRate": {
        "score": 86,
        "detail": null
      },
      "warrantyClaims": {
        "score": 87,
        "detail": null
      },
      "sellerReliability": {
        "score": 96,
        "detail": null
      },
      "priceStability": {
        "score": 80,
        "detail": null
      }
    }
  },
  {
    "id": "dj95",
    "name": "Rolex Cellini Date Black Dial",
    "brand": "Rolex",
    "category": "Fashion",
    "price": 746999,
    "originalPrice": 820878,
    "discount": 9,
    "rating": 5,
    "reviewCount": 51531,
    "inStock": true,
    "isPrime": false,
    "delivery": "Get it by Day after Tomorrow",
    "deliveryFree": true,
    "trustScore": 96,
    "trustLabel": "Genuine",
    "buyNowOrWait": "buy",
    "waitReason": null,
    "spikePriceMonths": [
      0,
      7
    ],
    "priceHistory": [
      895538,
      746999,
      704793,
      746999,
      746999,
      746999,
      746999,
      872532,
      746999,
      698649,
      746999,
      746999
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/mens-watches/rolex-cellini-date-black-dial/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/mens-watches/rolex-cellini-date-black-dial/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/mens-watches/rolex-cellini-date-black-dial/1.webp",
      "https://cdn.dummyjson.com/product-images/mens-watches/rolex-cellini-date-black-dial/2.webp"
    ],
    "description": "The Rolex Cellini Date with Black Dial is a classic and prestigious watch. With a black dial and date complication, it exudes sophistication and is a symbol of Rolex's heritage.",
    "features": [
      "The Rolex Cellini Date with Black Dial is a classic and prestigious watch",
      "Case: matte black PVD — 13mm diameter",
      "Solar-powered movement — ±15 sec/month accuracy",
      "10 ATM water resistant — suitable for daily wear and light water activities",
      "Strap: mesh stainless — comfortable and stylish"
    ],
    "specs": {
      "Case Material": "IP Gold-plated",
      "Case Diameter": "40mm",
      "Case Thickness": "9mm",
      "Movement": "Japanese Quartz",
      "Water Resistance": "10 ATM (100m)",
      "Strap Material": "NATO Nylon",
      "Warranty": "3 months warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Rolex",
    "soldByRating": 4.3,
    "sellerSince": "2021",
    "fulfillment": "Seller fulfilled",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 96,
        "detail": null
      },
      "returnRate": {
        "score": 96,
        "detail": null
      },
      "warrantyClaims": {
        "score": 89,
        "detail": null
      },
      "sellerReliability": {
        "score": 96,
        "detail": null
      },
      "priceStability": {
        "score": 96,
        "detail": null
      }
    }
  },
  {
    "id": "dj96",
    "name": "Rolex Cellini Moonphase",
    "brand": "Rolex",
    "category": "Fashion",
    "price": 1078999,
    "originalPrice": 1315852,
    "discount": 18,
    "rating": 2.6,
    "reviewCount": 54383,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Tomorrow, 6 PM",
    "deliveryFree": true,
    "trustScore": 82,
    "trustLabel": "Genuine",
    "buyNowOrWait": "wait",
    "waitReason": "Sale expected in the next 2 weeks based on historical patterns.",
    "spikePriceMonths": [
      2,
      9
    ],
    "priceHistory": [
      1078999,
      1078999,
      1250825,
      1078999,
      1078999,
      1078999,
      1180025,
      1078999,
      1078999,
      1379785,
      1078999,
      1078999
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/mens-watches/rolex-cellini-moonphase/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/mens-watches/rolex-cellini-moonphase/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/mens-watches/rolex-cellini-moonphase/1.webp",
      "https://cdn.dummyjson.com/product-images/mens-watches/rolex-cellini-moonphase/2.webp"
    ],
    "description": "The Rolex Cellini Moonphase is a masterpiece of horology, featuring a moon phase complication and exquisite design. It reflects Rolex's commitment to precision and elegance.",
    "features": [
      "The Rolex Cellini Moonphase is a masterpiece of horology, featuring a moon phase complication and exquisite design",
      "Case: rose gold plated — 26mm diameter",
      "Solar-powered movement — Swiss precision",
      "3 ATM water resistant — suitable for daily wear and light water activities",
      "Strap: stainless steel bracelet — comfortable and stylish"
    ],
    "specs": {
      "Case Material": "PVD Black Coated",
      "Case Diameter": "44mm",
      "Case Thickness": "9mm",
      "Movement": "Swiss Automatic",
      "Water Resistance": "3 ATM (30m)",
      "Strap Material": "Stainless Steel Bracelet",
      "Warranty": "6 months warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Rolex",
    "soldByRating": 4.1,
    "sellerSince": "2019",
    "fulfillment": "Seller fulfilled",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 93,
        "detail": null
      },
      "returnRate": {
        "score": 86,
        "detail": null
      },
      "warrantyClaims": {
        "score": 78,
        "detail": null
      },
      "sellerReliability": {
        "score": 96,
        "detail": null
      },
      "priceStability": {
        "score": 84,
        "detail": null
      }
    }
  },
  {
    "id": "dj97",
    "name": "Rolex Datejust",
    "brand": "Rolex",
    "category": "Fashion",
    "price": 912999,
    "originalPrice": 951041,
    "discount": 4,
    "rating": 3.7,
    "reviewCount": 52593,
    "inStock": true,
    "isPrime": false,
    "delivery": "Get it by Day after Tomorrow",
    "deliveryFree": true,
    "trustScore": 89,
    "trustLabel": "Genuine",
    "buyNowOrWait": "buy",
    "waitReason": null,
    "spikePriceMonths": [],
    "priceHistory": [
      828970,
      912999,
      912999,
      912999,
      912999,
      912999,
      912999,
      912999,
      912999,
      912999,
      912999,
      912999
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/mens-watches/rolex-datejust/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/mens-watches/rolex-datejust/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/mens-watches/rolex-datejust/1.webp",
      "https://cdn.dummyjson.com/product-images/mens-watches/rolex-datejust/2.webp"
    ],
    "description": "The Rolex Datejust is an iconic and versatile timepiece with a date window. Known for its timeless design and reliability, it's a symbol of Rolex's watchmaking excellence.",
    "features": [
      "The Rolex Datejust is an iconic and versatile timepiece with a date window",
      "Case: rose gold plated — 23mm diameter",
      "Solar-powered movement — ±15 sec/month accuracy",
      "5 ATM water resistant — suitable for daily wear and light water activities",
      "Strap: silicone sport strap — comfortable and stylish"
    ],
    "specs": {
      "Case Material": "PVD Black Coated",
      "Case Diameter": "42mm",
      "Case Thickness": "9mm",
      "Movement": "Japanese Quartz",
      "Water Resistance": "5 ATM (50m)",
      "Strap Material": "Silicone",
      "Warranty": "2 year warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Rolex",
    "soldByRating": 4.1,
    "sellerSince": "2021",
    "fulfillment": "Seller fulfilled",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 77,
        "detail": null
      },
      "returnRate": {
        "score": 86,
        "detail": null
      },
      "warrantyClaims": {
        "score": 86,
        "detail": null
      },
      "sellerReliability": {
        "score": 96,
        "detail": null
      },
      "priceStability": {
        "score": 84,
        "detail": null
      }
    }
  },
  {
    "id": "dj98",
    "name": "Rolex Submariner Watch",
    "brand": "Rolex",
    "category": "Fashion",
    "price": 1161999,
    "originalPrice": 1223157,
    "discount": 5,
    "rating": 2.7,
    "reviewCount": 65207,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Day after Tomorrow",
    "deliveryFree": true,
    "trustScore": 79,
    "trustLabel": "Genuine",
    "buyNowOrWait": "buy",
    "waitReason": null,
    "spikePriceMonths": [
      4
    ],
    "priceHistory": [
      1161999,
      1161999,
      1161999,
      1161999,
      1318553,
      1037842,
      1161999,
      1161999,
      1161999,
      1161999,
      1161999,
      1161999
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/mens-watches/rolex-submariner-watch/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/mens-watches/rolex-submariner-watch/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/mens-watches/rolex-submariner-watch/1.webp",
      "https://cdn.dummyjson.com/product-images/mens-watches/rolex-submariner-watch/2.webp"
    ],
    "description": "The Rolex Submariner is a legendary dive watch with a rich history. Known for its durability and water resistance, it's a symbol of adventure and exploration.",
    "features": [
      "The Rolex Submariner is a legendary dive watch with a rich history",
      "Case: IP gold-plated — 18mm diameter",
      "Chronograph movement — high accuracy",
      "10 ATM water resistant — suitable for daily wear and light water activities",
      "Strap: NATO nylon — comfortable and stylish"
    ],
    "specs": {
      "Case Material": "IP Gold-plated",
      "Case Diameter": "44mm",
      "Case Thickness": "11mm",
      "Movement": "Mechanical Hand-wind",
      "Water Resistance": "10 ATM (100m)",
      "Strap Material": "Mesh Stainless",
      "Warranty": "5 year warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Rolex",
    "soldByRating": 4.6,
    "sellerSince": "2011",
    "fulfillment": "Seller fulfilled",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 65,
        "detail": null
      },
      "returnRate": {
        "score": 81,
        "detail": null
      },
      "warrantyClaims": {
        "score": 73,
        "detail": null
      },
      "sellerReliability": {
        "score": 95,
        "detail": null
      },
      "priceStability": {
        "score": 77,
        "detail": null
      }
    }
  },
  {
    "id": "dj99",
    "name": "Amazon Echo Plus",
    "brand": "Amazon",
    "category": "Electronics > Accessories",
    "price": 8299,
    "originalPrice": 9431,
    "discount": 12,
    "rating": 5,
    "reviewCount": 61221,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Tomorrow, 6 PM",
    "deliveryFree": true,
    "trustScore": 96,
    "trustLabel": "Genuine",
    "buyNowOrWait": "wait",
    "waitReason": "Sale expected in the next 2 weeks based on historical patterns.",
    "spikePriceMonths": [],
    "priceHistory": [
      8299,
      8299,
      8299,
      8449,
      8299,
      8299,
      8299,
      8299,
      8299,
      8299,
      8299,
      8299
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/mobile-accessories/amazon-echo-plus/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/mobile-accessories/amazon-echo-plus/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/mobile-accessories/amazon-echo-plus/1.webp",
      "https://cdn.dummyjson.com/product-images/mobile-accessories/amazon-echo-plus/2.webp"
    ],
    "description": "The Amazon Echo Plus is a smart speaker with built-in Alexa voice control. It features premium sound quality and serves as a hub for controlling smart home devices.",
    "features": [
      "The Amazon Echo Plus is a smart speaker with built-in Alexa voice control",
      "Precisely engineered for Amazon compatibility — fits most smartphones and tablets",
      "Heavy-duty construction that protects your device against daily wear and tear",
      "Ergonomic profile — adds protection without adding bulk",
      "Hassle-free installation and removal — no tools required"
    ],
    "specs": {
      "Brand": "Amazon",
      "Material": "Hard Plastic",
      "Compatibility": "Amazon — compatible with most smartphones",
      "Connectivity": "Bluetooth 5.0",
      "Weight": "187g",
      "Warranty": "6 months warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Amazon",
    "soldByRating": 3.9,
    "sellerSince": "2012",
    "fulfillment": "Fulfilled by Amazon",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 84,
        "detail": null
      },
      "returnRate": {
        "score": 87,
        "detail": null
      },
      "warrantyClaims": {
        "score": 96,
        "detail": null
      },
      "sellerReliability": {
        "score": 96,
        "detail": null
      },
      "priceStability": {
        "score": 95,
        "detail": null
      }
    }
  },
  {
    "id": "dj100",
    "name": "Apple Airpods",
    "brand": "Apple",
    "category": "Electronics > Accessories",
    "price": 10789,
    "originalPrice": 12844,
    "discount": 16,
    "rating": 4.2,
    "reviewCount": 42709,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Day after Tomorrow",
    "deliveryFree": true,
    "trustScore": 87,
    "trustLabel": "Genuine",
    "buyNowOrWait": "buy",
    "waitReason": null,
    "spikePriceMonths": [
      0,
      5,
      6
    ],
    "priceHistory": [
      13536,
      10789,
      10789,
      10789,
      10789,
      14688,
      13896,
      10789,
      10789,
      10789,
      10789,
      10789
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/mobile-accessories/apple-airpods/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/mobile-accessories/apple-airpods/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/mobile-accessories/apple-airpods/1.webp",
      "https://cdn.dummyjson.com/product-images/mobile-accessories/apple-airpods/2.webp"
    ],
    "description": "The Apple Airpods offer a seamless wireless audio experience. With easy pairing, high-quality sound, and Siri integration, they are perfect for on-the-go listening.",
    "features": [
      "The Apple Airpods offer a seamless wireless audio experience",
      "Precisely engineered for Apple compatibility — fits most smartphones and tablets",
      "Scratch-resistant construction that protects your device against daily wear and tear",
      "Lightweight profile — adds protection without adding bulk",
      "Hassle-free installation and removal — no tools required"
    ],
    "specs": {
      "Brand": "Apple",
      "Material": "Thermoplastic Polyurethane (TPU)",
      "Compatibility": "Apple — compatible with most smartphones",
      "Connectivity": "3.5mm Audio Jack",
      "Weight": "32g",
      "Warranty": "3 year warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Apple",
    "soldByRating": 5,
    "sellerSince": "2020",
    "fulfillment": "Seller fulfilled",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 90,
        "detail": null
      },
      "returnRate": {
        "score": 92,
        "detail": null
      },
      "warrantyClaims": {
        "score": 89,
        "detail": null
      },
      "sellerReliability": {
        "score": 96,
        "detail": null
      },
      "priceStability": {
        "score": 82,
        "detail": null
      }
    }
  },
  {
    "id": "dj101",
    "name": "Apple AirPods Max Silver",
    "brand": "Apple",
    "category": "Electronics > Accessories",
    "price": 45649,
    "originalPrice": 53080,
    "discount": 14,
    "rating": 3.5,
    "reviewCount": 31577,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Tomorrow, 6 PM",
    "deliveryFree": true,
    "trustScore": 84,
    "trustLabel": "Genuine",
    "buyNowOrWait": "buy",
    "waitReason": null,
    "spikePriceMonths": [
      6
    ],
    "priceHistory": [
      45649,
      45649,
      45649,
      45649,
      45649,
      45649,
      56855,
      45649,
      45649,
      45649,
      45649,
      45649
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/mobile-accessories/apple-airpods-max-silver/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/mobile-accessories/apple-airpods-max-silver/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/mobile-accessories/apple-airpods-max-silver/1.webp"
    ],
    "description": "The Apple AirPods Max in Silver are premium over-ear headphones with high-fidelity audio, adaptive EQ, and active noise cancellation. Experience immersive sound in style.",
    "features": [
      "The Apple AirPods Max in Silver are premium over-ear headphones with high-fidelity audio, adaptive EQ, and active noise cancellation",
      "Precisely engineered for Apple compatibility — fits most smartphones and tablets",
      "Military-grade construction that protects your device against daily wear and tear",
      "Ultra-slim profile — adds protection without adding bulk",
      "Hassle-free installation and removal — no tools required"
    ],
    "specs": {
      "Brand": "Apple",
      "Material": "Tempered Glass",
      "Compatibility": "Apple — compatible with most smartphones",
      "Connectivity": "Wired",
      "Weight": "172g",
      "Warranty": "No warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Apple",
    "soldByRating": 4.5,
    "sellerSince": "2010",
    "fulfillment": "Fulfilled by Amazon",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 80,
        "detail": null
      },
      "returnRate": {
        "score": 78,
        "detail": null
      },
      "warrantyClaims": {
        "score": 79,
        "detail": null
      },
      "sellerReliability": {
        "score": 88,
        "detail": null
      },
      "priceStability": {
        "score": 86,
        "detail": null
      }
    }
  },
  {
    "id": "dj102",
    "name": "Apple Airpower Wireless Charger",
    "brand": "Apple",
    "category": "Electronics > Accessories",
    "price": 6639,
    "originalPrice": 6916,
    "discount": 4,
    "rating": 3.7,
    "reviewCount": 36152,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Tomorrow, 6 PM",
    "deliveryFree": true,
    "trustScore": 84,
    "trustLabel": "Genuine",
    "buyNowOrWait": "wait",
    "waitReason": "Sale expected in the next 2 weeks based on historical patterns.",
    "spikePriceMonths": [],
    "priceHistory": [
      6639,
      6639,
      6639,
      6639,
      6639,
      6288,
      5819,
      6624,
      6639,
      6639,
      7404,
      6639
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/mobile-accessories/apple-airpower-wireless-charger/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/mobile-accessories/apple-airpower-wireless-charger/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/mobile-accessories/apple-airpower-wireless-charger/1.webp"
    ],
    "description": "The Apple AirPower Wireless Charger provides a convenient way to charge your compatible Apple devices wirelessly. Simply place your devices on the charging mat for effortless charging.",
    "features": [
      "The Apple AirPower Wireless Charger provides a convenient way to charge your compatible Apple devices wirelessly",
      "Precisely engineered for Apple compatibility — fits most smartphones and tablets",
      "Scratch-resistant construction that protects your device against daily wear and tear",
      "Lightweight profile — adds protection without adding bulk",
      "Hassle-free installation and removal — no tools required"
    ],
    "specs": {
      "Brand": "Apple",
      "Material": "Thermoplastic Polyurethane (TPU)",
      "Compatibility": "Apple — compatible with most smartphones",
      "Connectivity": "3.5mm Audio Jack",
      "Weight": "133g",
      "Warranty": "2 year warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Apple",
    "soldByRating": 4.8,
    "sellerSince": "2018",
    "fulfillment": "Fulfilled by Amazon",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 73,
        "detail": null
      },
      "returnRate": {
        "score": 93,
        "detail": null
      },
      "warrantyClaims": {
        "score": 82,
        "detail": null
      },
      "sellerReliability": {
        "score": 89,
        "detail": null
      },
      "priceStability": {
        "score": 75,
        "detail": null
      }
    }
  },
  {
    "id": "dj103",
    "name": "Apple HomePod Mini Cosmic Grey",
    "brand": "Apple",
    "category": "Electronics > Accessories",
    "price": 8299,
    "originalPrice": 10121,
    "discount": 18,
    "rating": 4.6,
    "reviewCount": 60707,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Day after Tomorrow",
    "deliveryFree": true,
    "trustScore": 87,
    "trustLabel": "Genuine",
    "buyNowOrWait": "wait",
    "waitReason": "Sale expected in the next 2 weeks based on historical patterns.",
    "spikePriceMonths": [],
    "priceHistory": [
      8299,
      8299,
      8299,
      8299,
      8299,
      8299,
      8299,
      8675,
      8299,
      9142,
      8299,
      8299
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/mobile-accessories/apple-homepod-mini-cosmic-grey/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/mobile-accessories/apple-homepod-mini-cosmic-grey/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/mobile-accessories/apple-homepod-mini-cosmic-grey/1.webp"
    ],
    "description": "The Apple HomePod Mini in Cosmic Grey is a compact smart speaker that delivers impressive audio and integrates seamlessly with the Apple ecosystem for a smart home experience.",
    "features": [
      "The Apple HomePod Mini in Cosmic Grey is a compact smart speaker that delivers impressive audio and integrates seamlessly with the Apple ecosystem for a smart home experience",
      "Precisely engineered for Apple compatibility — fits most smartphones and tablets",
      "Shock-absorbing construction that protects your device against daily wear and tear",
      "Lightweight profile — adds protection without adding bulk",
      "Hassle-free installation and removal — no tools required"
    ],
    "specs": {
      "Brand": "Apple",
      "Material": "Polycarbonate",
      "Compatibility": "Apple — compatible with most smartphones",
      "Connectivity": "3.5mm Audio Jack",
      "Weight": "41g",
      "Warranty": "3 months warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Apple",
    "soldByRating": 4,
    "sellerSince": "2014",
    "fulfillment": "Seller fulfilled",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 88,
        "detail": null
      },
      "returnRate": {
        "score": 85,
        "detail": null
      },
      "warrantyClaims": {
        "score": 92,
        "detail": null
      },
      "sellerReliability": {
        "score": 94,
        "detail": null
      },
      "priceStability": {
        "score": 80,
        "detail": null
      }
    }
  },
  {
    "id": "dj104",
    "name": "Apple iPhone Charger",
    "brand": "Apple",
    "category": "Electronics > Accessories",
    "price": 1659,
    "originalPrice": 2048,
    "discount": 19,
    "rating": 4.2,
    "reviewCount": 10865,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Tomorrow, 6 PM",
    "deliveryFree": true,
    "trustScore": 92,
    "trustLabel": "Genuine",
    "buyNowOrWait": "buy",
    "waitReason": null,
    "spikePriceMonths": [],
    "priceHistory": [
      1659,
      1659,
      1659,
      1659,
      1659,
      1659,
      1659,
      1842,
      1659,
      1659,
      1659,
      1659
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/mobile-accessories/apple-iphone-charger/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/mobile-accessories/apple-iphone-charger/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/mobile-accessories/apple-iphone-charger/1.webp",
      "https://cdn.dummyjson.com/product-images/mobile-accessories/apple-iphone-charger/2.webp"
    ],
    "description": "The Apple iPhone Charger is a high-quality charger designed for fast and efficient charging of your iPhone. Ensure your device stays powered up and ready to go.",
    "features": [
      "The Apple iPhone Charger is a high-quality charger designed for fast and efficient charging of your iPhone",
      "Precisely engineered for Apple compatibility — fits most smartphones and tablets",
      "Military-grade construction that protects your device against daily wear and tear",
      "Ergonomic profile — adds protection without adding bulk",
      "Hassle-free installation and removal — no tools required"
    ],
    "specs": {
      "Brand": "Apple",
      "Material": "Silicone",
      "Compatibility": "Apple — compatible with most smartphones",
      "Connectivity": "USB-C",
      "Weight": "214g",
      "Warranty": "1 year warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Apple",
    "soldByRating": 4.9,
    "sellerSince": "2012",
    "fulfillment": "Fulfilled by Amazon",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 96,
        "detail": null
      },
      "returnRate": {
        "score": 96,
        "detail": null
      },
      "warrantyClaims": {
        "score": 96,
        "detail": null
      },
      "sellerReliability": {
        "score": 96,
        "detail": null
      },
      "priceStability": {
        "score": 96,
        "detail": null
      }
    }
  },
  {
    "id": "dj105",
    "name": "Apple MagSafe Battery Pack",
    "brand": "Apple",
    "category": "Electronics > Accessories",
    "price": 8299,
    "originalPrice": 9999,
    "discount": 17,
    "rating": 3.6,
    "reviewCount": 7630,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Tomorrow, 6 PM",
    "deliveryFree": true,
    "trustScore": 43,
    "trustLabel": "Suspicious",
    "buyNowOrWait": "wait",
    "waitReason": "Sale expected in the next 2 weeks based on historical patterns.",
    "spikePriceMonths": [
      2,
      4
    ],
    "priceHistory": [
      8299,
      8299,
      10169,
      8299,
      10233,
      8299,
      8299,
      8299,
      8299,
      8299,
      8299,
      8299
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/mobile-accessories/apple-magsafe-battery-pack/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/mobile-accessories/apple-magsafe-battery-pack/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/mobile-accessories/apple-magsafe-battery-pack/1.webp",
      "https://cdn.dummyjson.com/product-images/mobile-accessories/apple-magsafe-battery-pack/2.webp"
    ],
    "description": "The Apple MagSafe Battery Pack is a portable and convenient way to add extra battery life to your MagSafe-compatible iPhone. Attach it magnetically for a secure connection.",
    "features": [
      "The Apple MagSafe Battery Pack is a portable and convenient way to add extra battery life to your MagSafe-compatible iPhone",
      "Precisely engineered for Apple compatibility — fits most smartphones and tablets",
      "Military-grade construction that protects your device against daily wear and tear",
      "Compact profile — adds protection without adding bulk",
      "Hassle-free installation and removal — no tools required"
    ],
    "specs": {
      "Brand": "Apple",
      "Material": "Silicone",
      "Compatibility": "Apple — compatible with most smartphones",
      "Connectivity": "Wireless",
      "Weight": "120g",
      "Warranty": "2 year warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Apple",
    "soldByRating": 2.5,
    "sellerSince": "2024",
    "fulfillment": "Seller fulfilled",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 46,
        "detail": null
      },
      "returnRate": {
        "score": 35,
        "detail": null
      },
      "warrantyClaims": {
        "score": 47,
        "detail": null
      },
      "sellerReliability": {
        "score": 59,
        "detail": null
      },
      "priceStability": {
        "score": 33,
        "detail": null
      }
    }
  },
  {
    "id": "dj106",
    "name": "Apple Watch Series 4 Gold",
    "brand": "Apple",
    "category": "Electronics > Accessories",
    "price": 29049,
    "originalPrice": 33010,
    "discount": 12,
    "rating": 2.7,
    "reviewCount": 32935,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Day after Tomorrow",
    "deliveryFree": true,
    "trustScore": 79,
    "trustLabel": "Genuine",
    "buyNowOrWait": "wait",
    "waitReason": "Sale expected in the next 2 weeks based on historical patterns.",
    "spikePriceMonths": [
      4
    ],
    "priceHistory": [
      29049,
      28058,
      29049,
      29049,
      36973,
      29049,
      29049,
      29049,
      29049,
      29049,
      28092,
      29049
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/mobile-accessories/apple-watch-series-4-gold/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/mobile-accessories/apple-watch-series-4-gold/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/mobile-accessories/apple-watch-series-4-gold/1.webp",
      "https://cdn.dummyjson.com/product-images/mobile-accessories/apple-watch-series-4-gold/2.webp"
    ],
    "description": "The Apple Watch Series 4 in Gold is a stylish and advanced smartwatch with features like heart rate monitoring, fitness tracking, and a beautiful Retina display.",
    "features": [
      "The Apple Watch Series 4 in Gold is a stylish and advanced smartwatch with features like heart rate monitoring, fitness tracking, and a beautiful Retina display",
      "Precisely engineered for Apple compatibility — fits most smartphones and tablets",
      "Military-grade construction that protects your device against daily wear and tear",
      "Ultra-slim profile — adds protection without adding bulk",
      "Hassle-free installation and removal — no tools required"
    ],
    "specs": {
      "Brand": "Apple",
      "Material": "Tempered Glass",
      "Compatibility": "Apple — compatible with most smartphones",
      "Connectivity": "Wired",
      "Weight": "36g",
      "Warranty": "6 months warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Apple",
    "soldByRating": 4.7,
    "sellerSince": "2014",
    "fulfillment": "Seller fulfilled",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 65,
        "detail": null
      },
      "returnRate": {
        "score": 76,
        "detail": null
      },
      "warrantyClaims": {
        "score": 84,
        "detail": null
      },
      "sellerReliability": {
        "score": 80,
        "detail": null
      },
      "priceStability": {
        "score": 81,
        "detail": null
      }
    }
  },
  {
    "id": "dj107",
    "name": "Beats Flex Wireless Earphones",
    "brand": "Beats",
    "category": "Electronics > Accessories",
    "price": 4149,
    "originalPrice": 4414,
    "discount": 6,
    "rating": 4.2,
    "reviewCount": 50259,
    "inStock": true,
    "isPrime": false,
    "delivery": "Get it by Tomorrow, 6 PM",
    "deliveryFree": true,
    "trustScore": 94,
    "trustLabel": "Genuine",
    "buyNowOrWait": "wait",
    "waitReason": "Sale expected in the next 2 weeks based on historical patterns.",
    "spikePriceMonths": [],
    "priceHistory": [
      4149,
      4149,
      4149,
      4149,
      4149,
      3716,
      4149,
      4149,
      4149,
      4149,
      4149,
      4149
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/mobile-accessories/beats-flex-wireless-earphones/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/mobile-accessories/beats-flex-wireless-earphones/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/mobile-accessories/beats-flex-wireless-earphones/1.webp"
    ],
    "description": "The Beats Flex Wireless Earphones offer a comfortable and versatile audio experience. With magnetic earbuds and up to 12 hours of battery life, they are ideal for everyday use.",
    "features": [
      "The Beats Flex Wireless Earphones offer a comfortable and versatile audio experience",
      "Precisely engineered for Beats compatibility — fits most smartphones and tablets",
      "Heavy-duty construction that protects your device against daily wear and tear",
      "Ultra-slim profile — adds protection without adding bulk",
      "Hassle-free installation and removal — no tools required"
    ],
    "specs": {
      "Brand": "Beats",
      "Material": "Hard Plastic",
      "Compatibility": "Beats — compatible with most smartphones",
      "Connectivity": "Wired",
      "Weight": "214g",
      "Warranty": "1 year warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Beats",
    "soldByRating": 4.7,
    "sellerSince": "2017",
    "fulfillment": "Fulfilled by Amazon",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 89,
        "detail": null
      },
      "returnRate": {
        "score": 96,
        "detail": null
      },
      "warrantyClaims": {
        "score": 86,
        "detail": null
      },
      "sellerReliability": {
        "score": 95,
        "detail": null
      },
      "priceStability": {
        "score": 88,
        "detail": null
      }
    }
  },
  {
    "id": "dj108",
    "name": "iPhone 12 Silicone Case with MagSafe Plum",
    "brand": "Apple",
    "category": "Electronics > Accessories",
    "price": 2489,
    "originalPrice": 2894,
    "discount": 14,
    "rating": 3.6,
    "reviewCount": 2439,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Day after Tomorrow",
    "deliveryFree": true,
    "trustScore": 81,
    "trustLabel": "Genuine",
    "buyNowOrWait": "buy",
    "waitReason": null,
    "spikePriceMonths": [
      2
    ],
    "priceHistory": [
      2655,
      2489,
      2809,
      2489,
      2489,
      2489,
      2489,
      2489,
      2489,
      2489,
      2489,
      2489
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/mobile-accessories/iphone-12-silicone-case-with-magsafe-plum/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/mobile-accessories/iphone-12-silicone-case-with-magsafe-plum/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/mobile-accessories/iphone-12-silicone-case-with-magsafe-plum/1.webp",
      "https://cdn.dummyjson.com/product-images/mobile-accessories/iphone-12-silicone-case-with-magsafe-plum/2.webp"
    ],
    "description": "The iPhone 12 Silicone Case with MagSafe in Plum is a stylish and protective case designed for the iPhone 12. It features MagSafe technology for easy attachment of accessories.",
    "features": [
      "The iPhone 12 Silicone Case with MagSafe in Plum is a stylish and protective case designed for the iPhone 12",
      "Precisely engineered for Apple compatibility — fits most smartphones and tablets",
      "Shock-absorbing construction that protects your device against daily wear and tear",
      "Compact profile — adds protection without adding bulk",
      "Hassle-free installation and removal — no tools required"
    ],
    "specs": {
      "Brand": "Apple",
      "Material": "Polycarbonate",
      "Compatibility": "Apple — compatible with most smartphones",
      "Connectivity": "Wireless",
      "Weight": "74g",
      "Warranty": "3 months warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Apple",
    "soldByRating": 4.3,
    "sellerSince": "2020",
    "fulfillment": "Fulfilled by Amazon",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 93,
        "detail": null
      },
      "returnRate": {
        "score": 76,
        "detail": null
      },
      "warrantyClaims": {
        "score": 88,
        "detail": null
      },
      "sellerReliability": {
        "score": 86,
        "detail": null
      },
      "priceStability": {
        "score": 76,
        "detail": null
      }
    }
  },
  {
    "id": "dj109",
    "name": "Monopod",
    "brand": "TechGear",
    "category": "Electronics > Accessories",
    "price": 1659,
    "originalPrice": 1823,
    "discount": 9,
    "rating": 4.4,
    "reviewCount": 16511,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Tomorrow, 6 PM",
    "deliveryFree": true,
    "trustScore": 95,
    "trustLabel": "Genuine",
    "buyNowOrWait": "buy",
    "waitReason": null,
    "spikePriceMonths": [],
    "priceHistory": [
      1659,
      1798,
      1659,
      1598,
      1659,
      1659,
      1659,
      1659,
      1659,
      1659,
      1659,
      1659
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/mobile-accessories/monopod/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/mobile-accessories/monopod/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/mobile-accessories/monopod/1.webp",
      "https://cdn.dummyjson.com/product-images/mobile-accessories/monopod/2.webp"
    ],
    "description": "The Monopod is a versatile camera accessory for stable and adjustable shooting. Perfect for capturing selfies, group photos, and videos with ease.",
    "features": [
      "The Monopod is a versatile camera accessory for stable and adjustable shooting",
      "Precisely engineered for TechGear compatibility — fits most smartphones and tablets",
      "Heavy-duty construction that protects your device against daily wear and tear",
      "Lightweight profile — adds protection without adding bulk",
      "Hassle-free installation and removal — no tools required"
    ],
    "specs": {
      "Brand": "TechGear",
      "Material": "Silicone",
      "Compatibility": "TechGear — compatible with most smartphones",
      "Connectivity": "USB-C",
      "Weight": "195g",
      "Warranty": "1 year warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "TechGear",
    "soldByRating": 4.9,
    "sellerSince": "2019",
    "fulfillment": "Seller fulfilled",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 96,
        "detail": null
      },
      "returnRate": {
        "score": 96,
        "detail": null
      },
      "warrantyClaims": {
        "score": 96,
        "detail": null
      },
      "sellerReliability": {
        "score": 96,
        "detail": null
      },
      "priceStability": {
        "score": 87,
        "detail": null
      }
    }
  },
  {
    "id": "dj110",
    "name": "Selfie Lamp with iPhone",
    "brand": "GadgetMaster",
    "category": "Electronics > Accessories",
    "price": 1244,
    "originalPrice": 1536,
    "discount": 19,
    "rating": 3.6,
    "reviewCount": 26029,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Tomorrow, 6 PM",
    "deliveryFree": true,
    "trustScore": 85,
    "trustLabel": "Genuine",
    "buyNowOrWait": "buy",
    "waitReason": null,
    "spikePriceMonths": [
      3
    ],
    "priceHistory": [
      1244,
      1373,
      1244,
      1483,
      1244,
      1244,
      1244,
      1244,
      1282,
      1244,
      1244,
      1244
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/mobile-accessories/selfie-lamp-with-iphone/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/mobile-accessories/selfie-lamp-with-iphone/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/mobile-accessories/selfie-lamp-with-iphone/1.webp"
    ],
    "description": "The Selfie Lamp with iPhone is a portable and adjustable LED light designed to enhance your selfies and video calls. Attach it to your iPhone for well-lit photos.",
    "features": [
      "The Selfie Lamp with iPhone is a portable and adjustable LED light designed to enhance your selfies and video calls",
      "Precisely engineered for GadgetMaster compatibility — fits most smartphones and tablets",
      "Military-grade construction that protects your device against daily wear and tear",
      "Lightweight profile — adds protection without adding bulk",
      "Hassle-free installation and removal — no tools required"
    ],
    "specs": {
      "Brand": "GadgetMaster",
      "Material": "Tempered Glass",
      "Compatibility": "GadgetMaster — compatible with most smartphones",
      "Connectivity": "USB-C",
      "Weight": "213g",
      "Warranty": "Lifetime warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "GadgetMaster",
    "soldByRating": 4.8,
    "sellerSince": "2018",
    "fulfillment": "Seller fulfilled",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 93,
        "detail": null
      },
      "returnRate": {
        "score": 84,
        "detail": null
      },
      "warrantyClaims": {
        "score": 93,
        "detail": null
      },
      "sellerReliability": {
        "score": 95,
        "detail": null
      },
      "priceStability": {
        "score": 78,
        "detail": null
      }
    }
  },
  {
    "id": "dj111",
    "name": "Selfie Stick Monopod",
    "brand": "SnapTech",
    "category": "Electronics > Accessories",
    "price": 1078,
    "originalPrice": 1331,
    "discount": 19,
    "rating": 3.9,
    "reviewCount": 49101,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Tomorrow, 6 PM",
    "deliveryFree": true,
    "trustScore": 45,
    "trustLabel": "Suspicious",
    "buyNowOrWait": "wait",
    "waitReason": "Sale expected in the next 2 weeks based on historical patterns.",
    "spikePriceMonths": [
      1,
      2,
      3,
      9
    ],
    "priceHistory": [
      1078,
      1541,
      1268,
      1471,
      1192,
      1078,
      1078,
      1078,
      1078,
      1411,
      1078,
      1078
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/mobile-accessories/selfie-stick-monopod/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/mobile-accessories/selfie-stick-monopod/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/mobile-accessories/selfie-stick-monopod/1.webp"
    ],
    "description": "The Selfie Stick Monopod is a extendable and foldable device for capturing the perfect selfie or group photo. Compatible with smartphones and cameras.",
    "features": [
      "The Selfie Stick Monopod is a extendable and foldable device for capturing the perfect selfie or group photo",
      "Precisely engineered for SnapTech compatibility — fits most smartphones and tablets",
      "Heavy-duty construction that protects your device against daily wear and tear",
      "Ultra-slim profile — adds protection without adding bulk",
      "Hassle-free installation and removal — no tools required"
    ],
    "specs": {
      "Brand": "SnapTech",
      "Material": "Silicone",
      "Compatibility": "SnapTech — compatible with most smartphones",
      "Connectivity": "Wired",
      "Weight": "217g",
      "Warranty": "3 year warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "SnapTech",
    "soldByRating": 2.8,
    "sellerSince": "2021",
    "fulfillment": "Seller fulfilled",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 31,
        "detail": null
      },
      "returnRate": {
        "score": 46,
        "detail": null
      },
      "warrantyClaims": {
        "score": 49,
        "detail": null
      },
      "sellerReliability": {
        "score": 48,
        "detail": null
      },
      "priceStability": {
        "score": 55,
        "detail": null
      }
    }
  },
  {
    "id": "dj112",
    "name": "TV Studio Camera Pedestal",
    "brand": "ProVision",
    "category": "Electronics > Accessories",
    "price": 41499,
    "originalPrice": 45108,
    "discount": 8,
    "rating": 2.8,
    "reviewCount": 65638,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Day after Tomorrow",
    "deliveryFree": true,
    "trustScore": 77,
    "trustLabel": "Genuine",
    "buyNowOrWait": "wait",
    "waitReason": "Sale expected in the next 2 weeks based on historical patterns.",
    "spikePriceMonths": [
      5
    ],
    "priceHistory": [
      41499,
      41499,
      41499,
      41499,
      41499,
      49745,
      41499,
      41499,
      43631,
      41499,
      44010,
      41499
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/mobile-accessories/tv-studio-camera-pedestal/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/mobile-accessories/tv-studio-camera-pedestal/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/mobile-accessories/tv-studio-camera-pedestal/1.webp"
    ],
    "description": "The TV Studio Camera Pedestal is a professional-grade camera support system for smooth and precise camera movements in a studio setting. Ideal for broadcast and production.",
    "features": [
      "The TV Studio Camera Pedestal is a professional-grade camera support system for smooth and precise camera movements in a studio setting",
      "Precisely engineered for ProVision compatibility — fits most smartphones and tablets",
      "Scratch-resistant construction that protects your device against daily wear and tear",
      "Lightweight profile — adds protection without adding bulk",
      "Hassle-free installation and removal — no tools required"
    ],
    "specs": {
      "Brand": "ProVision",
      "Material": "Thermoplastic Polyurethane (TPU)",
      "Compatibility": "ProVision — compatible with most smartphones",
      "Connectivity": "3.5mm Audio Jack",
      "Weight": "66g",
      "Warranty": "3 months warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "ProVision",
    "soldByRating": 3.9,
    "sellerSince": "2010",
    "fulfillment": "Fulfilled by Amazon",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 79,
        "detail": null
      },
      "returnRate": {
        "score": 75,
        "detail": null
      },
      "warrantyClaims": {
        "score": 72,
        "detail": null
      },
      "sellerReliability": {
        "score": 79,
        "detail": null
      },
      "priceStability": {
        "score": 82,
        "detail": null
      }
    }
  },
  {
    "id": "dj118",
    "name": "Attitude Super Leaves Hand Soap",
    "brand": "Attitude",
    "category": "Beauty",
    "price": 746,
    "originalPrice": 910,
    "discount": 18,
    "rating": 3.2,
    "reviewCount": 1539,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Day after Tomorrow",
    "deliveryFree": true,
    "trustScore": 77,
    "trustLabel": "Genuine",
    "buyNowOrWait": "buy",
    "waitReason": null,
    "spikePriceMonths": [
      9
    ],
    "priceHistory": [
      746,
      746,
      790,
      746,
      746,
      746,
      821,
      746,
      746,
      980,
      746,
      746
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/skin-care/attitude-super-leaves-hand-soap/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/skin-care/attitude-super-leaves-hand-soap/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/skin-care/attitude-super-leaves-hand-soap/1.webp",
      "https://cdn.dummyjson.com/product-images/skin-care/attitude-super-leaves-hand-soap/2.webp"
    ],
    "description": "Attitude Super Leaves Hand Soap is a natural and nourishing hand soap enriched with the goodness of super leaves. It cleanses and moisturizes your hands, leaving them feeling fresh and soft.",
    "features": [
      "Attitude Super Leaves Hand Soap is a natural and nourishing hand soap enriched with the goodness of super leaves",
      "Formulated with hyaluronic acid & niacinamide for visible results",
      "Suitable for dry to normal skin",
      "Paraben-free, sulphate-free & fragrance-free",
      "Visible results in 7 days of regular use"
    ],
    "specs": {
      "Volume": "1000 ml",
      "Skin Type": "Dry to Normal",
      "Key Active Ingredients": "Hyaluronic Acid 2%, Niacinamide 5%",
      "Free From": "Mineral Oil, SLS, SLES",
      "SPF": "No SPF",
      "Usage": "Twice weekly",
      "Shelf Life": "12 months after opening"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Attitude",
    "soldByRating": 3.9,
    "sellerSince": "2020",
    "fulfillment": "Seller fulfilled",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 76,
        "detail": null
      },
      "returnRate": {
        "score": 74,
        "detail": null
      },
      "warrantyClaims": {
        "score": 84,
        "detail": null
      },
      "sellerReliability": {
        "score": 84,
        "detail": null
      },
      "priceStability": {
        "score": 76,
        "detail": null
      }
    }
  },
  {
    "id": "dj119",
    "name": "Olay Ultra Moisture Shea Butter Body Wash",
    "brand": "Olay",
    "category": "Beauty",
    "price": 1078,
    "originalPrice": 1283,
    "discount": 16,
    "rating": 4.5,
    "reviewCount": 4940,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Day after Tomorrow",
    "deliveryFree": true,
    "trustScore": 96,
    "trustLabel": "Genuine",
    "buyNowOrWait": "wait",
    "waitReason": "Sale expected in the next 2 weeks based on historical patterns.",
    "spikePriceMonths": [
      9
    ],
    "priceHistory": [
      1078,
      1078,
      1078,
      1078,
      1078,
      1078,
      1078,
      1078,
      1078,
      1227,
      1076,
      1078
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/skin-care/olay-ultra-moisture-shea-butter-body-wash/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/skin-care/olay-ultra-moisture-shea-butter-body-wash/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/skin-care/olay-ultra-moisture-shea-butter-body-wash/1.webp",
      "https://cdn.dummyjson.com/product-images/skin-care/olay-ultra-moisture-shea-butter-body-wash/2.webp"
    ],
    "description": "Olay Ultra Moisture Shea Butter Body Wash is a luxurious body wash that hydrates and nourishes your skin with the moisturizing power of shea butter. Enjoy a rich lather and silky-smooth skin.",
    "features": [
      "Olay Ultra Moisture Shea Butter Body Wash is a luxurious body wash that hydrates and nourishes your skin with the moisturizing power of shea butter",
      "Formulated with kojic acid & glycerin for visible results",
      "Suitable for dry to normal skin",
      "Paraben-free, sulphate-free & fragrance-free",
      "Visible results in 3 weeks of regular use"
    ],
    "specs": {
      "Volume": "4000 ml",
      "Skin Type": "Dry to Normal",
      "Key Active Ingredients": "Hyaluronic Acid 2%, Niacinamide 5%",
      "Free From": "Phthalates, Artificial Fragrance",
      "SPF": "SPF 15",
      "Usage": "AM only",
      "Shelf Life": "12 months after opening"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Olay",
    "soldByRating": 4.2,
    "sellerSince": "2019",
    "fulfillment": "Seller fulfilled",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 83,
        "detail": null
      },
      "returnRate": {
        "score": 96,
        "detail": null
      },
      "warrantyClaims": {
        "score": 96,
        "detail": null
      },
      "sellerReliability": {
        "score": 96,
        "detail": null
      },
      "priceStability": {
        "score": 96,
        "detail": null
      }
    }
  },
  {
    "id": "dj120",
    "name": "Vaseline Men Body and Face Lotion",
    "brand": "Vaseline",
    "category": "Beauty",
    "price": 829,
    "originalPrice": 953,
    "discount": 13,
    "rating": 3.2,
    "reviewCount": 49954,
    "inStock": true,
    "isPrime": false,
    "delivery": "Get it by Day after Tomorrow",
    "deliveryFree": true,
    "trustScore": 34,
    "trustLabel": "Suspicious",
    "buyNowOrWait": "buy",
    "waitReason": null,
    "spikePriceMonths": [
      2,
      6
    ],
    "priceHistory": [
      829,
      829,
      1062,
      829,
      840,
      829,
      961,
      829,
      829,
      829,
      916,
      829
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/skin-care/vaseline-men-body-and-face-lotion/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/skin-care/vaseline-men-body-and-face-lotion/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/skin-care/vaseline-men-body-and-face-lotion/1.webp",
      "https://cdn.dummyjson.com/product-images/skin-care/vaseline-men-body-and-face-lotion/2.webp"
    ],
    "description": "Vaseline Men Body and Face Lotion is a specially formulated lotion designed to provide long-lasting moisture to men's skin. It absorbs quickly and helps keep the skin hydrated and healthy.",
    "features": [
      "Vaseline Men Body and Face Lotion is a specially formulated lotion designed to provide long-lasting moisture to men's skin",
      "Formulated with hyaluronic acid & niacinamide for visible results",
      "Suitable for all skin types",
      "Vegan & cruelty-free",
      "Visible results in 2–4 weeks of regular use"
    ],
    "specs": {
      "Volume": "4000 ml",
      "Skin Type": "All skin types",
      "Key Active Ingredients": "Ceramide NP, Hyaluronic Acid",
      "Free From": "Paraben, Sulphate, Fragrance",
      "SPF": "No SPF",
      "Usage": "Twice weekly",
      "Shelf Life": "12 months after opening"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Vaseline",
    "soldByRating": 2.4,
    "sellerSince": "2024",
    "fulfillment": "Seller fulfilled",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 39,
        "detail": null
      },
      "returnRate": {
        "score": 28,
        "detail": null
      },
      "warrantyClaims": {
        "score": 27,
        "detail": null
      },
      "sellerReliability": {
        "score": 37,
        "detail": null
      },
      "priceStability": {
        "score": 26,
        "detail": null
      }
    }
  },
  {
    "id": "dj121",
    "name": "iPhone 5s",
    "brand": "Apple",
    "category": "Electronics > Mobiles",
    "price": 16599,
    "originalPrice": 19079,
    "discount": 13,
    "rating": 2.8,
    "reviewCount": 31279,
    "inStock": true,
    "isPrime": false,
    "delivery": "Get it by Tomorrow, 6 PM",
    "deliveryFree": true,
    "trustScore": 82,
    "trustLabel": "Genuine",
    "buyNowOrWait": "wait",
    "waitReason": "Sale expected in the next 2 weeks based on historical patterns.",
    "spikePriceMonths": [
      10
    ],
    "priceHistory": [
      16599,
      16599,
      16599,
      16599,
      16599,
      16599,
      16599,
      16599,
      16599,
      17968,
      22193,
      16599
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/smartphones/iphone-5s/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/smartphones/iphone-5s/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/smartphones/iphone-5s/1.webp",
      "https://cdn.dummyjson.com/product-images/smartphones/iphone-5s/2.webp"
    ],
    "description": "The iPhone 5s is a classic smartphone known for its compact design and advanced features during its release. While it's an older model, it still provides a reliable user experience.",
    "features": [
      "The iPhone 5s is a classic smartphone known for its compact design and advanced features during its release",
      "4GB RAM with 256GB storage for seamless multitasking",
      "4500mAh battery with 33W fast charging",
      "6.1\" AMOLED display with 120Hz refresh rate",
      "Quad rear camera with 108MP primary sensor for professional-grade photography"
    ],
    "specs": {
      "Brand": "Apple",
      "Display": "6.1\" AMOLED",
      "Processor": "Exynos 2200",
      "RAM": "4GB",
      "Storage": "64GB",
      "Battery": "4500mAh",
      "Rear Camera": "108MP + 10MP + 8MP",
      "Front Camera": "12MP",
      "OS": "Android 13",
      "SIM": "Dual SIM (Nano + Nano)",
      "Connectivity": "5G / Wi-Fi 6 / Bluetooth 5.2 / NFC",
      "Warranty": "Lifetime warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Apple",
    "soldByRating": 3.9,
    "sellerSince": "2015",
    "fulfillment": "Seller fulfilled",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 95,
        "detail": null
      },
      "returnRate": {
        "score": 89,
        "detail": null
      },
      "warrantyClaims": {
        "score": 85,
        "detail": null
      },
      "sellerReliability": {
        "score": 88,
        "detail": null
      },
      "priceStability": {
        "score": 73,
        "detail": null
      }
    }
  },
  {
    "id": "dj122",
    "name": "iPhone 6",
    "brand": "Apple",
    "category": "Electronics > Mobiles",
    "price": 24899,
    "originalPrice": 26773,
    "discount": 7,
    "rating": 3.4,
    "reviewCount": 4696,
    "inStock": true,
    "isPrime": false,
    "delivery": "Get it by Tomorrow, 6 PM",
    "deliveryFree": true,
    "trustScore": 86,
    "trustLabel": "Genuine",
    "buyNowOrWait": "buy",
    "waitReason": null,
    "spikePriceMonths": [
      7
    ],
    "priceHistory": [
      24899,
      24899,
      24899,
      24899,
      24899,
      24899,
      24899,
      30724,
      24899,
      24899,
      27218,
      24899
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/smartphones/iphone-6/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/smartphones/iphone-6/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/smartphones/iphone-6/1.webp",
      "https://cdn.dummyjson.com/product-images/smartphones/iphone-6/2.webp"
    ],
    "description": "The iPhone 6 is a stylish and capable smartphone with a larger display and improved performance. It introduced new features and design elements, making it a popular choice in its time.",
    "features": [
      "The iPhone 6 is a stylish and capable smartphone with a larger display and improved performance",
      "4GB RAM with 256GB storage for seamless multitasking",
      "5000mAh battery with 33W fast charging",
      "6.4\" OLED display with 120Hz refresh rate",
      "Triple rear camera with 48MP primary sensor for professional-grade photography"
    ],
    "specs": {
      "Brand": "Apple",
      "Display": "6.4\" OLED",
      "Processor": "Exynos 2200",
      "RAM": "4GB",
      "Storage": "256GB",
      "Battery": "5000mAh",
      "Rear Camera": "48MP + 12MP + 2MP",
      "Front Camera": "32MP",
      "OS": "Android 13",
      "SIM": "Dual SIM (Nano + eSIM)",
      "Connectivity": "5G / Wi-Fi 6 / Bluetooth 5.2 / NFC",
      "Warranty": "1 month warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Apple",
    "soldByRating": 3.9,
    "sellerSince": "2020",
    "fulfillment": "Fulfilled by Amazon",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 78,
        "detail": null
      },
      "returnRate": {
        "score": 85,
        "detail": null
      },
      "warrantyClaims": {
        "score": 91,
        "detail": null
      },
      "sellerReliability": {
        "score": 92,
        "detail": null
      },
      "priceStability": {
        "score": 92,
        "detail": null
      }
    }
  },
  {
    "id": "dj123",
    "name": "iPhone 13 Pro",
    "brand": "Apple",
    "category": "Electronics > Mobiles",
    "price": 91299,
    "originalPrice": 100329,
    "discount": 9,
    "rating": 4.1,
    "reviewCount": 70574,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Day after Tomorrow",
    "deliveryFree": true,
    "trustScore": 83,
    "trustLabel": "Genuine",
    "buyNowOrWait": "wait",
    "waitReason": "Sale expected in the next 2 weeks based on historical patterns.",
    "spikePriceMonths": [
      5
    ],
    "priceHistory": [
      91299,
      91299,
      91299,
      91299,
      91299,
      108523,
      88597,
      91299,
      91299,
      91299,
      91299,
      91299
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/smartphones/iphone-13-pro/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/smartphones/iphone-13-pro/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/smartphones/iphone-13-pro/1.webp",
      "https://cdn.dummyjson.com/product-images/smartphones/iphone-13-pro/2.webp"
    ],
    "description": "The iPhone 13 Pro is a cutting-edge smartphone with a powerful camera system, high-performance chip, and stunning display. It offers advanced features for users who demand top-notch technology.",
    "features": [
      "The iPhone 13 Pro is a cutting-edge smartphone with a powerful camera system, high-performance chip, and stunning display",
      "8GB RAM with 128GB storage for seamless multitasking",
      "5500mAh battery with 45W fast charging",
      "6.5\" AMOLED display with 60Hz refresh rate",
      "Quad rear camera with 48MP primary sensor for professional-grade photography"
    ],
    "specs": {
      "Brand": "Apple",
      "Display": "6.5\" AMOLED",
      "Processor": "Exynos 2200",
      "RAM": "8GB",
      "Storage": "128GB",
      "Battery": "5500mAh",
      "Rear Camera": "48MP + 5MP + 2MP",
      "Front Camera": "12MP",
      "OS": "Android 14",
      "SIM": "Dual SIM (Nano + eSIM)",
      "Connectivity": "5G / Wi-Fi 6 / Bluetooth 5.2 / NFC",
      "Warranty": "3 year warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Apple",
    "soldByRating": 4.7,
    "sellerSince": "2013",
    "fulfillment": "Fulfilled by Amazon",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 85,
        "detail": null
      },
      "returnRate": {
        "score": 78,
        "detail": null
      },
      "warrantyClaims": {
        "score": 85,
        "detail": null
      },
      "sellerReliability": {
        "score": 85,
        "detail": null
      },
      "priceStability": {
        "score": 88,
        "detail": null
      }
    }
  },
  {
    "id": "dj124",
    "name": "iPhone X",
    "brand": "Apple",
    "category": "Electronics > Mobiles",
    "price": 74699,
    "originalPrice": 93374,
    "discount": 20,
    "rating": 2.5,
    "reviewCount": 27230,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Day after Tomorrow",
    "deliveryFree": true,
    "trustScore": 73,
    "trustLabel": "Mixed",
    "buyNowOrWait": "buy",
    "waitReason": null,
    "spikePriceMonths": [
      3
    ],
    "priceHistory": [
      74699,
      74699,
      74699,
      91554,
      74699,
      74699,
      74699,
      74699,
      74699,
      74699,
      74699,
      74699
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/smartphones/iphone-x/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/smartphones/iphone-x/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/smartphones/iphone-x/1.webp",
      "https://cdn.dummyjson.com/product-images/smartphones/iphone-x/2.webp"
    ],
    "description": "The iPhone X is a flagship smartphone featuring a bezel-less OLED display, facial recognition technology (Face ID), and impressive performance. It represents a milestone in iPhone design and innovation.",
    "features": [
      "The iPhone X is a flagship smartphone featuring a bezel-less OLED display, facial recognition technology (Face ID), and impressive performance",
      "6GB RAM with 64GB storage for seamless multitasking",
      "4500mAh battery with 25W fast charging",
      "6.5\" IPS LCD display with 120Hz refresh rate",
      "Triple rear camera with 48MP primary sensor for professional-grade photography"
    ],
    "specs": {
      "Brand": "Apple",
      "Display": "6.5\" IPS LCD",
      "Processor": "Snapdragon 8 Gen 2",
      "RAM": "6GB",
      "Storage": "512GB",
      "Battery": "4500mAh",
      "Rear Camera": "48MP + 5MP + 2MP",
      "Front Camera": "32MP",
      "OS": "Android 14",
      "SIM": "Single SIM",
      "Connectivity": "5G / Wi-Fi 6 / Bluetooth 5.2 / NFC",
      "Warranty": "3 months warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Apple",
    "soldByRating": 4.1,
    "sellerSince": "2011",
    "fulfillment": "Fulfilled by Amazon",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 77,
        "detail": null
      },
      "returnRate": {
        "score": 69,
        "detail": null
      },
      "warrantyClaims": {
        "score": 79,
        "detail": null
      },
      "sellerReliability": {
        "score": 75,
        "detail": null
      },
      "priceStability": {
        "score": 77,
        "detail": null
      }
    }
  },
  {
    "id": "dj125",
    "name": "Oppo A57",
    "brand": "Oppo",
    "category": "Electronics > Mobiles",
    "price": 20749,
    "originalPrice": 21172,
    "discount": 2,
    "rating": 3.9,
    "reviewCount": 30913,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Tomorrow, 6 PM",
    "deliveryFree": true,
    "trustScore": 91,
    "trustLabel": "Genuine",
    "buyNowOrWait": "wait",
    "waitReason": "Sale expected in the next 2 weeks based on historical patterns.",
    "spikePriceMonths": [
      2
    ],
    "priceHistory": [
      20749,
      20749,
      24770,
      20749,
      20749,
      20749,
      21347,
      20749,
      20749,
      20749,
      20749,
      20749
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/smartphones/oppo-a57/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/smartphones/oppo-a57/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/smartphones/oppo-a57/1.webp",
      "https://cdn.dummyjson.com/product-images/smartphones/oppo-a57/2.webp"
    ],
    "description": "The Oppo A57 is a mid-range smartphone known for its sleek design and capable features. It offers a balance of performance and affordability, making it a popular choice.",
    "features": [
      "The Oppo A57 is a mid-range smartphone known for its sleek design and capable features",
      "4GB RAM with 128GB storage for seamless multitasking",
      "4500mAh battery with 65W fast charging",
      "6.1\" IPS LCD display with 60Hz refresh rate",
      "Triple rear camera with 50MP primary sensor for professional-grade photography"
    ],
    "specs": {
      "Brand": "Oppo",
      "Display": "6.1\" IPS LCD",
      "Processor": "Snapdragon 7s Gen 2",
      "RAM": "4GB",
      "Storage": "128GB",
      "Battery": "4500mAh",
      "Rear Camera": "50MP + 5MP + 2MP",
      "Front Camera": "20MP",
      "OS": "Android 12",
      "SIM": "Single SIM",
      "Connectivity": "5G / Wi-Fi 6 / Bluetooth 5.2 / NFC",
      "Warranty": "Lifetime warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Oppo",
    "soldByRating": 4.4,
    "sellerSince": "2021",
    "fulfillment": "Fulfilled by Amazon",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 96,
        "detail": null
      },
      "returnRate": {
        "score": 91,
        "detail": null
      },
      "warrantyClaims": {
        "score": 88,
        "detail": null
      },
      "sellerReliability": {
        "score": 96,
        "detail": null
      },
      "priceStability": {
        "score": 87,
        "detail": null
      }
    }
  },
  {
    "id": "dj126",
    "name": "Oppo F19 Pro Plus",
    "brand": "Oppo",
    "category": "Electronics > Mobiles",
    "price": 33199,
    "originalPrice": 40986,
    "discount": 19,
    "rating": 3.5,
    "reviewCount": 2753,
    "inStock": true,
    "isPrime": false,
    "delivery": "Get it by Day after Tomorrow",
    "deliveryFree": true,
    "trustScore": 42,
    "trustLabel": "Suspicious",
    "buyNowOrWait": "wait",
    "waitReason": "Sale expected in the next 2 weeks based on historical patterns.",
    "spikePriceMonths": [
      2,
      9
    ],
    "priceHistory": [
      33199,
      33199,
      46543,
      33199,
      33199,
      33199,
      33199,
      35986,
      35147,
      44721,
      33199,
      33199
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/smartphones/oppo-f19-pro-plus/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/smartphones/oppo-f19-pro-plus/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/smartphones/oppo-f19-pro-plus/1.webp",
      "https://cdn.dummyjson.com/product-images/smartphones/oppo-f19-pro-plus/2.webp"
    ],
    "description": "The Oppo F19 Pro Plus is a feature-rich smartphone with a focus on camera capabilities. It boasts advanced photography features and a powerful performance for a premium user experience.",
    "features": [
      "The Oppo F19 Pro Plus is a feature-rich smartphone with a focus on camera capabilities",
      "12GB RAM with 128GB storage for seamless multitasking",
      "5000mAh battery with 33W fast charging",
      "6.5\" AMOLED display with 90Hz refresh rate",
      "Dual rear camera with 48MP primary sensor for professional-grade photography"
    ],
    "specs": {
      "Brand": "Oppo",
      "Display": "6.5\" AMOLED",
      "Processor": "Snapdragon 7s Gen 2",
      "RAM": "12GB",
      "Storage": "256GB",
      "Battery": "5000mAh",
      "Rear Camera": "48MP + 12MP + 8MP",
      "Front Camera": "32MP",
      "OS": "Android 13",
      "SIM": "Dual SIM (Nano + Nano)",
      "Connectivity": "5G / Wi-Fi 6 / Bluetooth 5.2 / NFC",
      "Warranty": "3 year warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Oppo",
    "soldByRating": 2.2,
    "sellerSince": "2023",
    "fulfillment": "Seller fulfilled",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 32,
        "detail": null
      },
      "returnRate": {
        "score": 48,
        "detail": null
      },
      "warrantyClaims": {
        "score": 47,
        "detail": null
      },
      "sellerReliability": {
        "score": 50,
        "detail": null
      },
      "priceStability": {
        "score": 35,
        "detail": null
      }
    }
  },
  {
    "id": "dj127",
    "name": "Oppo K1",
    "brand": "Oppo",
    "category": "Electronics > Mobiles",
    "price": 24899,
    "originalPrice": 30365,
    "discount": 18,
    "rating": 4.3,
    "reviewCount": 20326,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Day after Tomorrow",
    "deliveryFree": true,
    "trustScore": 84,
    "trustLabel": "Genuine",
    "buyNowOrWait": "buy",
    "waitReason": null,
    "spikePriceMonths": [
      4,
      7
    ],
    "priceHistory": [
      24899,
      24899,
      24899,
      24899,
      35032,
      24899,
      27519,
      31044,
      24899,
      24899,
      24899,
      24899
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/smartphones/oppo-k1/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/smartphones/oppo-k1/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/smartphones/oppo-k1/1.webp",
      "https://cdn.dummyjson.com/product-images/smartphones/oppo-k1/2.webp"
    ],
    "description": "The Oppo K1 series offers a range of smartphones with various features and specifications. Known for their stylish design and reliable performance, the Oppo K1 series caters to diverse user preferences.",
    "features": [
      "The Oppo K1 series offers a range of smartphones with various features and specifications",
      "8GB RAM with 256GB storage for seamless multitasking",
      "4500mAh battery with 25W fast charging",
      "6.7\" IPS LCD display with 144Hz refresh rate",
      "Triple rear camera with 50MP primary sensor for professional-grade photography"
    ],
    "specs": {
      "Brand": "Oppo",
      "Display": "6.7\" IPS LCD",
      "Processor": "MediaTek Dimensity 9000",
      "RAM": "8GB",
      "Storage": "256GB",
      "Battery": "4500mAh",
      "Rear Camera": "50MP + 12MP + 8MP",
      "Front Camera": "32MP",
      "OS": "Android 12",
      "SIM": "Dual SIM (Nano + Nano)",
      "Connectivity": "5G / Wi-Fi 6 / Bluetooth 5.2 / NFC",
      "Warranty": "1 month warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Oppo",
    "soldByRating": 4.8,
    "sellerSince": "2021",
    "fulfillment": "Fulfilled by Amazon",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 72,
        "detail": null
      },
      "returnRate": {
        "score": 91,
        "detail": null
      },
      "warrantyClaims": {
        "score": 92,
        "detail": null
      },
      "sellerReliability": {
        "score": 95,
        "detail": null
      },
      "priceStability": {
        "score": 74,
        "detail": null
      }
    }
  },
  {
    "id": "dj128",
    "name": "Realme C35",
    "brand": "Realme",
    "category": "Electronics > Mobiles",
    "price": 12449,
    "originalPrice": 14646,
    "discount": 15,
    "rating": 4.2,
    "reviewCount": 30242,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Tomorrow, 6 PM",
    "deliveryFree": true,
    "trustScore": 94,
    "trustLabel": "Genuine",
    "buyNowOrWait": "buy",
    "waitReason": null,
    "spikePriceMonths": [
      4,
      5,
      10
    ],
    "priceHistory": [
      12449,
      12449,
      12449,
      12449,
      14203,
      16490,
      12449,
      12449,
      12449,
      12449,
      14021,
      12449
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/smartphones/realme-c35/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/smartphones/realme-c35/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/smartphones/realme-c35/1.webp",
      "https://cdn.dummyjson.com/product-images/smartphones/realme-c35/2.webp"
    ],
    "description": "The Realme C35 is a budget-friendly smartphone with a focus on providing essential features for everyday use. It offers a reliable performance and user-friendly experience.",
    "features": [
      "The Realme C35 is a budget-friendly smartphone with a focus on providing essential features for everyday use",
      "4GB RAM with 256GB storage for seamless multitasking",
      "4500mAh battery with 25W fast charging",
      "6.1\" IPS LCD display with 120Hz refresh rate",
      "Quad rear camera with 48MP primary sensor for professional-grade photography"
    ],
    "specs": {
      "Brand": "Realme",
      "Display": "6.1\" IPS LCD",
      "Processor": "MediaTek Dimensity 9000",
      "RAM": "4GB",
      "Storage": "256GB",
      "Battery": "4500mAh",
      "Rear Camera": "48MP + 12MP + 5MP",
      "Front Camera": "12MP",
      "OS": "Android 12",
      "SIM": "Dual SIM (Nano + Nano)",
      "Connectivity": "5G / Wi-Fi 6 / Bluetooth 5.2 / NFC",
      "Warranty": "3 year warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Realme",
    "soldByRating": 3.9,
    "sellerSince": "2015",
    "fulfillment": "Fulfilled by Amazon",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 83,
        "detail": null
      },
      "returnRate": {
        "score": 96,
        "detail": null
      },
      "warrantyClaims": {
        "score": 94,
        "detail": null
      },
      "sellerReliability": {
        "score": 96,
        "detail": null
      },
      "priceStability": {
        "score": 90,
        "detail": null
      }
    }
  },
  {
    "id": "dj129",
    "name": "Realme X",
    "brand": "Realme",
    "category": "Electronics > Mobiles",
    "price": 24899,
    "originalPrice": 26773,
    "discount": 7,
    "rating": 3.7,
    "reviewCount": 53618,
    "inStock": true,
    "isPrime": false,
    "delivery": "Get it by Tomorrow, 6 PM",
    "deliveryFree": true,
    "trustScore": 88,
    "trustLabel": "Genuine",
    "buyNowOrWait": "wait",
    "waitReason": "Sale expected in the next 2 weeks based on historical patterns.",
    "spikePriceMonths": [],
    "priceHistory": [
      25983,
      24899,
      24899,
      24899,
      24899,
      24899,
      24899,
      24899,
      24899,
      24899,
      24899,
      24899
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/smartphones/realme-x/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/smartphones/realme-x/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/smartphones/realme-x/1.webp",
      "https://cdn.dummyjson.com/product-images/smartphones/realme-x/2.webp"
    ],
    "description": "The Realme X is a mid-range smartphone known for its sleek design and impressive display. It offers a good balance of performance and camera capabilities for users seeking a quality device.",
    "features": [
      "The Realme X is a mid-range smartphone known for its sleek design and impressive display",
      "12GB RAM with 128GB storage for seamless multitasking",
      "4000mAh battery with 33W fast charging",
      "6.1\" Super AMOLED display with 144Hz refresh rate",
      "Quad rear camera with 64MP primary sensor for professional-grade photography"
    ],
    "specs": {
      "Brand": "Realme",
      "Display": "6.1\" Super AMOLED",
      "Processor": "Exynos 2200",
      "RAM": "12GB",
      "Storage": "128GB",
      "Battery": "4000mAh",
      "Rear Camera": "64MP + 8MP + 5MP",
      "Front Camera": "16MP",
      "OS": "Android 12",
      "SIM": "Dual SIM (Nano + Nano)",
      "Connectivity": "5G / Wi-Fi 6 / Bluetooth 5.2 / NFC",
      "Warranty": "1 month warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Realme",
    "soldByRating": 5,
    "sellerSince": "2015",
    "fulfillment": "Seller fulfilled",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 82,
        "detail": null
      },
      "returnRate": {
        "score": 81,
        "detail": null
      },
      "warrantyClaims": {
        "score": 81,
        "detail": null
      },
      "sellerReliability": {
        "score": 96,
        "detail": null
      },
      "priceStability": {
        "score": 83,
        "detail": null
      }
    }
  },
  {
    "id": "dj130",
    "name": "Realme XT",
    "brand": "Realme",
    "category": "Electronics > Mobiles",
    "price": 29049,
    "originalPrice": 33010,
    "discount": 12,
    "rating": 4.6,
    "reviewCount": 54952,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Day after Tomorrow",
    "deliveryFree": true,
    "trustScore": 90,
    "trustLabel": "Genuine",
    "buyNowOrWait": "buy",
    "waitReason": null,
    "spikePriceMonths": [
      7
    ],
    "priceHistory": [
      29049,
      29049,
      29049,
      29049,
      29049,
      29049,
      29049,
      34922,
      29049,
      29049,
      27570,
      29049
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/smartphones/realme-xt/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/smartphones/realme-xt/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/smartphones/realme-xt/1.webp",
      "https://cdn.dummyjson.com/product-images/smartphones/realme-xt/2.webp"
    ],
    "description": "The Realme XT is a feature-rich smartphone with a focus on camera technology. It comes equipped with advanced camera sensors, delivering high-quality photos and videos for photography enthusiasts.",
    "features": [
      "The Realme XT is a feature-rich smartphone with a focus on camera technology",
      "8GB RAM with 64GB storage for seamless multitasking",
      "4000mAh battery with 25W fast charging",
      "6.5\" IPS LCD display with 90Hz refresh rate",
      "Quad rear camera with 50MP primary sensor for professional-grade photography"
    ],
    "specs": {
      "Brand": "Realme",
      "Display": "6.5\" IPS LCD",
      "Processor": "Snapdragon 8 Gen 2",
      "RAM": "8GB",
      "Storage": "512GB",
      "Battery": "4000mAh",
      "Rear Camera": "50MP + 5MP + 2MP",
      "Front Camera": "20MP",
      "OS": "Android 14",
      "SIM": "Single SIM",
      "Connectivity": "5G / Wi-Fi 6 / Bluetooth 5.2 / NFC",
      "Warranty": "3 year warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Realme",
    "soldByRating": 4.3,
    "sellerSince": "2015",
    "fulfillment": "Fulfilled by Amazon",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 96,
        "detail": null
      },
      "returnRate": {
        "score": 89,
        "detail": null
      },
      "warrantyClaims": {
        "score": 86,
        "detail": null
      },
      "sellerReliability": {
        "score": 92,
        "detail": null
      },
      "priceStability": {
        "score": 91,
        "detail": null
      }
    }
  },
  {
    "id": "dj131",
    "name": "Samsung Galaxy S7",
    "brand": "Samsung",
    "category": "Electronics > Mobiles",
    "price": 24899,
    "originalPrice": 31124,
    "discount": 20,
    "rating": 3.3,
    "reviewCount": 16416,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Tomorrow, 6 PM",
    "deliveryFree": true,
    "trustScore": 82,
    "trustLabel": "Genuine",
    "buyNowOrWait": "wait",
    "waitReason": "Sale expected in the next 2 weeks based on historical patterns.",
    "spikePriceMonths": [
      1,
      6
    ],
    "priceHistory": [
      24899,
      34956,
      27315,
      24899,
      24899,
      24899,
      33276,
      24899,
      24899,
      24899,
      24899,
      24899
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/smartphones/samsung-galaxy-s7/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/smartphones/samsung-galaxy-s7/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/smartphones/samsung-galaxy-s7/1.webp",
      "https://cdn.dummyjson.com/product-images/smartphones/samsung-galaxy-s7/2.webp"
    ],
    "description": "The Samsung Galaxy S7 is a flagship smartphone known for its sleek design and advanced features. It features a high-resolution display, powerful camera, and robust performance.",
    "features": [
      "The Samsung Galaxy S7 is a flagship smartphone known for its sleek design and advanced features",
      "6GB RAM with 128GB storage for seamless multitasking",
      "5000mAh battery with 45W fast charging",
      "6.4\" AMOLED display with 60Hz refresh rate",
      "Triple rear camera with 108MP primary sensor for professional-grade photography"
    ],
    "specs": {
      "Brand": "Samsung",
      "Display": "6.4\" AMOLED",
      "Processor": "Exynos 2200",
      "RAM": "6GB",
      "Storage": "128GB",
      "Battery": "5000mAh",
      "Rear Camera": "108MP + 10MP + 5MP",
      "Front Camera": "20MP",
      "OS": "Android 13",
      "SIM": "Dual SIM (Nano + Nano)",
      "Connectivity": "5G / Wi-Fi 6 / Bluetooth 5.2 / NFC",
      "Warranty": "3 months warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Samsung",
    "soldByRating": 4.5,
    "sellerSince": "2012",
    "fulfillment": "Seller fulfilled",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 94,
        "detail": null
      },
      "returnRate": {
        "score": 74,
        "detail": null
      },
      "warrantyClaims": {
        "score": 81,
        "detail": null
      },
      "sellerReliability": {
        "score": 89,
        "detail": null
      },
      "priceStability": {
        "score": 78,
        "detail": null
      }
    }
  },
  {
    "id": "dj132",
    "name": "Samsung Galaxy S8",
    "brand": "Samsung",
    "category": "Electronics > Mobiles",
    "price": 41499,
    "originalPrice": 51233,
    "discount": 19,
    "rating": 4.4,
    "reviewCount": 10591,
    "inStock": false,
    "isPrime": true,
    "delivery": "Get it by Day after Tomorrow",
    "deliveryFree": true,
    "trustScore": 96,
    "trustLabel": "Genuine",
    "buyNowOrWait": "wait",
    "waitReason": "Sale expected in the next 2 weeks based on historical patterns.",
    "spikePriceMonths": [
      9,
      10
    ],
    "priceHistory": [
      41499,
      42867,
      41499,
      41499,
      41499,
      41499,
      41499,
      41499,
      41499,
      59895,
      55816,
      41499
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/smartphones/samsung-galaxy-s8/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/smartphones/samsung-galaxy-s8/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/smartphones/samsung-galaxy-s8/1.webp",
      "https://cdn.dummyjson.com/product-images/smartphones/samsung-galaxy-s8/2.webp"
    ],
    "description": "The Samsung Galaxy S8 is a premium smartphone with an Infinity Display, offering a stunning visual experience. It boasts advanced camera capabilities and cutting-edge technology.",
    "features": [
      "The Samsung Galaxy S8 is a premium smartphone with an Infinity Display, offering a stunning visual experience",
      "4GB RAM with 128GB storage for seamless multitasking",
      "5000mAh battery with 45W fast charging",
      "6.5\" Super AMOLED display with 60Hz refresh rate",
      "Quad rear camera with 50MP primary sensor for professional-grade photography"
    ],
    "specs": {
      "Brand": "Samsung",
      "Display": "6.5\" Super AMOLED",
      "Processor": "MediaTek Dimensity 9000",
      "RAM": "4GB",
      "Storage": "256GB",
      "Battery": "5000mAh",
      "Rear Camera": "50MP + 10MP + 5MP",
      "Front Camera": "32MP",
      "OS": "Android 12",
      "SIM": "Single SIM",
      "Connectivity": "5G / Wi-Fi 6 / Bluetooth 5.2 / NFC",
      "Warranty": "2 year warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Samsung",
    "soldByRating": 4.7,
    "sellerSince": "2015",
    "fulfillment": "Fulfilled by Amazon",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 96,
        "detail": null
      },
      "returnRate": {
        "score": 96,
        "detail": null
      },
      "warrantyClaims": {
        "score": 96,
        "detail": null
      },
      "sellerReliability": {
        "score": 96,
        "detail": null
      },
      "priceStability": {
        "score": 96,
        "detail": null
      }
    }
  },
  {
    "id": "dj133",
    "name": "Samsung Galaxy S10",
    "brand": "Samsung",
    "category": "Electronics > Mobiles",
    "price": 58099,
    "originalPrice": 61807,
    "discount": 6,
    "rating": 3.1,
    "reviewCount": 1624,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Day after Tomorrow",
    "deliveryFree": true,
    "trustScore": 78,
    "trustLabel": "Genuine",
    "buyNowOrWait": "wait",
    "waitReason": "Sale expected in the next 2 weeks based on historical patterns.",
    "spikePriceMonths": [
      3
    ],
    "priceHistory": [
      58099,
      60683,
      58099,
      66699,
      58099,
      58099,
      58099,
      58099,
      58099,
      58099,
      53919,
      58099
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/smartphones/samsung-galaxy-s10/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/smartphones/samsung-galaxy-s10/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/smartphones/samsung-galaxy-s10/1.webp",
      "https://cdn.dummyjson.com/product-images/smartphones/samsung-galaxy-s10/2.webp"
    ],
    "description": "The Samsung Galaxy S10 is a flagship device featuring a dynamic AMOLED display, versatile camera system, and powerful performance. It represents innovation and excellence in smartphone technology.",
    "features": [
      "The Samsung Galaxy S10 is a flagship device featuring a dynamic AMOLED display, versatile camera system, and powerful performance",
      "6GB RAM with 256GB storage for seamless multitasking",
      "5000mAh battery with 25W fast charging",
      "6.5\" AMOLED display with 90Hz refresh rate",
      "Triple rear camera with 50MP primary sensor for professional-grade photography"
    ],
    "specs": {
      "Brand": "Samsung",
      "Display": "6.5\" AMOLED",
      "Processor": "Exynos 2200",
      "RAM": "6GB",
      "Storage": "64GB",
      "Battery": "5000mAh",
      "Rear Camera": "50MP + 5MP + 5MP",
      "Front Camera": "12MP",
      "OS": "Android 14",
      "SIM": "Dual SIM (Nano + Nano)",
      "Connectivity": "5G / Wi-Fi 6 / Bluetooth 5.2 / NFC",
      "Warranty": "No warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Samsung",
    "soldByRating": 4.2,
    "sellerSince": "2019",
    "fulfillment": "Fulfilled by Amazon",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 79,
        "detail": null
      },
      "returnRate": {
        "score": 69,
        "detail": null
      },
      "warrantyClaims": {
        "score": 81,
        "detail": null
      },
      "sellerReliability": {
        "score": 80,
        "detail": null
      },
      "priceStability": {
        "score": 74,
        "detail": null
      }
    }
  },
  {
    "id": "dj134",
    "name": "Vivo S1",
    "brand": "Vivo",
    "category": "Electronics > Mobiles",
    "price": 20749,
    "originalPrice": 23054,
    "discount": 10,
    "rating": 3.5,
    "reviewCount": 50698,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Tomorrow, 6 PM",
    "deliveryFree": true,
    "trustScore": 38,
    "trustLabel": "Suspicious",
    "buyNowOrWait": "wait",
    "waitReason": "Sale expected in the next 2 weeks based on historical patterns.",
    "spikePriceMonths": [
      0,
      1,
      2,
      6,
      9
    ],
    "priceHistory": [
      24066,
      26640,
      24495,
      20749,
      20749,
      20749,
      24795,
      20749,
      20749,
      23566,
      20749,
      20749
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/smartphones/vivo-s1/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/smartphones/vivo-s1/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/smartphones/vivo-s1/1.webp",
      "https://cdn.dummyjson.com/product-images/smartphones/vivo-s1/2.webp"
    ],
    "description": "The Vivo S1 is a stylish and mid-range smartphone offering a blend of design and performance. It features a vibrant display, capable camera system, and reliable functionality.",
    "features": [
      "The Vivo S1 is a stylish and mid-range smartphone offering a blend of design and performance",
      "8GB RAM with 64GB storage for seamless multitasking",
      "4000mAh battery with 25W fast charging",
      "6.1\" AMOLED display with 120Hz refresh rate",
      "Quad rear camera with 12MP primary sensor for professional-grade photography"
    ],
    "specs": {
      "Brand": "Vivo",
      "Display": "6.1\" AMOLED",
      "Processor": "Snapdragon 7s Gen 2",
      "RAM": "8GB",
      "Storage": "512GB",
      "Battery": "4000mAh",
      "Rear Camera": "12MP + 8MP + 8MP",
      "Front Camera": "20MP",
      "OS": "Android 12",
      "SIM": "Single SIM",
      "Connectivity": "5G / Wi-Fi 6 / Bluetooth 5.2 / NFC",
      "Warranty": "6 months warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Vivo",
    "soldByRating": 2,
    "sellerSince": "2022",
    "fulfillment": "Seller fulfilled",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 31,
        "detail": null
      },
      "returnRate": {
        "score": 48,
        "detail": null
      },
      "warrantyClaims": {
        "score": 37,
        "detail": null
      },
      "sellerReliability": {
        "score": 49,
        "detail": null
      },
      "priceStability": {
        "score": 39,
        "detail": null
      }
    }
  },
  {
    "id": "dj135",
    "name": "Vivo V9",
    "brand": "Vivo",
    "category": "Electronics > Mobiles",
    "price": 24899,
    "originalPrice": 30365,
    "discount": 18,
    "rating": 3.6,
    "reviewCount": 67159,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Tomorrow, 6 PM",
    "deliveryFree": true,
    "trustScore": 90,
    "trustLabel": "Genuine",
    "buyNowOrWait": "buy",
    "waitReason": null,
    "spikePriceMonths": [
      10
    ],
    "priceHistory": [
      24899,
      24899,
      24899,
      26545,
      26415,
      24899,
      24899,
      24899,
      24899,
      24899,
      31589,
      24899
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/smartphones/vivo-v9/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/smartphones/vivo-v9/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/smartphones/vivo-v9/1.webp",
      "https://cdn.dummyjson.com/product-images/smartphones/vivo-v9/2.webp"
    ],
    "description": "The Vivo V9 is a smartphone known for its sleek design and emphasis on capturing high-quality selfies. It features a notch display, dual-camera setup, and a modern design.",
    "features": [
      "The Vivo V9 is a smartphone known for its sleek design and emphasis on capturing high-quality selfies",
      "4GB RAM with 128GB storage for seamless multitasking",
      "5500mAh battery with 65W fast charging",
      "6.4\" IPS LCD display with 144Hz refresh rate",
      "Triple rear camera with 12MP primary sensor for professional-grade photography"
    ],
    "specs": {
      "Brand": "Vivo",
      "Display": "6.4\" IPS LCD",
      "Processor": "MediaTek Dimensity 9000",
      "RAM": "4GB",
      "Storage": "256GB",
      "Battery": "5500mAh",
      "Rear Camera": "12MP + 10MP + 8MP",
      "Front Camera": "16MP",
      "OS": "Android 12",
      "SIM": "Single SIM",
      "Connectivity": "5G / Wi-Fi 6 / Bluetooth 5.2 / NFC",
      "Warranty": "2 year warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Vivo",
    "soldByRating": 4.9,
    "sellerSince": "2010",
    "fulfillment": "Fulfilled by Amazon",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 96,
        "detail": null
      },
      "returnRate": {
        "score": 96,
        "detail": null
      },
      "warrantyClaims": {
        "score": 83,
        "detail": null
      },
      "sellerReliability": {
        "score": 96,
        "detail": null
      },
      "priceStability": {
        "score": 87,
        "detail": null
      }
    }
  },
  {
    "id": "dj136",
    "name": "Vivo X21",
    "brand": "Vivo",
    "category": "Electronics > Mobiles",
    "price": 41499,
    "originalPrice": 49999,
    "discount": 17,
    "rating": 4.3,
    "reviewCount": 67407,
    "inStock": true,
    "isPrime": false,
    "delivery": "Get it by Day after Tomorrow",
    "deliveryFree": true,
    "trustScore": 84,
    "trustLabel": "Genuine",
    "buyNowOrWait": "wait",
    "waitReason": "Sale expected in the next 2 weeks based on historical patterns.",
    "spikePriceMonths": [
      4,
      5
    ],
    "priceHistory": [
      41499,
      41499,
      41499,
      41499,
      57848,
      51237,
      41499,
      41499,
      41499,
      41499,
      43198,
      41499
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/smartphones/vivo-x21/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/smartphones/vivo-x21/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/smartphones/vivo-x21/1.webp",
      "https://cdn.dummyjson.com/product-images/smartphones/vivo-x21/2.webp"
    ],
    "description": "The Vivo X21 is a premium smartphone with a focus on cutting-edge technology. It features an in-display fingerprint sensor, a high-resolution display, and advanced camera capabilities.",
    "features": [
      "The Vivo X21 is a premium smartphone with a focus on cutting-edge technology",
      "8GB RAM with 256GB storage for seamless multitasking",
      "5500mAh battery with 33W fast charging",
      "6.7\" Super AMOLED display with 90Hz refresh rate",
      "Triple rear camera with 12MP primary sensor for professional-grade photography"
    ],
    "specs": {
      "Brand": "Vivo",
      "Display": "6.7\" Super AMOLED",
      "Processor": "Snapdragon 7s Gen 2",
      "RAM": "8GB",
      "Storage": "256GB",
      "Battery": "5500mAh",
      "Rear Camera": "12MP + 10MP + 2MP",
      "Front Camera": "20MP",
      "OS": "Android 12",
      "SIM": "Single SIM",
      "Connectivity": "5G / Wi-Fi 6 / Bluetooth 5.2 / NFC",
      "Warranty": "1 year warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Vivo",
    "soldByRating": 4.3,
    "sellerSince": "2011",
    "fulfillment": "Fulfilled by Amazon",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 94,
        "detail": null
      },
      "returnRate": {
        "score": 83,
        "detail": null
      },
      "warrantyClaims": {
        "score": 83,
        "detail": null
      },
      "sellerReliability": {
        "score": 96,
        "detail": null
      },
      "priceStability": {
        "score": 82,
        "detail": null
      }
    }
  },
  {
    "id": "dj137",
    "name": "American Football",
    "brand": "Generic",
    "category": "Sports",
    "price": 1659,
    "originalPrice": 1746,
    "discount": 5,
    "rating": 4.9,
    "reviewCount": 53199,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Tomorrow, 6 PM",
    "deliveryFree": true,
    "trustScore": 92,
    "trustLabel": "Genuine",
    "buyNowOrWait": "wait",
    "waitReason": "Sale expected in the next 2 weeks based on historical patterns.",
    "spikePriceMonths": [],
    "priceHistory": [
      1686,
      1659,
      1659,
      1700,
      1659,
      1542,
      1659,
      1659,
      1659,
      1659,
      1659,
      1659
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/sports-accessories/american-football/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/sports-accessories/american-football/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/sports-accessories/american-football/1.webp"
    ],
    "description": "The American Football is a classic ball used in American football games. It is designed for throwing and catching, making it an essential piece of equipment for the sport.",
    "features": [
      "The American Football is a classic ball used in American football games",
      "Constructed from reinforced polypropylene for performance under pressure",
      "Weighs only 2 kg — lightweight for unrestricted movement during intense workouts",
      "Anti-sweat grip — engineered for peak athletic performance",
      "Suitable for running, cycling, and cross-training"
    ],
    "specs": {
      "Material": "Reinforced Polypropylene",
      "Weight": "2 kg",
      "Dimensions": "7W × 6H × 22D cm",
      "Colour Options": "5 colours available",
      "Recommended Use": "Running & Cross-training",
      "Warranty": "6 months warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Third-party Seller",
    "soldByRating": 4.1,
    "sellerSince": "2019",
    "fulfillment": "Seller fulfilled",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 79,
        "detail": null
      },
      "returnRate": {
        "score": 88,
        "detail": null
      },
      "warrantyClaims": {
        "score": 96,
        "detail": null
      },
      "sellerReliability": {
        "score": 96,
        "detail": null
      },
      "priceStability": {
        "score": 84,
        "detail": null
      }
    }
  },
  {
    "id": "dj138",
    "name": "Baseball Ball",
    "brand": "Generic",
    "category": "Sports",
    "price": 746,
    "originalPrice": 761,
    "discount": 2,
    "rating": 2.6,
    "reviewCount": 56427,
    "inStock": true,
    "isPrime": false,
    "delivery": "Get it by Day after Tomorrow",
    "deliveryFree": true,
    "trustScore": 77,
    "trustLabel": "Genuine",
    "buyNowOrWait": "buy",
    "waitReason": null,
    "spikePriceMonths": [],
    "priceHistory": [
      746,
      746,
      673,
      670,
      759,
      746,
      746,
      746,
      746,
      746,
      683,
      746
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/sports-accessories/baseball-ball/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/sports-accessories/baseball-ball/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/sports-accessories/baseball-ball/1.webp"
    ],
    "description": "The Baseball Ball is a standard baseball used in baseball games. It features a durable leather cover and is designed for pitching, hitting, and fielding in the game of baseball.",
    "features": [
      "The Baseball Ball is a standard baseball used in baseball games",
      "Constructed from breathable mesh fabric for performance under pressure",
      "Weighs only 5 kg — lightweight for unrestricted movement during intense workouts",
      "Anti-sweat grip — engineered for peak athletic performance",
      "Suitable for running, cycling, and cross-training"
    ],
    "specs": {
      "Material": "Natural Rubber",
      "Weight": "5 kg",
      "Dimensions": "14W × 23H × 16D cm",
      "Colour Options": "4 colours available",
      "Recommended Use": "Gym & Home Workout",
      "Warranty": "6 months warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Third-party Seller",
    "soldByRating": 4.9,
    "sellerSince": "2010",
    "fulfillment": "Fulfilled by Amazon",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 74,
        "detail": null
      },
      "returnRate": {
        "score": 82,
        "detail": null
      },
      "warrantyClaims": {
        "score": 83,
        "detail": null
      },
      "sellerReliability": {
        "score": 86,
        "detail": null
      },
      "priceStability": {
        "score": 74,
        "detail": null
      }
    }
  },
  {
    "id": "dj139",
    "name": "Baseball Glove",
    "brand": "Generic",
    "category": "Sports",
    "price": 2074,
    "originalPrice": 2138,
    "discount": 3,
    "rating": 4,
    "reviewCount": 39855,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Tomorrow, 6 PM",
    "deliveryFree": true,
    "trustScore": 84,
    "trustLabel": "Genuine",
    "buyNowOrWait": "wait",
    "waitReason": "Sale expected in the next 2 weeks based on historical patterns.",
    "spikePriceMonths": [],
    "priceHistory": [
      1832,
      2007,
      2074,
      1893,
      2116,
      2074,
      2074,
      2075,
      2074,
      2074,
      2074,
      2074
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/sports-accessories/baseball-glove/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/sports-accessories/baseball-glove/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/sports-accessories/baseball-glove/1.webp",
      "https://cdn.dummyjson.com/product-images/sports-accessories/baseball-glove/2.webp"
    ],
    "description": "The Baseball Glove is a protective glove worn by baseball players. It is designed to catch and field the baseball, providing players with comfort and control during the game.",
    "features": [
      "The Baseball Glove is a protective glove worn by baseball players",
      "Constructed from high-tensile nylon for performance under pressure",
      "Weighs only 1 kg — lightweight for unrestricted movement during intense workouts",
      "Non-slip base — engineered for peak athletic performance",
      "Suitable for running, cycling, and cross-training"
    ],
    "specs": {
      "Material": "High-tensile Nylon",
      "Weight": "1 kg",
      "Dimensions": "24W × 11H × 6D cm",
      "Colour Options": "5 colours available",
      "Recommended Use": "Running & Cross-training",
      "Warranty": "Lifetime warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Third-party Seller",
    "soldByRating": 4.6,
    "sellerSince": "2010",
    "fulfillment": "Fulfilled by Amazon",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 92,
        "detail": null
      },
      "returnRate": {
        "score": 78,
        "detail": null
      },
      "warrantyClaims": {
        "score": 81,
        "detail": null
      },
      "sellerReliability": {
        "score": 96,
        "detail": null
      },
      "priceStability": {
        "score": 93,
        "detail": null
      }
    }
  },
  {
    "id": "dj140",
    "name": "Basketball",
    "brand": "Generic",
    "category": "Sports",
    "price": 1244,
    "originalPrice": 1338,
    "discount": 7,
    "rating": 4.7,
    "reviewCount": 56228,
    "inStock": true,
    "isPrime": false,
    "delivery": "Get it by Day after Tomorrow",
    "deliveryFree": true,
    "trustScore": 96,
    "trustLabel": "Genuine",
    "buyNowOrWait": "buy",
    "waitReason": null,
    "spikePriceMonths": [],
    "priceHistory": [
      1244,
      1244,
      1244,
      1244,
      1244,
      1244,
      1244,
      1244,
      1244,
      1244,
      1244,
      1244
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/sports-accessories/basketball/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/sports-accessories/basketball/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/sports-accessories/basketball/1.webp"
    ],
    "description": "The Basketball is a standard ball used in basketball games. It is designed for dribbling, shooting, and passing in the game of basketball, suitable for both indoor and outdoor play.",
    "features": [
      "The Basketball is a standard ball used in basketball games",
      "Constructed from carbon-composite for performance under pressure",
      "Weighs only 7 kg — lightweight for unrestricted movement during intense workouts",
      "Quick-dry surface — engineered for peak athletic performance",
      "Suitable for running, cycling, and cross-training"
    ],
    "specs": {
      "Material": "Carbon-Composite",
      "Weight": "7 kg",
      "Dimensions": "28W × 11H × 19D cm",
      "Colour Options": "4 colours available",
      "Recommended Use": "Gym & Home Workout",
      "Warranty": "1 year warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Third-party Seller",
    "soldByRating": 4.3,
    "sellerSince": "2014",
    "fulfillment": "Seller fulfilled",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 93,
        "detail": null
      },
      "returnRate": {
        "score": 96,
        "detail": null
      },
      "warrantyClaims": {
        "score": 96,
        "detail": null
      },
      "sellerReliability": {
        "score": 96,
        "detail": null
      },
      "priceStability": {
        "score": 96,
        "detail": null
      }
    }
  },
  {
    "id": "dj141",
    "name": "Basketball Rim",
    "brand": "Generic",
    "category": "Sports",
    "price": 3319,
    "originalPrice": 3608,
    "discount": 8,
    "rating": 4.6,
    "reviewCount": 42785,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Day after Tomorrow",
    "deliveryFree": true,
    "trustScore": 96,
    "trustLabel": "Genuine",
    "buyNowOrWait": "wait",
    "waitReason": "Sale expected in the next 2 weeks based on historical patterns.",
    "spikePriceMonths": [
      4,
      6
    ],
    "priceHistory": [
      3319,
      3319,
      3319,
      3703,
      4063,
      3319,
      3755,
      3319,
      3319,
      3348,
      3319,
      3319
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/sports-accessories/basketball-rim/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/sports-accessories/basketball-rim/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/sports-accessories/basketball-rim/1.webp"
    ],
    "description": "The Basketball Rim is a sturdy hoop and net assembly mounted on a basketball backboard. It provides a target for shooting and scoring in the game of basketball.",
    "features": [
      "The Basketball Rim is a sturdy hoop and net assembly mounted on a basketball backboard",
      "Constructed from natural rubber for performance under pressure",
      "Weighs only 1 kg — lightweight for unrestricted movement during intense workouts",
      "Non-slip base — engineered for peak athletic performance",
      "Suitable for running, cycling, and cross-training"
    ],
    "specs": {
      "Material": "Breathable Mesh Fabric",
      "Weight": "1 kg",
      "Dimensions": "16W × 21H × 7D cm",
      "Colour Options": "4 colours available",
      "Recommended Use": "Outdoor Sports & Fitness",
      "Warranty": "3 months warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Third-party Seller",
    "soldByRating": 4.9,
    "sellerSince": "2015",
    "fulfillment": "Fulfilled by Amazon",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 96,
        "detail": null
      },
      "returnRate": {
        "score": 96,
        "detail": null
      },
      "warrantyClaims": {
        "score": 96,
        "detail": null
      },
      "sellerReliability": {
        "score": 96,
        "detail": null
      },
      "priceStability": {
        "score": 96,
        "detail": null
      }
    }
  },
  {
    "id": "dj142",
    "name": "Cricket Ball",
    "brand": "Generic",
    "category": "Sports",
    "price": 1078,
    "originalPrice": 1185,
    "discount": 9,
    "rating": 3.5,
    "reviewCount": 15873,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Day after Tomorrow",
    "deliveryFree": true,
    "trustScore": 84,
    "trustLabel": "Genuine",
    "buyNowOrWait": "wait",
    "waitReason": "Sale expected in the next 2 weeks based on historical patterns.",
    "spikePriceMonths": [
      6,
      10
    ],
    "priceHistory": [
      1078,
      1078,
      1078,
      1078,
      1078,
      1078,
      1287,
      1078,
      1078,
      1078,
      1293,
      1078
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/sports-accessories/cricket-ball/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/sports-accessories/cricket-ball/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/sports-accessories/cricket-ball/1.webp"
    ],
    "description": "The Cricket Ball is a hard leather ball used in the sport of cricket. It is bowled and batted in the game, and its hardness and seam contribute to the dynamics of cricket play.",
    "features": [
      "The Cricket Ball is a hard leather ball used in the sport of cricket",
      "Constructed from breathable mesh fabric for performance under pressure",
      "Weighs only 7 kg — lightweight for unrestricted movement during intense workouts",
      "Moisture-wicking fabric — engineered for peak athletic performance",
      "Suitable for gym, yoga, and home workouts"
    ],
    "specs": {
      "Material": "Natural Rubber",
      "Weight": "7 kg",
      "Dimensions": "29W × 21H × 18D cm",
      "Colour Options": "7 colours available",
      "Recommended Use": "Yoga & Pilates",
      "Warranty": "3 year warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Third-party Seller",
    "soldByRating": 4,
    "sellerSince": "2014",
    "fulfillment": "Seller fulfilled",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 76,
        "detail": null
      },
      "returnRate": {
        "score": 81,
        "detail": null
      },
      "warrantyClaims": {
        "score": 90,
        "detail": null
      },
      "sellerReliability": {
        "score": 96,
        "detail": null
      },
      "priceStability": {
        "score": 87,
        "detail": null
      }
    }
  },
  {
    "id": "dj143",
    "name": "Cricket Bat",
    "brand": "Generic",
    "category": "Sports",
    "price": 2489,
    "originalPrice": 2566,
    "discount": 3,
    "rating": 3.2,
    "reviewCount": 26199,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Day after Tomorrow",
    "deliveryFree": true,
    "trustScore": 81,
    "trustLabel": "Genuine",
    "buyNowOrWait": "wait",
    "waitReason": "Sale expected in the next 2 weeks based on historical patterns.",
    "spikePriceMonths": [],
    "priceHistory": [
      2489,
      2489,
      2489,
      2489,
      2489,
      2489,
      2512,
      2489,
      2489,
      2489,
      2489,
      2489
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/sports-accessories/cricket-bat/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/sports-accessories/cricket-bat/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/sports-accessories/cricket-bat/1.webp"
    ],
    "description": "The Cricket Bat is an essential piece of cricket equipment used by batsmen to hit the cricket ball. It is made of wood and comes in various sizes and designs.",
    "features": [
      "The Cricket Bat is an essential piece of cricket equipment used by batsmen to hit the cricket ball",
      "Constructed from breathable mesh fabric for performance under pressure",
      "Weighs only 10 kg — lightweight for unrestricted movement during intense workouts",
      "Quick-dry surface — engineered for peak athletic performance",
      "Suitable for outdoor sports and fitness"
    ],
    "specs": {
      "Material": "Natural Rubber",
      "Weight": "10 kg",
      "Dimensions": "27W × 24H × 17D cm",
      "Colour Options": "3 colours available",
      "Recommended Use": "Yoga & Pilates",
      "Warranty": "1 year warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Third-party Seller",
    "soldByRating": 4.9,
    "sellerSince": "2011",
    "fulfillment": "Fulfilled by Amazon",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 70,
        "detail": null
      },
      "returnRate": {
        "score": 72,
        "detail": null
      },
      "warrantyClaims": {
        "score": 89,
        "detail": null
      },
      "sellerReliability": {
        "score": 84,
        "detail": null
      },
      "priceStability": {
        "score": 82,
        "detail": null
      }
    }
  },
  {
    "id": "dj144",
    "name": "Cricket Helmet",
    "brand": "Generic",
    "category": "Sports",
    "price": 3734,
    "originalPrice": 4149,
    "discount": 10,
    "rating": 4.7,
    "reviewCount": 47881,
    "inStock": true,
    "isPrime": false,
    "delivery": "Get it by Day after Tomorrow",
    "deliveryFree": true,
    "trustScore": 90,
    "trustLabel": "Genuine",
    "buyNowOrWait": "buy",
    "waitReason": null,
    "spikePriceMonths": [
      2,
      3,
      4
    ],
    "priceHistory": [
      3734,
      3734,
      4734,
      4263,
      4509,
      3734,
      3734,
      3565,
      3734,
      3734,
      3734,
      3734
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/sports-accessories/cricket-helmet/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/sports-accessories/cricket-helmet/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/sports-accessories/cricket-helmet/1.webp",
      "https://cdn.dummyjson.com/product-images/sports-accessories/cricket-helmet/2.webp"
    ],
    "description": "The Cricket Helmet is a protective headgear worn by cricket players, especially batsmen and wicketkeepers. It provides protection against fast bowling and bouncers.",
    "features": [
      "The Cricket Helmet is a protective headgear worn by cricket players, especially batsmen and wicketkeepers",
      "Constructed from reinforced polypropylene for performance under pressure",
      "Weighs only 10 kg — lightweight for unrestricted movement during intense workouts",
      "Moisture-wicking fabric — engineered for peak athletic performance",
      "Suitable for running, cycling, and cross-training"
    ],
    "specs": {
      "Material": "Reinforced Polypropylene",
      "Weight": "10 kg",
      "Dimensions": "6W × 6H × 22D cm",
      "Colour Options": "6 colours available",
      "Recommended Use": "Gym & Home Workout",
      "Warranty": "Lifetime warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Third-party Seller",
    "soldByRating": 4.9,
    "sellerSince": "2020",
    "fulfillment": "Fulfilled by Amazon",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 92,
        "detail": null
      },
      "returnRate": {
        "score": 85,
        "detail": null
      },
      "warrantyClaims": {
        "score": 90,
        "detail": null
      },
      "sellerReliability": {
        "score": 96,
        "detail": null
      },
      "priceStability": {
        "score": 86,
        "detail": null
      }
    }
  },
  {
    "id": "dj145",
    "name": "Cricket Wicket",
    "brand": "Generic",
    "category": "Sports",
    "price": 2489,
    "originalPrice": 2999,
    "discount": 17,
    "rating": 4.7,
    "reviewCount": 24627,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Tomorrow, 6 PM",
    "deliveryFree": true,
    "trustScore": 96,
    "trustLabel": "Genuine",
    "buyNowOrWait": "buy",
    "waitReason": null,
    "spikePriceMonths": [
      3
    ],
    "priceHistory": [
      2489,
      2489,
      2489,
      2972,
      2489,
      2489,
      2489,
      2489,
      2489,
      2596,
      2489,
      2489
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/sports-accessories/cricket-wicket/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/sports-accessories/cricket-wicket/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/sports-accessories/cricket-wicket/1.webp"
    ],
    "description": "The Cricket Wicket is a set of three stumps and two bails, forming a wicket used in the sport of cricket. Batsmen aim to protect the wicket while scoring runs.",
    "features": [
      "The Cricket Wicket is a set of three stumps and two bails, forming a wicket used in the sport of cricket",
      "Constructed from natural rubber for performance under pressure",
      "Weighs only 5 kg — lightweight for unrestricted movement during intense workouts",
      "Non-slip base — engineered for peak athletic performance",
      "Suitable for outdoor sports and fitness"
    ],
    "specs": {
      "Material": "Breathable Mesh Fabric",
      "Weight": "5 kg",
      "Dimensions": "16W × 15H × 23D cm",
      "Colour Options": "4 colours available",
      "Recommended Use": "Outdoor Sports & Fitness",
      "Warranty": "3 months warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Third-party Seller",
    "soldByRating": 4.4,
    "sellerSince": "2012",
    "fulfillment": "Seller fulfilled",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 90,
        "detail": null
      },
      "returnRate": {
        "score": 85,
        "detail": null
      },
      "warrantyClaims": {
        "score": 96,
        "detail": null
      },
      "sellerReliability": {
        "score": 96,
        "detail": null
      },
      "priceStability": {
        "score": 96,
        "detail": null
      }
    }
  },
  {
    "id": "dj146",
    "name": "Feather Shuttlecock",
    "brand": "Generic",
    "category": "Sports",
    "price": 497,
    "originalPrice": 512,
    "discount": 3,
    "rating": 2.9,
    "reviewCount": 34204,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Tomorrow, 6 PM",
    "deliveryFree": true,
    "trustScore": 39,
    "trustLabel": "Suspicious",
    "buyNowOrWait": "wait",
    "waitReason": "Sale expected in the next 2 weeks based on historical patterns.",
    "spikePriceMonths": [
      2
    ],
    "priceHistory": [
      497,
      497,
      597,
      497,
      432,
      485,
      497,
      497,
      497,
      497,
      497,
      497
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/sports-accessories/feather-shuttlecock/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/sports-accessories/feather-shuttlecock/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/sports-accessories/feather-shuttlecock/1.webp"
    ],
    "description": "The Feather Shuttlecock is used in the sport of badminton. It features natural feathers and is designed for high-speed play, providing stability and accuracy during matches.",
    "features": [
      "The Feather Shuttlecock is used in the sport of badminton",
      "Constructed from carbon-composite for performance under pressure",
      "Weighs only 4 kg — lightweight for unrestricted movement during intense workouts",
      "Moisture-wicking fabric — engineered for peak athletic performance",
      "Suitable for outdoor sports and fitness"
    ],
    "specs": {
      "Material": "Carbon-Composite",
      "Weight": "4 kg",
      "Dimensions": "20W × 24H × 19D cm",
      "Colour Options": "3 colours available",
      "Recommended Use": "Outdoor Sports & Fitness",
      "Warranty": "3 year warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Third-party Seller",
    "soldByRating": 2.4,
    "sellerSince": "2021",
    "fulfillment": "Seller fulfilled",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 28,
        "detail": null
      },
      "returnRate": {
        "score": 28,
        "detail": null
      },
      "warrantyClaims": {
        "score": 42,
        "detail": null
      },
      "sellerReliability": {
        "score": 52,
        "detail": null
      },
      "priceStability": {
        "score": 34,
        "detail": null
      }
    }
  },
  {
    "id": "dj147",
    "name": "Football",
    "brand": "Generic",
    "category": "Sports",
    "price": 1493,
    "originalPrice": 1572,
    "discount": 5,
    "rating": 3.3,
    "reviewCount": 7804,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Tomorrow, 6 PM",
    "deliveryFree": true,
    "trustScore": 87,
    "trustLabel": "Genuine",
    "buyNowOrWait": "wait",
    "waitReason": "Sale expected in the next 2 weeks based on historical patterns.",
    "spikePriceMonths": [],
    "priceHistory": [
      1493,
      1493,
      1493,
      1493,
      1493,
      1493,
      1493,
      1493,
      1493,
      1493,
      1493,
      1493
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/sports-accessories/football/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/sports-accessories/football/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/sports-accessories/football/1.webp"
    ],
    "description": "The Football, also known as a soccer ball, is the standard ball used in the sport of football (soccer). It is designed for kicking and passing in the game.",
    "features": [
      "The Football, also known as a soccer ball, is the standard ball used in the sport of football (soccer)",
      "Constructed from natural rubber for performance under pressure",
      "Weighs only 1 kg — lightweight for unrestricted movement during intense workouts",
      "Anti-sweat grip — engineered for peak athletic performance",
      "Suitable for gym, yoga, and home workouts"
    ],
    "specs": {
      "Material": "Breathable Mesh Fabric",
      "Weight": "1 kg",
      "Dimensions": "29W × 23H × 12D cm",
      "Colour Options": "8 colours available",
      "Recommended Use": "Outdoor Sports & Fitness",
      "Warranty": "Lifetime warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Third-party Seller",
    "soldByRating": 4.9,
    "sellerSince": "2010",
    "fulfillment": "Fulfilled by Amazon",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 84,
        "detail": null
      },
      "returnRate": {
        "score": 85,
        "detail": null
      },
      "warrantyClaims": {
        "score": 78,
        "detail": null
      },
      "sellerReliability": {
        "score": 96,
        "detail": null
      },
      "priceStability": {
        "score": 96,
        "detail": null
      }
    }
  },
  {
    "id": "dj148",
    "name": "Golf Ball",
    "brand": "Generic",
    "category": "Sports",
    "price": 829,
    "originalPrice": 999,
    "discount": 17,
    "rating": 4.3,
    "reviewCount": 48116,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Day after Tomorrow",
    "deliveryFree": true,
    "trustScore": 93,
    "trustLabel": "Genuine",
    "buyNowOrWait": "buy",
    "waitReason": null,
    "spikePriceMonths": [
      1,
      2
    ],
    "priceHistory": [
      829,
      1015,
      1030,
      829,
      829,
      829,
      829,
      829,
      829,
      829,
      829,
      829
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/sports-accessories/golf-ball/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/sports-accessories/golf-ball/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/sports-accessories/golf-ball/1.webp"
    ],
    "description": "The Golf Ball is a small ball used in the sport of golf. It features dimples on its surface, providing aerodynamic lift and distance when struck by a golf club.",
    "features": [
      "The Golf Ball is a small ball used in the sport of golf",
      "Constructed from carbon-composite for performance under pressure",
      "Weighs only 3 kg — lightweight for unrestricted movement during intense workouts",
      "Anti-sweat grip — engineered for peak athletic performance",
      "Suitable for running, cycling, and cross-training"
    ],
    "specs": {
      "Material": "Carbon-Composite",
      "Weight": "3 kg",
      "Dimensions": "25W × 23H × 12D cm",
      "Colour Options": "6 colours available",
      "Recommended Use": "Yoga & Pilates",
      "Warranty": "1 year warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Third-party Seller",
    "soldByRating": 3.9,
    "sellerSince": "2020",
    "fulfillment": "Seller fulfilled",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 96,
        "detail": null
      },
      "returnRate": {
        "score": 93,
        "detail": null
      },
      "warrantyClaims": {
        "score": 96,
        "detail": null
      },
      "sellerReliability": {
        "score": 96,
        "detail": null
      },
      "priceStability": {
        "score": 96,
        "detail": null
      }
    }
  },
  {
    "id": "dj149",
    "name": "Iron Golf",
    "brand": "Generic",
    "category": "Sports",
    "price": 4149,
    "originalPrice": 4461,
    "discount": 7,
    "rating": 4.4,
    "reviewCount": 66152,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Day after Tomorrow",
    "deliveryFree": true,
    "trustScore": 93,
    "trustLabel": "Genuine",
    "buyNowOrWait": "buy",
    "waitReason": null,
    "spikePriceMonths": [
      7
    ],
    "priceHistory": [
      4149,
      4149,
      4149,
      4149,
      4149,
      4149,
      4149,
      4990,
      4149,
      4559,
      4149,
      4149
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/sports-accessories/iron-golf/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/sports-accessories/iron-golf/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/sports-accessories/iron-golf/1.webp"
    ],
    "description": "The Iron Golf is a type of golf club designed for various golf shots. It features a solid metal head and is used for approach shots, chipping, and other golfing techniques.",
    "features": [
      "The Iron Golf is a type of golf club designed for various golf shots",
      "Constructed from carbon-composite for performance under pressure",
      "Weighs only 2 kg — lightweight for unrestricted movement during intense workouts",
      "Moisture-wicking fabric — engineered for peak athletic performance",
      "Suitable for team sports and recreational play"
    ],
    "specs": {
      "Material": "Carbon-Composite",
      "Weight": "2 kg",
      "Dimensions": "10W × 20H × 6D cm",
      "Colour Options": "7 colours available",
      "Recommended Use": "Outdoor Sports & Fitness",
      "Warranty": "3 months warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Third-party Seller",
    "soldByRating": 4,
    "sellerSince": "2018",
    "fulfillment": "Fulfilled by Amazon",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 96,
        "detail": null
      },
      "returnRate": {
        "score": 91,
        "detail": null
      },
      "warrantyClaims": {
        "score": 96,
        "detail": null
      },
      "sellerReliability": {
        "score": 96,
        "detail": null
      },
      "priceStability": {
        "score": 96,
        "detail": null
      }
    }
  },
  {
    "id": "dj150",
    "name": "Metal Baseball Bat",
    "brand": "Generic",
    "category": "Sports",
    "price": 2489,
    "originalPrice": 3111,
    "discount": 20,
    "rating": 4.7,
    "reviewCount": 68718,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Tomorrow, 6 PM",
    "deliveryFree": true,
    "trustScore": 51,
    "trustLabel": "Mixed",
    "buyNowOrWait": "wait",
    "waitReason": "Sale expected in the next 2 weeks based on historical patterns.",
    "spikePriceMonths": [
      2
    ],
    "priceHistory": [
      2489,
      2489,
      3094,
      2489,
      2489,
      2624,
      2489,
      2489,
      2489,
      2489,
      2489,
      2489
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/sports-accessories/metal-baseball-bat/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/sports-accessories/metal-baseball-bat/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/sports-accessories/metal-baseball-bat/1.webp"
    ],
    "description": "The Metal Baseball Bat is a durable and lightweight baseball bat made from metal alloys. It is commonly used in baseball games for hitting and batting practice.",
    "features": [
      "The Metal Baseball Bat is a durable and lightweight baseball bat made from metal alloys",
      "Constructed from natural rubber for performance under pressure",
      "Weighs only 7 kg — lightweight for unrestricted movement during intense workouts",
      "Non-slip base — engineered for peak athletic performance",
      "Suitable for team sports and recreational play"
    ],
    "specs": {
      "Material": "Breathable Mesh Fabric",
      "Weight": "7 kg",
      "Dimensions": "28W × 14H × 7D cm",
      "Colour Options": "7 colours available",
      "Recommended Use": "Yoga & Pilates",
      "Warranty": "3 year warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Third-party Seller",
    "soldByRating": 2.2,
    "sellerSince": "2021",
    "fulfillment": "Seller fulfilled",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 40,
        "detail": null
      },
      "returnRate": {
        "score": 45,
        "detail": null
      },
      "warrantyClaims": {
        "score": 46,
        "detail": null
      },
      "sellerReliability": {
        "score": 65,
        "detail": null
      },
      "priceStability": {
        "score": 59,
        "detail": null
      }
    }
  },
  {
    "id": "dj151",
    "name": "Tennis Ball",
    "brand": "Generic",
    "category": "Sports",
    "price": 580,
    "originalPrice": 659,
    "discount": 12,
    "rating": 4.1,
    "reviewCount": 73628,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Day after Tomorrow",
    "deliveryFree": true,
    "trustScore": 85,
    "trustLabel": "Genuine",
    "buyNowOrWait": "buy",
    "waitReason": null,
    "spikePriceMonths": [],
    "priceHistory": [
      580,
      580,
      580,
      580,
      580,
      580,
      580,
      580,
      580,
      580,
      580,
      580
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/sports-accessories/tennis-ball/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/sports-accessories/tennis-ball/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/sports-accessories/tennis-ball/1.webp"
    ],
    "description": "The Tennis Ball is a standard ball used in the sport of tennis. It is designed for bouncing and hitting with tennis rackets during matches or practice sessions.",
    "features": [
      "The Tennis Ball is a standard ball used in the sport of tennis",
      "Constructed from reinforced polypropylene for performance under pressure",
      "Weighs only 3 kg — lightweight for unrestricted movement during intense workouts",
      "Moisture-wicking fabric — engineered for peak athletic performance",
      "Suitable for team sports and recreational play"
    ],
    "specs": {
      "Material": "Reinforced Polypropylene",
      "Weight": "3 kg",
      "Dimensions": "8W × 20H × 12D cm",
      "Colour Options": "6 colours available",
      "Recommended Use": "Yoga & Pilates",
      "Warranty": "5 year warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Third-party Seller",
    "soldByRating": 4.4,
    "sellerSince": "2015",
    "fulfillment": "Fulfilled by Amazon",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 81,
        "detail": null
      },
      "returnRate": {
        "score": 96,
        "detail": null
      },
      "warrantyClaims": {
        "score": 79,
        "detail": null
      },
      "sellerReliability": {
        "score": 96,
        "detail": null
      },
      "priceStability": {
        "score": 88,
        "detail": null
      }
    }
  },
  {
    "id": "dj152",
    "name": "Tennis Racket",
    "brand": "Generic",
    "category": "Sports",
    "price": 4149,
    "originalPrice": 5186,
    "discount": 20,
    "rating": 4,
    "reviewCount": 28377,
    "inStock": true,
    "isPrime": false,
    "delivery": "Get it by Tomorrow, 6 PM",
    "deliveryFree": true,
    "trustScore": 91,
    "trustLabel": "Genuine",
    "buyNowOrWait": "buy",
    "waitReason": null,
    "spikePriceMonths": [
      1,
      4,
      5,
      8
    ],
    "priceHistory": [
      4149,
      5703,
      4149,
      4149,
      4732,
      5232,
      4149,
      4149,
      5766,
      4149,
      4149,
      4149
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/sports-accessories/tennis-racket/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/sports-accessories/tennis-racket/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/sports-accessories/tennis-racket/1.webp"
    ],
    "description": "The Tennis Racket is an essential piece of equipment used in the sport of tennis. It features a frame with strings and a grip, allowing players to hit the tennis ball.",
    "features": [
      "The Tennis Racket is an essential piece of equipment used in the sport of tennis",
      "Constructed from carbon-composite for performance under pressure",
      "Weighs only 7 kg — lightweight for unrestricted movement during intense workouts",
      "Moisture-wicking fabric — engineered for peak athletic performance",
      "Suitable for outdoor sports and fitness"
    ],
    "specs": {
      "Material": "Carbon-Composite",
      "Weight": "7 kg",
      "Dimensions": "18W × 16H × 19D cm",
      "Colour Options": "5 colours available",
      "Recommended Use": "Outdoor Sports & Fitness",
      "Warranty": "5 year warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Third-party Seller",
    "soldByRating": 4.4,
    "sellerSince": "2021",
    "fulfillment": "Fulfilled by Amazon",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 80,
        "detail": null
      },
      "returnRate": {
        "score": 85,
        "detail": null
      },
      "warrantyClaims": {
        "score": 91,
        "detail": null
      },
      "sellerReliability": {
        "score": 96,
        "detail": null
      },
      "priceStability": {
        "score": 84,
        "detail": null
      }
    }
  },
  {
    "id": "dj153",
    "name": "Volleyball",
    "brand": "Generic",
    "category": "Sports",
    "price": 995,
    "originalPrice": 1131,
    "discount": 12,
    "rating": 3.8,
    "reviewCount": 26203,
    "inStock": false,
    "isPrime": true,
    "delivery": "Get it by Day after Tomorrow",
    "deliveryFree": true,
    "trustScore": 81,
    "trustLabel": "Genuine",
    "buyNowOrWait": "buy",
    "waitReason": null,
    "spikePriceMonths": [
      6,
      7
    ],
    "priceHistory": [
      995,
      995,
      995,
      995,
      1065,
      995,
      1203,
      1160,
      995,
      995,
      995,
      995
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/sports-accessories/volleyball/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/sports-accessories/volleyball/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/sports-accessories/volleyball/1.webp"
    ],
    "description": "The Volleyball is a standard ball used in the sport of volleyball. It is designed for passing, setting, and spiking over the net during volleyball matches.",
    "features": [
      "The Volleyball is a standard ball used in the sport of volleyball",
      "Constructed from high-tensile nylon for performance under pressure",
      "Weighs only 3 kg — lightweight for unrestricted movement during intense workouts",
      "Non-slip base — engineered for peak athletic performance",
      "Suitable for outdoor sports and fitness"
    ],
    "specs": {
      "Material": "High-tensile Nylon",
      "Weight": "3 kg",
      "Dimensions": "17W × 20H × 10D cm",
      "Colour Options": "4 colours available",
      "Recommended Use": "Running & Cross-training",
      "Warranty": "1 week warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Third-party Seller",
    "soldByRating": 4.9,
    "sellerSince": "2021",
    "fulfillment": "Seller fulfilled",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 88,
        "detail": null
      },
      "returnRate": {
        "score": 83,
        "detail": null
      },
      "warrantyClaims": {
        "score": 77,
        "detail": null
      },
      "sellerReliability": {
        "score": 96,
        "detail": null
      },
      "priceStability": {
        "score": 70,
        "detail": null
      }
    }
  },
  {
    "id": "dj154",
    "name": "Black Sun Glasses",
    "brand": "Fashion Shades",
    "category": "Fashion",
    "price": 2489,
    "originalPrice": 2620,
    "discount": 5,
    "rating": 4.4,
    "reviewCount": 66178,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Day after Tomorrow",
    "deliveryFree": true,
    "trustScore": 86,
    "trustLabel": "Genuine",
    "buyNowOrWait": "wait",
    "waitReason": "Sale expected in the next 2 weeks based on historical patterns.",
    "spikePriceMonths": [],
    "priceHistory": [
      2489,
      2489,
      2489,
      2489,
      2715,
      2489,
      2642,
      2489,
      2489,
      2489,
      2489,
      2489
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/sunglasses/black-sun-glasses/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/sunglasses/black-sun-glasses/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/sunglasses/black-sun-glasses/1.webp",
      "https://cdn.dummyjson.com/product-images/sunglasses/black-sun-glasses/2.webp"
    ],
    "description": "The Black Sun Glasses are a classic and stylish choice, featuring a sleek black frame and tinted lenses. They provide both UV protection and a fashionable look.",
    "features": [
      "The Black Sun Glasses are a classic and stylish choice, featuring a sleek black frame and tinted lenses",
      "UV400 protection — shields eyes from harmful sun exposure",
      "Frame material: eco-friendly recycled nylon",
      "Blue-light filter lens — clear, distortion-free vision",
      "Spring-hinge temples for comfortable all-day wear"
    ],
    "specs": {
      "Frame Material": "TR90 Lightweight Plastic",
      "Lens Material": "TAC Polarised",
      "UV Protection": "Photochromic",
      "Lens Width": "59mm",
      "Bridge Width": "18mm",
      "Temple Length": "140mm",
      "Face Shape": "Oval",
      "Warranty": "6 Months against manufacturing defects"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Fashion Shades",
    "soldByRating": 3.9,
    "sellerSince": "2012",
    "fulfillment": "Seller fulfilled",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 87,
        "detail": null
      },
      "returnRate": {
        "score": 90,
        "detail": null
      },
      "warrantyClaims": {
        "score": 85,
        "detail": null
      },
      "sellerReliability": {
        "score": 96,
        "detail": null
      },
      "priceStability": {
        "score": 93,
        "detail": null
      }
    }
  },
  {
    "id": "dj155",
    "name": "Classic Sun Glasses",
    "brand": "Fashion Shades",
    "category": "Fashion",
    "price": 2074,
    "originalPrice": 2183,
    "discount": 5,
    "rating": 3.9,
    "reviewCount": 38512,
    "inStock": true,
    "isPrime": false,
    "delivery": "Get it by Day after Tomorrow",
    "deliveryFree": true,
    "trustScore": 87,
    "trustLabel": "Genuine",
    "buyNowOrWait": "buy",
    "waitReason": null,
    "spikePriceMonths": [],
    "priceHistory": [
      2074,
      1965,
      2074,
      2074,
      2074,
      2074,
      2074,
      2074,
      2074,
      2074,
      2074,
      2074
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/sunglasses/classic-sun-glasses/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/sunglasses/classic-sun-glasses/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/sunglasses/classic-sun-glasses/1.webp",
      "https://cdn.dummyjson.com/product-images/sunglasses/classic-sun-glasses/2.webp"
    ],
    "description": "The Classic Sun Glasses offer a timeless design with a neutral frame and UV-protected lenses. These sunglasses are versatile and suitable for various occasions.",
    "features": [
      "The Classic Sun Glasses offer a timeless design with a neutral frame and UV-protected lenses",
      "100% UVA/UVB protection — shields eyes from harmful sun exposure",
      "Frame material: titanium alloy",
      "Anti-glare lens — clear, distortion-free vision",
      "Fits most face shapes for comfortable all-day wear"
    ],
    "specs": {
      "Frame Material": "Stainless Steel",
      "Lens Material": "Glass",
      "UV Protection": "Polarised UV400",
      "Lens Width": "51mm",
      "Bridge Width": "17mm",
      "Temple Length": "143mm",
      "Face Shape": "Round",
      "Warranty": "6 Months against manufacturing defects"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Fashion Shades",
    "soldByRating": 4.4,
    "sellerSince": "2020",
    "fulfillment": "Fulfilled by Amazon",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 83,
        "detail": null
      },
      "returnRate": {
        "score": 88,
        "detail": null
      },
      "warrantyClaims": {
        "score": 93,
        "detail": null
      },
      "sellerReliability": {
        "score": 94,
        "detail": null
      },
      "priceStability": {
        "score": 89,
        "detail": null
      }
    }
  },
  {
    "id": "dj156",
    "name": "Green and Black Glasses",
    "brand": "Fashion Shades",
    "category": "Fashion",
    "price": 2904,
    "originalPrice": 2933,
    "discount": 1,
    "rating": 4.6,
    "reviewCount": 14917,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Tomorrow, 6 PM",
    "deliveryFree": true,
    "trustScore": 92,
    "trustLabel": "Genuine",
    "buyNowOrWait": "buy",
    "waitReason": null,
    "spikePriceMonths": [],
    "priceHistory": [
      2904,
      2904,
      2904,
      2904,
      2904,
      2904,
      2904,
      2904,
      2904,
      2904,
      3076,
      2904
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/sunglasses/green-and-black-glasses/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/sunglasses/green-and-black-glasses/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/sunglasses/green-and-black-glasses/1.webp",
      "https://cdn.dummyjson.com/product-images/sunglasses/green-and-black-glasses/2.webp"
    ],
    "description": "The Green and Black Glasses feature a bold combination of green and black colors, adding a touch of vibrancy to your eyewear collection. They are both stylish and eye-catching.",
    "features": [
      "The Green and Black Glasses feature a bold combination of green and black colors, adding a touch of vibrancy to your eyewear collection",
      "Polarised UV400 protection — shields eyes from harmful sun exposure",
      "Frame material: premium acetate",
      "Impact-resistant lens — clear, distortion-free vision",
      "Adjustable nose pads for comfortable all-day wear"
    ],
    "specs": {
      "Frame Material": "Stainless Steel",
      "Lens Material": "CR-39 Optical",
      "UV Protection": "UV400",
      "Lens Width": "55mm",
      "Bridge Width": "20mm",
      "Temple Length": "141mm",
      "Face Shape": "Oval",
      "Warranty": "6 Months against manufacturing defects"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Fashion Shades",
    "soldByRating": 4,
    "sellerSince": "2012",
    "fulfillment": "Seller fulfilled",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 84,
        "detail": null
      },
      "returnRate": {
        "score": 91,
        "detail": null
      },
      "warrantyClaims": {
        "score": 84,
        "detail": null
      },
      "sellerReliability": {
        "score": 96,
        "detail": null
      },
      "priceStability": {
        "score": 94,
        "detail": null
      }
    }
  },
  {
    "id": "dj157",
    "name": "Party Glasses",
    "brand": "Fashion Fun",
    "category": "Fashion",
    "price": 1659,
    "originalPrice": 1864,
    "discount": 11,
    "rating": 2.8,
    "reviewCount": 26161,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Day after Tomorrow",
    "deliveryFree": true,
    "trustScore": 79,
    "trustLabel": "Genuine",
    "buyNowOrWait": "wait",
    "waitReason": "Sale expected in the next 2 weeks based on historical patterns.",
    "spikePriceMonths": [],
    "priceHistory": [
      1659,
      1683,
      1659,
      1659,
      1659,
      1659,
      1659,
      1659,
      1659,
      1659,
      1659,
      1659
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/sunglasses/party-glasses/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/sunglasses/party-glasses/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/sunglasses/party-glasses/1.webp",
      "https://cdn.dummyjson.com/product-images/sunglasses/party-glasses/2.webp"
    ],
    "description": "The Party Glasses are designed to add flair to your party outfit. With unique shapes or colorful frames, they're perfect for adding a playful touch to your look during celebrations.",
    "features": [
      "The Party Glasses are designed to add flair to your party outfit",
      "100% UVA/UVB protection — shields eyes from harmful sun exposure",
      "Frame material: stainless steel",
      "Scratch-resistant lens — clear, distortion-free vision",
      "Spring-hinge temples for comfortable all-day wear"
    ],
    "specs": {
      "Frame Material": "Stainless Steel",
      "Lens Material": "Glass",
      "UV Protection": "100% UVA/UVB",
      "Lens Width": "57mm",
      "Bridge Width": "16mm",
      "Temple Length": "135mm",
      "Face Shape": "Heart",
      "Warranty": "6 Months against manufacturing defects"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Fashion Fun",
    "soldByRating": 4.5,
    "sellerSince": "2015",
    "fulfillment": "Fulfilled by Amazon",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 74,
        "detail": null
      },
      "returnRate": {
        "score": 83,
        "detail": null
      },
      "warrantyClaims": {
        "score": 82,
        "detail": null
      },
      "sellerReliability": {
        "score": 90,
        "detail": null
      },
      "priceStability": {
        "score": 90,
        "detail": null
      }
    }
  },
  {
    "id": "dj158",
    "name": "Sunglasses",
    "brand": "Fashion Shades",
    "category": "Fashion",
    "price": 1908,
    "originalPrice": 1947,
    "discount": 2,
    "rating": 3,
    "reviewCount": 48480,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Day after Tomorrow",
    "deliveryFree": true,
    "trustScore": 80,
    "trustLabel": "Genuine",
    "buyNowOrWait": "wait",
    "waitReason": "Sale expected in the next 2 weeks based on historical patterns.",
    "spikePriceMonths": [
      1
    ],
    "priceHistory": [
      1908,
      2151,
      1908,
      1908,
      1908,
      1707,
      1908,
      1908,
      1908,
      1908,
      1908,
      1908
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/sunglasses/sunglasses/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/sunglasses/sunglasses/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/sunglasses/sunglasses/1.webp",
      "https://cdn.dummyjson.com/product-images/sunglasses/sunglasses/2.webp"
    ],
    "description": "The Sunglasses offer a classic and simple design with a focus on functionality. These sunglasses provide essential UV protection while maintaining a timeless look.",
    "features": [
      "The Sunglasses offer a classic and simple design with a focus on functionality",
      "Polarised UV400 protection — shields eyes from harmful sun exposure",
      "Frame material: eco-friendly recycled nylon",
      "Anti-glare lens — clear, distortion-free vision",
      "Spring-hinge temples for comfortable all-day wear"
    ],
    "specs": {
      "Frame Material": "Stainless Steel",
      "Lens Material": "TAC Polarised",
      "UV Protection": "Polarised UV400",
      "Lens Width": "58mm",
      "Bridge Width": "19mm",
      "Temple Length": "139mm",
      "Face Shape": "Oval",
      "Warranty": "6 Months against manufacturing defects"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Fashion Shades",
    "soldByRating": 3.9,
    "sellerSince": "2014",
    "fulfillment": "Fulfilled by Amazon",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 93,
        "detail": null
      },
      "returnRate": {
        "score": 84,
        "detail": null
      },
      "warrantyClaims": {
        "score": 75,
        "detail": null
      },
      "sellerReliability": {
        "score": 83,
        "detail": null
      },
      "priceStability": {
        "score": 81,
        "detail": null
      }
    }
  },
  {
    "id": "dj159",
    "name": "iPad Mini 2021 Starlight",
    "brand": "Apple",
    "category": "Electronics > Mobiles",
    "price": 41499,
    "originalPrice": 43228,
    "discount": 4,
    "rating": 3.2,
    "reviewCount": 10209,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Day after Tomorrow",
    "deliveryFree": true,
    "trustScore": 81,
    "trustLabel": "Genuine",
    "buyNowOrWait": "wait",
    "waitReason": "Sale expected in the next 2 weeks based on historical patterns.",
    "spikePriceMonths": [],
    "priceHistory": [
      41499,
      41499,
      41499,
      41499,
      45551,
      41499,
      41499,
      41499,
      41499,
      37782,
      41499,
      41499
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/tablets/ipad-mini-2021-starlight/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/tablets/ipad-mini-2021-starlight/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/tablets/ipad-mini-2021-starlight/1.webp",
      "https://cdn.dummyjson.com/product-images/tablets/ipad-mini-2021-starlight/2.webp"
    ],
    "description": "The iPad Mini 2021 in Starlight is a compact and powerful tablet from Apple. Featuring a stunning Retina display, powerful A-series chip, and a sleek design, it offers a premium tablet experience.",
    "features": [
      "The iPad Mini 2021 in Starlight is a compact and powerful tablet from Apple",
      "6GB RAM with 64GB storage, expandable up to 1TB via microSD",
      "11\" IPS LCD display — perfect for streaming, reading and productivity",
      "7040mAh battery for up to 8 hours of continuous use",
      "Thin and lightweight design — ideal for travel, study, and entertainment on the go"
    ],
    "specs": {
      "Brand": "Apple",
      "Display": "11\" IPS LCD",
      "Processor": "Exynos 1280",
      "RAM": "4GB",
      "Storage": "256GB (expandable up to 1TB)",
      "Battery": "7040mAh",
      "Connectivity": "Wi-Fi 6 / Bluetooth 5.0 / USB-C",
      "OS": "Android 14",
      "Warranty": "2 year warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Apple",
    "soldByRating": 4.1,
    "sellerSince": "2016",
    "fulfillment": "Fulfilled by Amazon",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 93,
        "detail": null
      },
      "returnRate": {
        "score": 79,
        "detail": null
      },
      "warrantyClaims": {
        "score": 78,
        "detail": null
      },
      "sellerReliability": {
        "score": 82,
        "detail": null
      },
      "priceStability": {
        "score": 90,
        "detail": null
      }
    }
  },
  {
    "id": "dj160",
    "name": "Samsung Galaxy Tab S8 Plus Grey",
    "brand": "Samsung",
    "category": "Electronics > Mobiles",
    "price": 49799,
    "originalPrice": 57240,
    "discount": 13,
    "rating": 4.7,
    "reviewCount": 32092,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Day after Tomorrow",
    "deliveryFree": true,
    "trustScore": 94,
    "trustLabel": "Genuine",
    "buyNowOrWait": "wait",
    "waitReason": "Sale expected in the next 2 weeks based on historical patterns.",
    "spikePriceMonths": [
      7
    ],
    "priceHistory": [
      49799,
      49799,
      49799,
      49799,
      49799,
      53824,
      49799,
      60413,
      49799,
      55048,
      55292,
      49799
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/tablets/samsung-galaxy-tab-s8-plus-grey/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/tablets/samsung-galaxy-tab-s8-plus-grey/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/tablets/samsung-galaxy-tab-s8-plus-grey/1.webp",
      "https://cdn.dummyjson.com/product-images/tablets/samsung-galaxy-tab-s8-plus-grey/2.webp"
    ],
    "description": "The Samsung Galaxy Tab S8 Plus in Grey is a high-performance Android tablet by Samsung. With a large AMOLED display, powerful processor, and S Pen support, it's ideal for productivity and entertainment.",
    "features": [
      "The Samsung Galaxy Tab S8 Plus in Grey is a high-performance Android tablet by Samsung",
      "6GB RAM with 256GB storage, expandable up to 1TB via microSD",
      "11\" AMOLED display — perfect for streaming, reading and productivity",
      "8000mAh battery for up to 14 hours of continuous use",
      "Thin and lightweight design — ideal for travel, study, and entertainment on the go"
    ],
    "specs": {
      "Brand": "Samsung",
      "Display": "11\" AMOLED",
      "Processor": "Snapdragon 680",
      "RAM": "4GB",
      "Storage": "32GB (expandable up to 1TB)",
      "Battery": "8000mAh",
      "Connectivity": "Wi-Fi 6 / Bluetooth 5.0 / USB-C",
      "OS": "Android 14",
      "Warranty": "3 months warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Samsung",
    "soldByRating": 4.2,
    "sellerSince": "2020",
    "fulfillment": "Fulfilled by Amazon",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 89,
        "detail": null
      },
      "returnRate": {
        "score": 96,
        "detail": null
      },
      "warrantyClaims": {
        "score": 89,
        "detail": null
      },
      "sellerReliability": {
        "score": 96,
        "detail": null
      },
      "priceStability": {
        "score": 96,
        "detail": null
      }
    }
  },
  {
    "id": "dj161",
    "name": "Samsung Galaxy Tab White",
    "brand": "Samsung",
    "category": "Electronics > Mobiles",
    "price": 29049,
    "originalPrice": 35426,
    "discount": 18,
    "rating": 3.7,
    "reviewCount": 72160,
    "inStock": true,
    "isPrime": false,
    "delivery": "Get it by Day after Tomorrow",
    "deliveryFree": true,
    "trustScore": 84,
    "trustLabel": "Genuine",
    "buyNowOrWait": "buy",
    "waitReason": null,
    "spikePriceMonths": [],
    "priceHistory": [
      29049,
      29049,
      29049,
      29049,
      29049,
      29049,
      29049,
      29049,
      29049,
      29049,
      29049,
      29049
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/tablets/samsung-galaxy-tab-white/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/tablets/samsung-galaxy-tab-white/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/tablets/samsung-galaxy-tab-white/1.webp",
      "https://cdn.dummyjson.com/product-images/tablets/samsung-galaxy-tab-white/2.webp"
    ],
    "description": "The Samsung Galaxy Tab in White is a sleek and versatile Android tablet. With a vibrant display, long-lasting battery, and a range of features, it offers a great user experience for various tasks.",
    "features": [
      "The Samsung Galaxy Tab in White is a sleek and versatile Android tablet",
      "6GB RAM with 64GB storage, expandable up to 1TB via microSD",
      "11\" AMOLED display — perfect for streaming, reading and productivity",
      "6000mAh battery for up to 10 hours of continuous use",
      "Thin and lightweight design — ideal for travel, study, and entertainment on the go"
    ],
    "specs": {
      "Brand": "Samsung",
      "Display": "11\" AMOLED",
      "Processor": "Snapdragon 680",
      "RAM": "6GB",
      "Storage": "256GB (expandable up to 1TB)",
      "Battery": "6000mAh",
      "Connectivity": "Wi-Fi 6 / Bluetooth 5.0 / USB-C",
      "OS": "Android 12",
      "Warranty": "3 months warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Samsung",
    "soldByRating": 4.7,
    "sellerSince": "2011",
    "fulfillment": "Seller fulfilled",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 74,
        "detail": null
      },
      "returnRate": {
        "score": 87,
        "detail": null
      },
      "warrantyClaims": {
        "score": 90,
        "detail": null
      },
      "sellerReliability": {
        "score": 96,
        "detail": null
      },
      "priceStability": {
        "score": 76,
        "detail": null
      }
    }
  },
  {
    "id": "dj162",
    "name": "Blue Frock",
    "brand": "Generic",
    "category": "Fashion",
    "price": 2489,
    "originalPrice": 2828,
    "discount": 12,
    "rating": 4.2,
    "reviewCount": 56224,
    "inStock": true,
    "isPrime": false,
    "delivery": "Get it by Day after Tomorrow",
    "deliveryFree": true,
    "trustScore": 86,
    "trustLabel": "Genuine",
    "buyNowOrWait": "buy",
    "waitReason": null,
    "spikePriceMonths": [],
    "priceHistory": [
      2489,
      2489,
      2489,
      2489,
      2489,
      2489,
      2489,
      2489,
      2489,
      2489,
      2489,
      2489
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/tops/blue-frock/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/tops/blue-frock/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/tops/blue-frock/1.webp",
      "https://cdn.dummyjson.com/product-images/tops/blue-frock/2.webp"
    ],
    "description": "The Blue Frock is a charming and stylish dress for various occasions. With a vibrant blue color and a comfortable design, it adds a touch of elegance to your wardrobe.",
    "features": [
      "The Blue Frock is a charming and stylish dress for various occasions",
      "Fabric: polyester-spandex blend — soft, breathable, and skin-friendly",
      "Relaxed fit — easy to style for any occasion",
      "Dry-clean recommended — simple care routine",
      "Pair with high-waist jeans for a casual look"
    ],
    "specs": {
      "Fabric": "Polyester-Spandex Blend",
      "Fit": "Regular Fit",
      "Neckline": "Boat Neck",
      "Sleeve": "Sleeveless",
      "Care": "Machine Wash 30°C",
      "Sizes": "XS / S / M / L / XL / XXL",
      "Weight": "225g"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Third-party Seller",
    "soldByRating": 4.7,
    "sellerSince": "2017",
    "fulfillment": "Fulfilled by Amazon",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 80,
        "detail": null
      },
      "returnRate": {
        "score": 92,
        "detail": null
      },
      "warrantyClaims": {
        "score": 84,
        "detail": null
      },
      "sellerReliability": {
        "score": 91,
        "detail": null
      },
      "priceStability": {
        "score": 89,
        "detail": null
      }
    }
  },
  {
    "id": "dj163",
    "name": "Girl Summer Dress",
    "brand": "Generic",
    "category": "Fashion",
    "price": 1659,
    "originalPrice": 2048,
    "discount": 19,
    "rating": 4.8,
    "reviewCount": 33390,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Day after Tomorrow",
    "deliveryFree": true,
    "trustScore": 96,
    "trustLabel": "Genuine",
    "buyNowOrWait": "buy",
    "waitReason": null,
    "spikePriceMonths": [
      3,
      4,
      5,
      10
    ],
    "priceHistory": [
      1659,
      1659,
      1659,
      2381,
      2190,
      1925,
      1659,
      1659,
      1659,
      1659,
      1901,
      1659
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/tops/girl-summer-dress/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/tops/girl-summer-dress/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/tops/girl-summer-dress/1.webp",
      "https://cdn.dummyjson.com/product-images/tops/girl-summer-dress/2.webp"
    ],
    "description": "The Girl Summer Dress is a cute and breezy dress designed for warm weather. With playful patterns and lightweight fabric, it's perfect for keeping cool and stylish during the summer.",
    "features": [
      "The Girl Summer Dress is a cute and breezy dress designed for warm weather",
      "Fabric: rayon crepe — soft, breathable, and skin-friendly",
      "Oversized silhouette — easy to style for any occasion",
      "Hand wash cold — simple care routine",
      "Pair with shorts for summer styling"
    ],
    "specs": {
      "Fabric": "Rayon Crepe",
      "Fit": "Cropped",
      "Neckline": "Square Neck",
      "Sleeve": "Puff Sleeve",
      "Care": "Machine Wash 30°C",
      "Sizes": "XS / S / M / L / XL / XXL",
      "Weight": "227g"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Third-party Seller",
    "soldByRating": 4.8,
    "sellerSince": "2021",
    "fulfillment": "Seller fulfilled",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 96,
        "detail": null
      },
      "returnRate": {
        "score": 96,
        "detail": null
      },
      "warrantyClaims": {
        "score": 96,
        "detail": null
      },
      "sellerReliability": {
        "score": 96,
        "detail": null
      },
      "priceStability": {
        "score": 91,
        "detail": null
      }
    }
  },
  {
    "id": "dj164",
    "name": "Gray Dress",
    "brand": "Generic",
    "category": "Fashion",
    "price": 2904,
    "originalPrice": 3377,
    "discount": 14,
    "rating": 2.7,
    "reviewCount": 2795,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Tomorrow, 6 PM",
    "deliveryFree": true,
    "trustScore": 78,
    "trustLabel": "Genuine",
    "buyNowOrWait": "wait",
    "waitReason": "Sale expected in the next 2 weeks based on historical patterns.",
    "spikePriceMonths": [
      1,
      9
    ],
    "priceHistory": [
      2904,
      3792,
      2904,
      2904,
      2904,
      2904,
      2904,
      2904,
      2904,
      3800,
      2904,
      2904
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/tops/gray-dress/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/tops/gray-dress/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/tops/gray-dress/1.webp",
      "https://cdn.dummyjson.com/product-images/tops/gray-dress/2.webp"
    ],
    "description": "The Gray Dress is a versatile and chic option for various occasions. With a neutral gray color, it can be dressed up or down, making it a wardrobe staple for any fashion-forward individual.",
    "features": [
      "The Gray Dress is a versatile and chic option for various occasions",
      "Fabric: 100% cotton — soft, breathable, and skin-friendly",
      "Cropped length — easy to style for any occasion",
      "Machine washable at 30°C — simple care routine",
      "Pair with shorts for summer styling"
    ],
    "specs": {
      "Fabric": "100% Cotton",
      "Fit": "Relaxed Fit",
      "Neckline": "V-neck",
      "Sleeve": "Flutter Sleeve",
      "Care": "Dry Clean Recommended",
      "Sizes": "XS / S / M / L / XL / XXL",
      "Weight": "179g"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Third-party Seller",
    "soldByRating": 4.8,
    "sellerSince": "2011",
    "fulfillment": "Seller fulfilled",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 65,
        "detail": null
      },
      "returnRate": {
        "score": 76,
        "detail": null
      },
      "warrantyClaims": {
        "score": 83,
        "detail": null
      },
      "sellerReliability": {
        "score": 79,
        "detail": null
      },
      "priceStability": {
        "score": 89,
        "detail": null
      }
    }
  },
  {
    "id": "dj165",
    "name": "Short Frock",
    "brand": "Generic",
    "category": "Fashion",
    "price": 2074,
    "originalPrice": 2384,
    "discount": 13,
    "rating": 3.2,
    "reviewCount": 19278,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Day after Tomorrow",
    "deliveryFree": true,
    "trustScore": 76,
    "trustLabel": "Genuine",
    "buyNowOrWait": "buy",
    "waitReason": null,
    "spikePriceMonths": [
      1,
      6
    ],
    "priceHistory": [
      2074,
      2743,
      2074,
      2074,
      2058,
      2074,
      2745,
      2074,
      2074,
      2074,
      2074,
      2074
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/tops/short-frock/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/tops/short-frock/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/tops/short-frock/1.webp",
      "https://cdn.dummyjson.com/product-images/tops/short-frock/2.webp"
    ],
    "description": "The Short Frock is a playful and trendy dress with a shorter length. Ideal for casual outings or special occasions, it combines style and comfort for a fashionable look.",
    "features": [
      "The Short Frock is a playful and trendy dress with a shorter length",
      "Fabric: 100% viscose — soft, breathable, and skin-friendly",
      "Oversized silhouette — easy to style for any occasion",
      "Machine washable at 30°C — simple care routine",
      "Pair with tailored trousers for smart casual"
    ],
    "specs": {
      "Fabric": "100% Viscose",
      "Fit": "Cropped",
      "Neckline": "Round Neck",
      "Sleeve": "3/4 Sleeve",
      "Care": "Hand Wash Cold",
      "Sizes": "XS / S / M / L / XL / XXL",
      "Weight": "224g"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Third-party Seller",
    "soldByRating": 4.2,
    "sellerSince": "2017",
    "fulfillment": "Seller fulfilled",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 84,
        "detail": null
      },
      "returnRate": {
        "score": 66,
        "detail": null
      },
      "warrantyClaims": {
        "score": 76,
        "detail": null
      },
      "sellerReliability": {
        "score": 84,
        "detail": null
      },
      "priceStability": {
        "score": 83,
        "detail": null
      }
    }
  },
  {
    "id": "dj166",
    "name": "Tartan Dress",
    "brand": "Generic",
    "category": "Fashion",
    "price": 3319,
    "originalPrice": 3815,
    "discount": 13,
    "rating": 4.1,
    "reviewCount": 46344,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Day after Tomorrow",
    "deliveryFree": true,
    "trustScore": 87,
    "trustLabel": "Genuine",
    "buyNowOrWait": "wait",
    "waitReason": "Sale expected in the next 2 weeks based on historical patterns.",
    "spikePriceMonths": [],
    "priceHistory": [
      3319,
      3319,
      3319,
      3319,
      3358,
      3319,
      3319,
      3319,
      3319,
      3319,
      3319,
      3319
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/tops/tartan-dress/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/tops/tartan-dress/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/tops/tartan-dress/1.webp",
      "https://cdn.dummyjson.com/product-images/tops/tartan-dress/2.webp"
    ],
    "description": "The Tartan Dress features a classic tartan pattern, bringing a timeless and sophisticated touch to your wardrobe. Perfect for fall and winter, it adds a hint of traditional charm.",
    "features": [
      "The Tartan Dress features a classic tartan pattern, bringing a timeless and sophisticated touch to your wardrobe",
      "Fabric: polyester-spandex blend — soft, breathable, and skin-friendly",
      "Fitted cut — easy to style for any occasion",
      "Hand wash cold — simple care routine",
      "Pair with tailored trousers for smart casual"
    ],
    "specs": {
      "Fabric": "Polyester-Spandex Blend",
      "Fit": "Oversized",
      "Neckline": "Off-shoulder",
      "Sleeve": "3/4 Sleeve",
      "Care": "Hand Wash Cold",
      "Sizes": "XS / S / M / L / XL / XXL",
      "Weight": "204g"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Third-party Seller",
    "soldByRating": 4.8,
    "sellerSince": "2016",
    "fulfillment": "Fulfilled by Amazon",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 90,
        "detail": null
      },
      "returnRate": {
        "score": 91,
        "detail": null
      },
      "warrantyClaims": {
        "score": 81,
        "detail": null
      },
      "sellerReliability": {
        "score": 90,
        "detail": null
      },
      "priceStability": {
        "score": 79,
        "detail": null
      }
    }
  },
  {
    "id": "dj172",
    "name": "Blue Women's Handbag",
    "brand": "Fashionista",
    "category": "Fashion",
    "price": 4149,
    "originalPrice": 5060,
    "discount": 18,
    "rating": 2.9,
    "reviewCount": 32818,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Tomorrow, 6 PM",
    "deliveryFree": true,
    "trustScore": 78,
    "trustLabel": "Genuine",
    "buyNowOrWait": "wait",
    "waitReason": "Sale expected in the next 2 weeks based on historical patterns.",
    "spikePriceMonths": [
      5
    ],
    "priceHistory": [
      4149,
      4149,
      4149,
      4149,
      4415,
      5409,
      4149,
      4149,
      4149,
      4149,
      4149,
      4149
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/womens-bags/blue-women's-handbag/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/womens-bags/blue-women's-handbag/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/womens-bags/blue-women's-handbag/1.webp",
      "https://cdn.dummyjson.com/product-images/womens-bags/blue-women's-handbag/2.webp"
    ],
    "description": "The Blue Women's Handbag is a stylish and spacious accessory for everyday use. With a vibrant blue color and multiple compartments, it combines fashion and functionality.",
    "features": [
      "The Blue Women's Handbag is a stylish and spacious accessory for everyday use",
      "Material: genuine pebble leather — durable and elegant",
      "Adjustable shoulder strap — functional and stylish",
      "Gold-plated hardware — adds a sophisticated finish",
      "Lining: easy-clean PU lining with dedicated card slots and zip pocket"
    ],
    "specs": {
      "Material": "Genuine Pebble Leather",
      "Dimensions": "14W × 12H × 26D cm",
      "Hardware Colour": "Silver-tone",
      "Closure": "Buckle Flap",
      "Compartments": "1 Main + Laptop Sleeve + 3 Pockets",
      "Strap": "Detachable Shoulder Strap"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Fashionista",
    "soldByRating": 5,
    "sellerSince": "2019",
    "fulfillment": "Seller fulfilled",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 79,
        "detail": null
      },
      "returnRate": {
        "score": 77,
        "detail": null
      },
      "warrantyClaims": {
        "score": 71,
        "detail": null
      },
      "sellerReliability": {
        "score": 93,
        "detail": null
      },
      "priceStability": {
        "score": 73,
        "detail": null
      }
    }
  },
  {
    "id": "dj173",
    "name": "Heshe Women's Leather Bag",
    "brand": "Heshe",
    "category": "Fashion",
    "price": 10789,
    "originalPrice": 11239,
    "discount": 4,
    "rating": 4.9,
    "reviewCount": 4367,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Tomorrow, 6 PM",
    "deliveryFree": true,
    "trustScore": 94,
    "trustLabel": "Genuine",
    "buyNowOrWait": "buy",
    "waitReason": null,
    "spikePriceMonths": [
      9
    ],
    "priceHistory": [
      10789,
      10789,
      10789,
      10789,
      10789,
      9445,
      10789,
      10789,
      10789,
      12283,
      11546,
      10789
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/womens-bags/heshe-women's-leather-bag/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/womens-bags/heshe-women's-leather-bag/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/womens-bags/heshe-women's-leather-bag/1.webp",
      "https://cdn.dummyjson.com/product-images/womens-bags/heshe-women's-leather-bag/2.webp"
    ],
    "description": "The Heshe Women's Leather Bag is a luxurious and high-quality leather bag for the sophisticated woman. With a timeless design and durable craftsmanship, it's a versatile accessory.",
    "features": [
      "The Heshe Women's Leather Bag is a luxurious and high-quality leather bag for the sophisticated woman",
      "Material: genuine pebble leather — durable and elegant",
      "Spacious main compartment — functional and stylish",
      "Brass-tone hardware — adds a sophisticated finish",
      "Lining: easy-clean PU lining with dedicated card slots and zip pocket"
    ],
    "specs": {
      "Material": "Genuine Pebble Leather",
      "Dimensions": "29W × 21H × 7D cm",
      "Hardware Colour": "Gold-tone",
      "Closure": "Buckle Flap",
      "Compartments": "1 Main + 1 Zip Pocket",
      "Strap": "Chain Strap"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Heshe",
    "soldByRating": 4.1,
    "sellerSince": "2016",
    "fulfillment": "Seller fulfilled",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 88,
        "detail": null
      },
      "returnRate": {
        "score": 83,
        "detail": null
      },
      "warrantyClaims": {
        "score": 93,
        "detail": null
      },
      "sellerReliability": {
        "score": 96,
        "detail": null
      },
      "priceStability": {
        "score": 96,
        "detail": null
      }
    }
  },
  {
    "id": "dj174",
    "name": "Prada Women Bag",
    "brand": "Prada",
    "category": "Fashion",
    "price": 49799,
    "originalPrice": 57906,
    "discount": 14,
    "rating": 2.7,
    "reviewCount": 54747,
    "inStock": true,
    "isPrime": false,
    "delivery": "Get it by Day after Tomorrow",
    "deliveryFree": true,
    "trustScore": 75,
    "trustLabel": "Mixed",
    "buyNowOrWait": "buy",
    "waitReason": null,
    "spikePriceMonths": [],
    "priceHistory": [
      49799,
      49799,
      49799,
      49799,
      49799,
      49799,
      49799,
      49799,
      52544,
      53918,
      49799,
      49799
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/womens-bags/prada-women-bag/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/womens-bags/prada-women-bag/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/womens-bags/prada-women-bag/1.webp",
      "https://cdn.dummyjson.com/product-images/womens-bags/prada-women-bag/2.webp"
    ],
    "description": "The Prada Women Bag is an iconic designer bag that exudes elegance and luxury. Crafted with precision and featuring the Prada logo, it's a statement piece for fashion enthusiasts.",
    "features": [
      "The Prada Women Bag is an iconic designer bag that exudes elegance and luxury",
      "Material: genuine pebble leather — durable and elegant",
      "Multiple zip pockets — functional and stylish",
      "Silver-tone hardware — adds a sophisticated finish",
      "Lining: fabric lining with dedicated card slots and zip pocket"
    ],
    "specs": {
      "Material": "Genuine Pebble Leather",
      "Dimensions": "22W × 22H × 13D cm",
      "Hardware Colour": "Gunmetal",
      "Closure": "Zip Top",
      "Compartments": "1 Main + 2 Inner Zip + Card Slots",
      "Strap": "Chain Strap"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Prada",
    "soldByRating": 4.2,
    "sellerSince": "2017",
    "fulfillment": "Seller fulfilled",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 89,
        "detail": null
      },
      "returnRate": {
        "score": 82,
        "detail": null
      },
      "warrantyClaims": {
        "score": 66,
        "detail": null
      },
      "sellerReliability": {
        "score": 82,
        "detail": null
      },
      "priceStability": {
        "score": 80,
        "detail": null
      }
    }
  },
  {
    "id": "dj175",
    "name": "White Faux Leather Backpack",
    "brand": "Urban Chic",
    "category": "Fashion",
    "price": 3319,
    "originalPrice": 3905,
    "discount": 15,
    "rating": 3.4,
    "reviewCount": 27658,
    "inStock": true,
    "isPrime": false,
    "delivery": "Get it by Day after Tomorrow",
    "deliveryFree": true,
    "trustScore": 88,
    "trustLabel": "Genuine",
    "buyNowOrWait": "wait",
    "waitReason": "Sale expected in the next 2 weeks based on historical patterns.",
    "spikePriceMonths": [],
    "priceHistory": [
      3319,
      3319,
      3319,
      3468,
      3319,
      3319,
      3319,
      3319,
      3319,
      3319,
      3371,
      3319
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/womens-bags/white-faux-leather-backpack/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/womens-bags/white-faux-leather-backpack/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/womens-bags/white-faux-leather-backpack/1.webp",
      "https://cdn.dummyjson.com/product-images/womens-bags/white-faux-leather-backpack/2.webp"
    ],
    "description": "The White Faux Leather Backpack is a trendy and practical backpack for the modern woman. With a sleek white design and ample storage space, it's perfect for both casual and on-the-go styles.",
    "features": [
      "The White Faux Leather Backpack is a trendy and practical backpack for the modern woman",
      "Material: quilted leather — durable and elegant",
      "Detachable crossbody strap — functional and stylish",
      "Gold-plated hardware — adds a sophisticated finish",
      "Lining: fabric lining with dedicated card slots and zip pocket"
    ],
    "specs": {
      "Material": "Satin",
      "Dimensions": "14W × 23H × 29D cm",
      "Hardware Colour": "Silver-tone",
      "Closure": "Zip Top",
      "Compartments": "1 Main + 2 Inner Zip + Card Slots",
      "Strap": "Adjustable Crossbody Strap"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Urban Chic",
    "soldByRating": 4.1,
    "sellerSince": "2020",
    "fulfillment": "Fulfilled by Amazon",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 75,
        "detail": null
      },
      "returnRate": {
        "score": 96,
        "detail": null
      },
      "warrantyClaims": {
        "score": 89,
        "detail": null
      },
      "sellerReliability": {
        "score": 96,
        "detail": null
      },
      "priceStability": {
        "score": 93,
        "detail": null
      }
    }
  },
  {
    "id": "dj176",
    "name": "Women Handbag Black",
    "brand": "Elegance Collection",
    "category": "Fashion",
    "price": 4979,
    "originalPrice": 5658,
    "discount": 12,
    "rating": 2.9,
    "reviewCount": 15311,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Tomorrow, 6 PM",
    "deliveryFree": true,
    "trustScore": 78,
    "trustLabel": "Genuine",
    "buyNowOrWait": "wait",
    "waitReason": "Sale expected in the next 2 weeks based on historical patterns.",
    "spikePriceMonths": [
      4
    ],
    "priceHistory": [
      4979,
      4979,
      4979,
      5244,
      6172,
      4979,
      4979,
      5165,
      4979,
      4979,
      4979,
      4979
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/womens-bags/women-handbag-black/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/womens-bags/women-handbag-black/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/womens-bags/women-handbag-black/1.webp",
      "https://cdn.dummyjson.com/product-images/womens-bags/women-handbag-black/2.webp"
    ],
    "description": "The Women Handbag in Black is a classic and versatile accessory that complements various outfits. With a timeless black color and functional design, it's a must-have in every woman's wardrobe.",
    "features": [
      "The Women Handbag in Black is a classic and versatile accessory that complements various outfits",
      "Material: nylon canvas — durable and elegant",
      "Detachable crossbody strap — functional and stylish",
      "Brass-tone hardware — adds a sophisticated finish",
      "Lining: easy-clean PU lining with dedicated card slots and zip pocket"
    ],
    "specs": {
      "Material": "Nylon Canvas",
      "Dimensions": "30W × 12H × 16D cm",
      "Hardware Colour": "Gold-tone",
      "Closure": "Buckle Flap",
      "Compartments": "1 Main + 1 Zip Pocket",
      "Strap": "Fixed Dual Handle"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Elegance Collection",
    "soldByRating": 4.1,
    "sellerSince": "2021",
    "fulfillment": "Fulfilled by Amazon",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 79,
        "detail": null
      },
      "returnRate": {
        "score": 83,
        "detail": null
      },
      "warrantyClaims": {
        "score": 80,
        "detail": null
      },
      "sellerReliability": {
        "score": 89,
        "detail": null
      },
      "priceStability": {
        "score": 83,
        "detail": null
      }
    }
  },
  {
    "id": "dj177",
    "name": "Black Women's Gown",
    "brand": "Generic",
    "category": "Fashion",
    "price": 10789,
    "originalPrice": 11988,
    "discount": 10,
    "rating": 3.6,
    "reviewCount": 70800,
    "inStock": true,
    "isPrime": false,
    "delivery": "Get it by Day after Tomorrow",
    "deliveryFree": true,
    "trustScore": 81,
    "trustLabel": "Genuine",
    "buyNowOrWait": "wait",
    "waitReason": "Sale expected in the next 2 weeks based on historical patterns.",
    "spikePriceMonths": [
      1
    ],
    "priceHistory": [
      10789,
      13115,
      10789,
      11309,
      10789,
      10789,
      10789,
      10789,
      10789,
      10789,
      10789,
      10789
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/womens-dresses/black-women's-gown/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/womens-dresses/black-women's-gown/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/womens-dresses/black-women's-gown/1.webp",
      "https://cdn.dummyjson.com/product-images/womens-dresses/black-women's-gown/2.webp"
    ],
    "description": "The Black Women's Gown is an elegant and timeless evening gown. With a sleek black design, it's perfect for formal events and special occasions, exuding sophistication and style.",
    "features": [
      "The Black Women's Gown is an elegant and timeless evening gown",
      "Fabric: 100% polyester chiffon — lightweight, breathable, and flattering",
      "Flowy midi length that flatters all body types",
      "Tie waist detail for effortless styling",
      "Machine washable at 30°C — easy care for busy lifestyles"
    ],
    "specs": {
      "Fabric": "100% Polyester Chiffon",
      "Fit": "Shift",
      "Neckline": "Halter Neck",
      "Sleeve": "Sleeveless",
      "Length": "Mini (above knee)",
      "Care": "Machine Wash 30°C",
      "Sizes": "XS / S / M / L / XL / XXL"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Third-party Seller",
    "soldByRating": 4,
    "sellerSince": "2010",
    "fulfillment": "Fulfilled by Amazon",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 92,
        "detail": null
      },
      "returnRate": {
        "score": 79,
        "detail": null
      },
      "warrantyClaims": {
        "score": 78,
        "detail": null
      },
      "sellerReliability": {
        "score": 87,
        "detail": null
      },
      "priceStability": {
        "score": 82,
        "detail": null
      }
    }
  },
  {
    "id": "dj178",
    "name": "Corset Leather With Skirt",
    "brand": "Generic",
    "category": "Fashion",
    "price": 7469,
    "originalPrice": 8892,
    "discount": 16,
    "rating": 3.1,
    "reviewCount": 23664,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Day after Tomorrow",
    "deliveryFree": true,
    "trustScore": 79,
    "trustLabel": "Genuine",
    "buyNowOrWait": "wait",
    "waitReason": "Sale expected in the next 2 weeks based on historical patterns.",
    "spikePriceMonths": [
      5
    ],
    "priceHistory": [
      8254,
      7469,
      7469,
      7880,
      7469,
      9795,
      7469,
      7469,
      7469,
      8315,
      7469,
      7469
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/womens-dresses/corset-leather-with-skirt/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/womens-dresses/corset-leather-with-skirt/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/womens-dresses/corset-leather-with-skirt/1.webp",
      "https://cdn.dummyjson.com/product-images/womens-dresses/corset-leather-with-skirt/2.webp"
    ],
    "description": "The Corset Leather With Skirt is a bold and edgy ensemble that combines a stylish corset with a matching skirt. Ideal for fashion-forward individuals, it makes a statement at any event.",
    "features": [
      "The Corset Leather With Skirt is a bold and edgy ensemble that combines a stylish corset with a matching skirt",
      "Fabric: cotton blend — lightweight, breathable, and flattering",
      "A-line silhouette that flatters all body types",
      "Back button closure for effortless styling",
      "Hand wash recommended — easy care for busy lifestyles"
    ],
    "specs": {
      "Fabric": "Cotton Blend",
      "Fit": "Wrap",
      "Neckline": "Round Neck",
      "Sleeve": "Short Sleeve",
      "Length": "Mini (above knee)",
      "Care": "Machine Wash 30°C",
      "Sizes": "XS / S / M / L / XL / XXL"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Third-party Seller",
    "soldByRating": 4.3,
    "sellerSince": "2012",
    "fulfillment": "Seller fulfilled",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 73,
        "detail": null
      },
      "returnRate": {
        "score": 72,
        "detail": null
      },
      "warrantyClaims": {
        "score": 77,
        "detail": null
      },
      "sellerReliability": {
        "score": 93,
        "detail": null
      },
      "priceStability": {
        "score": 79,
        "detail": null
      }
    }
  },
  {
    "id": "dj179",
    "name": "Corset With Black Skirt",
    "brand": "Generic",
    "category": "Fashion",
    "price": 6639,
    "originalPrice": 7811,
    "discount": 15,
    "rating": 4.5,
    "reviewCount": 63349,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Tomorrow, 6 PM",
    "deliveryFree": true,
    "trustScore": 41,
    "trustLabel": "Suspicious",
    "buyNowOrWait": "buy",
    "waitReason": null,
    "spikePriceMonths": [
      2,
      7
    ],
    "priceHistory": [
      6639,
      7160,
      7735,
      6639,
      6639,
      6639,
      6639,
      9068,
      6639,
      6639,
      6639,
      6639
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/womens-dresses/corset-with-black-skirt/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/womens-dresses/corset-with-black-skirt/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/womens-dresses/corset-with-black-skirt/1.webp",
      "https://cdn.dummyjson.com/product-images/womens-dresses/corset-with-black-skirt/2.webp"
    ],
    "description": "The Corset With Black Skirt is a chic and versatile outfit that pairs a fashionable corset with a classic black skirt. It offers a trendy and coordinated look for various occasions.",
    "features": [
      "The Corset With Black Skirt is a chic and versatile outfit that pairs a fashionable corset with a classic black skirt",
      "Fabric: 100% polyester chiffon — lightweight, breathable, and flattering",
      "Flowy midi length that flatters all body types",
      "Tie waist detail for effortless styling",
      "Hand wash recommended — easy care for busy lifestyles"
    ],
    "specs": {
      "Fabric": "100% Polyester Chiffon",
      "Fit": "Shift",
      "Neckline": "Halter Neck",
      "Sleeve": "Full Sleeve",
      "Length": "Maxi (floor length)",
      "Care": "Dry Clean Only",
      "Sizes": "XS / S / M / L / XL / XXL"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Third-party Seller",
    "soldByRating": 2.2,
    "sellerSince": "2023",
    "fulfillment": "Seller fulfilled",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 27,
        "detail": null
      },
      "returnRate": {
        "score": 41,
        "detail": null
      },
      "warrantyClaims": {
        "score": 46,
        "detail": null
      },
      "sellerReliability": {
        "score": 44,
        "detail": null
      },
      "priceStability": {
        "score": 40,
        "detail": null
      }
    }
  },
  {
    "id": "dj180",
    "name": "Dress Pea",
    "brand": "Generic",
    "category": "Fashion",
    "price": 4149,
    "originalPrice": 5060,
    "discount": 18,
    "rating": 4.9,
    "reviewCount": 18831,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Day after Tomorrow",
    "deliveryFree": true,
    "trustScore": 96,
    "trustLabel": "Genuine",
    "buyNowOrWait": "buy",
    "waitReason": null,
    "spikePriceMonths": [
      5,
      6,
      7
    ],
    "priceHistory": [
      4149,
      4149,
      4149,
      4149,
      4149,
      5591,
      5855,
      5439,
      4149,
      4149,
      4149,
      4149
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/womens-dresses/dress-pea/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/womens-dresses/dress-pea/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/womens-dresses/dress-pea/1.webp",
      "https://cdn.dummyjson.com/product-images/womens-dresses/dress-pea/2.webp"
    ],
    "description": "The Dress Pea is a stylish and comfortable dress with a pea pattern. Perfect for casual outings, it adds a playful and fun element to your wardrobe, making it a great choice for day-to-day wear.",
    "features": [
      "The Dress Pea is a stylish and comfortable dress with a pea pattern",
      "Fabric: satin crepe — lightweight, breathable, and flattering",
      "A-line silhouette that flatters all body types",
      "Side zip closure for effortless styling",
      "Dry-clean only — easy care for busy lifestyles"
    ],
    "specs": {
      "Fabric": "Satin Crepe",
      "Fit": "A-line",
      "Neckline": "Round Neck",
      "Sleeve": "Flutter Sleeve",
      "Length": "Mini (above knee)",
      "Care": "Dry Clean Only",
      "Sizes": "XS / S / M / L / XL / XXL"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Third-party Seller",
    "soldByRating": 4.3,
    "sellerSince": "2013",
    "fulfillment": "Fulfilled by Amazon",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 96,
        "detail": null
      },
      "returnRate": {
        "score": 96,
        "detail": null
      },
      "warrantyClaims": {
        "score": 91,
        "detail": null
      },
      "sellerReliability": {
        "score": 96,
        "detail": null
      },
      "priceStability": {
        "score": 96,
        "detail": null
      }
    }
  },
  {
    "id": "dj181",
    "name": "Marni Red & Black Suit",
    "brand": "Generic",
    "category": "Fashion",
    "price": 14939,
    "originalPrice": 18443,
    "discount": 19,
    "rating": 4.5,
    "reviewCount": 50015,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Tomorrow, 6 PM",
    "deliveryFree": true,
    "trustScore": 90,
    "trustLabel": "Genuine",
    "buyNowOrWait": "buy",
    "waitReason": null,
    "spikePriceMonths": [
      1,
      7,
      8
    ],
    "priceHistory": [
      14939,
      21153,
      14939,
      14939,
      14939,
      14939,
      14939,
      17463,
      17057,
      14939,
      14939,
      14939
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/womens-dresses/marni-red-&-black-suit/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/womens-dresses/marni-red-&-black-suit/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/womens-dresses/marni-red-&-black-suit/1.webp",
      "https://cdn.dummyjson.com/product-images/womens-dresses/marni-red-&-black-suit/2.webp"
    ],
    "description": "The Marni Red & Black Suit is a sophisticated and fashion-forward suit ensemble. With a combination of red and black tones, it showcases a modern design for a bold and confident look.",
    "features": [
      "The Marni Red & Black Suit is a sophisticated and fashion-forward suit ensemble",
      "Fabric: cotton blend — lightweight, breathable, and flattering",
      "Bodycon cut that flatters all body types",
      "Elastic waistband for effortless styling",
      "Hand wash recommended — easy care for busy lifestyles"
    ],
    "specs": {
      "Fabric": "Cotton Blend",
      "Fit": "Bodycon",
      "Neckline": "Off-shoulder",
      "Sleeve": "3/4 Sleeve",
      "Length": "Maxi (floor length)",
      "Care": "Dry Clean Only",
      "Sizes": "XS / S / M / L / XL / XXL"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Third-party Seller",
    "soldByRating": 3.9,
    "sellerSince": "2015",
    "fulfillment": "Seller fulfilled",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 80,
        "detail": null
      },
      "returnRate": {
        "score": 92,
        "detail": null
      },
      "warrantyClaims": {
        "score": 81,
        "detail": null
      },
      "sellerReliability": {
        "score": 91,
        "detail": null
      },
      "priceStability": {
        "score": 79,
        "detail": null
      }
    }
  },
  {
    "id": "dj182",
    "name": "Green Crystal Earring",
    "brand": "Generic",
    "category": "Fashion",
    "price": 2489,
    "originalPrice": 2928,
    "discount": 15,
    "rating": 4,
    "reviewCount": 41849,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Tomorrow, 6 PM",
    "deliveryFree": true,
    "trustScore": 89,
    "trustLabel": "Genuine",
    "buyNowOrWait": "wait",
    "waitReason": "Sale expected in the next 2 weeks based on historical patterns.",
    "spikePriceMonths": [
      10
    ],
    "priceHistory": [
      2489,
      2639,
      2489,
      2489,
      2489,
      2489,
      2489,
      2489,
      2489,
      2489,
      3424,
      2489
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/womens-jewellery/green-crystal-earring/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/womens-jewellery/green-crystal-earring/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/womens-jewellery/green-crystal-earring/1.webp",
      "https://cdn.dummyjson.com/product-images/womens-jewellery/green-crystal-earring/2.webp"
    ],
    "description": "The Green Crystal Earring is a dazzling accessory that features a vibrant green crystal. With a classic design, it adds a touch of elegance to your ensemble, perfect for formal or special occasions.",
    "features": [
      "The Green Crystal Earring is a dazzling accessory that features a vibrant green crystal",
      "Base metal: rose gold-plated copper — hypoallergenic and tarnish-resistant",
      "Freshwater pearl accent — sparkling and eye-catching",
      "Spring-ring clasp for secure and comfortable wear",
      "Suitable for gifting and special occasions"
    ],
    "specs": {
      "Base Metal": "Rose Gold-plated Copper",
      "Stone": "Freshwater Pearl",
      "Stone Colour": "Blush Pink",
      "Clasp": "Lobster Clasp",
      "Length": "15 cm",
      "Finish": "Satin Matte"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Third-party Seller",
    "soldByRating": 4.1,
    "sellerSince": "2021",
    "fulfillment": "Fulfilled by Amazon",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 93,
        "detail": null
      },
      "returnRate": {
        "score": 96,
        "detail": null
      },
      "warrantyClaims": {
        "score": 85,
        "detail": null
      },
      "sellerReliability": {
        "score": 96,
        "detail": null
      },
      "priceStability": {
        "score": 90,
        "detail": null
      }
    }
  },
  {
    "id": "dj183",
    "name": "Green Oval Earring",
    "brand": "Generic",
    "category": "Fashion",
    "price": 2074,
    "originalPrice": 2440,
    "discount": 15,
    "rating": 3.6,
    "reviewCount": 45585,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Tomorrow, 6 PM",
    "deliveryFree": true,
    "trustScore": 84,
    "trustLabel": "Genuine",
    "buyNowOrWait": "wait",
    "waitReason": "Sale expected in the next 2 weeks based on historical patterns.",
    "spikePriceMonths": [
      6,
      7
    ],
    "priceHistory": [
      2074,
      2074,
      2074,
      2074,
      2074,
      2074,
      2808,
      2393,
      2074,
      2074,
      2074,
      2074
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/womens-jewellery/green-oval-earring/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/womens-jewellery/green-oval-earring/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/womens-jewellery/green-oval-earring/1.webp",
      "https://cdn.dummyjson.com/product-images/womens-jewellery/green-oval-earring/2.webp"
    ],
    "description": "The Green Oval Earring is a stylish and versatile accessory with a unique oval shape. Whether for casual or dressy occasions, its green hue and contemporary design make it a standout piece.",
    "features": [
      "The Green Oval Earring is a stylish and versatile accessory with a unique oval shape",
      "Base metal: 18K gold-plated brass — hypoallergenic and tarnish-resistant",
      "Enamel finish — sparkling and eye-catching",
      "Spring-ring clasp for secure and comfortable wear",
      "Suitable for formal and casual styling"
    ],
    "specs": {
      "Base Metal": "18K Gold-plated Brass",
      "Stone": "No Stone — Polished Metal",
      "Stone Colour": "Midnight Blue",
      "Clasp": "Screw Back",
      "Length": "8 cm",
      "Finish": "High Polish"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Third-party Seller",
    "soldByRating": 4.6,
    "sellerSince": "2016",
    "fulfillment": "Fulfilled by Amazon",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 73,
        "detail": null
      },
      "returnRate": {
        "score": 84,
        "detail": null
      },
      "warrantyClaims": {
        "score": 92,
        "detail": null
      },
      "sellerReliability": {
        "score": 94,
        "detail": null
      },
      "priceStability": {
        "score": 80,
        "detail": null
      }
    }
  },
  {
    "id": "dj184",
    "name": "Tropical Earring",
    "brand": "Generic",
    "category": "Fashion",
    "price": 1659,
    "originalPrice": 1676,
    "discount": 1,
    "rating": 4.4,
    "reviewCount": 43858,
    "inStock": true,
    "isPrime": false,
    "delivery": "Get it by Day after Tomorrow",
    "deliveryFree": true,
    "trustScore": 86,
    "trustLabel": "Genuine",
    "buyNowOrWait": "buy",
    "waitReason": null,
    "spikePriceMonths": [
      4
    ],
    "priceHistory": [
      1659,
      1659,
      1659,
      1540,
      1901,
      1659,
      1659,
      1659,
      1659,
      1659,
      1705,
      1659
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/womens-jewellery/tropical-earring/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/womens-jewellery/tropical-earring/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/womens-jewellery/tropical-earring/1.webp",
      "https://cdn.dummyjson.com/product-images/womens-jewellery/tropical-earring/2.webp"
    ],
    "description": "The Tropical Earring is a fun and playful accessory inspired by tropical elements. Featuring vibrant colors and a lively design, it's perfect for adding a touch of summer to your look.",
    "features": [
      "The Tropical Earring is a fun and playful accessory inspired by tropical elements",
      "Base metal: sterling silver 925 — hypoallergenic and tarnish-resistant",
      "Natural stone inlay — sparkling and eye-catching",
      "Spring-ring clasp for secure and comfortable wear",
      "Suitable for daily wear"
    ],
    "specs": {
      "Base Metal": "Sterling Silver 925",
      "Stone": "Onyx",
      "Stone Colour": "Blush Pink",
      "Clasp": "Spring-ring Clasp",
      "Length": "15 cm",
      "Finish": "Antique Oxidised"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Third-party Seller",
    "soldByRating": 4.8,
    "sellerSince": "2014",
    "fulfillment": "Fulfilled by Amazon",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 81,
        "detail": null
      },
      "returnRate": {
        "score": 80,
        "detail": null
      },
      "warrantyClaims": {
        "score": 88,
        "detail": null
      },
      "sellerReliability": {
        "score": 91,
        "detail": null
      },
      "priceStability": {
        "score": 96,
        "detail": null
      }
    }
  },
  {
    "id": "dj185",
    "name": "Black & Brown Slipper",
    "brand": "Comfort Trends",
    "category": "Fashion",
    "price": 1659,
    "originalPrice": 1710,
    "discount": 3,
    "rating": 2.5,
    "reviewCount": 49655,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Tomorrow, 6 PM",
    "deliveryFree": true,
    "trustScore": 79,
    "trustLabel": "Genuine",
    "buyNowOrWait": "wait",
    "waitReason": "Sale expected in the next 2 weeks based on historical patterns.",
    "spikePriceMonths": [
      0
    ],
    "priceHistory": [
      1971,
      1659,
      1659,
      1659,
      1659,
      1659,
      1659,
      1659,
      1462,
      1659,
      1701,
      1659
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/womens-shoes/black-&-brown-slipper/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/womens-shoes/black-&-brown-slipper/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/womens-shoes/black-&-brown-slipper/1.webp",
      "https://cdn.dummyjson.com/product-images/womens-shoes/black-&-brown-slipper/2.webp"
    ],
    "description": "The Black & Brown Slipper is a comfortable and stylish choice for casual wear. Featuring a blend of black and brown colors, it adds a touch of sophistication to your relaxation.",
    "features": [
      "The Black & Brown Slipper is a comfortable and stylish choice for casual wear",
      "Upper: canvas — on-trend and long-lasting",
      "Cushioned footbed for all-day comfort",
      "Flat sole — 7cm height for comfortable all-day wear",
      "Leather outsole — safe and steady on all surfaces"
    ],
    "specs": {
      "Upper Material": "Canvas",
      "Heel Type": "Block Heel",
      "Heel Height": "9 cm",
      "Sole Material": "Flexible TPR",
      "Closure": "Velcro",
      "Sizes": "UK 3 – UK 8",
      "Warranty": "6 Months against manufacturing defects"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Comfort Trends",
    "soldByRating": 4.5,
    "sellerSince": "2011",
    "fulfillment": "Fulfilled by Amazon",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 85,
        "detail": null
      },
      "returnRate": {
        "score": 89,
        "detail": null
      },
      "warrantyClaims": {
        "score": 84,
        "detail": null
      },
      "sellerReliability": {
        "score": 89,
        "detail": null
      },
      "priceStability": {
        "score": 87,
        "detail": null
      }
    }
  },
  {
    "id": "dj186",
    "name": "Calvin Klein Heel Shoes",
    "brand": "Calvin Klein",
    "category": "Fashion",
    "price": 6639,
    "originalPrice": 6844,
    "discount": 3,
    "rating": 4.9,
    "reviewCount": 33664,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Tomorrow, 6 PM",
    "deliveryFree": true,
    "trustScore": 95,
    "trustLabel": "Genuine",
    "buyNowOrWait": "buy",
    "waitReason": null,
    "spikePriceMonths": [
      9
    ],
    "priceHistory": [
      6639,
      6639,
      6639,
      6639,
      6639,
      6639,
      6639,
      6639,
      6187,
      7443,
      6639,
      6639
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/womens-shoes/calvin-klein-heel-shoes/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/womens-shoes/calvin-klein-heel-shoes/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/womens-shoes/calvin-klein-heel-shoes/1.webp",
      "https://cdn.dummyjson.com/product-images/womens-shoes/calvin-klein-heel-shoes/2.webp"
    ],
    "description": "Calvin Klein Heel Shoes are elegant and sophisticated, designed for formal occasions. With a classic design and high-quality materials, they complement your stylish ensemble.",
    "features": [
      "Calvin Klein Heel Shoes are elegant and sophisticated, designed for formal occasions",
      "Upper: suede — on-trend and long-lasting",
      "Padded ankle strap for all-day comfort",
      "Kitten heel — 5cm height for comfortable all-day wear",
      "Flexible TPR sole — safe and steady on all surfaces"
    ],
    "specs": {
      "Upper Material": "Suede",
      "Heel Type": "Flat",
      "Heel Height": "2 cm",
      "Sole Material": "Leather",
      "Closure": "Lace-up",
      "Sizes": "UK 3 – UK 8",
      "Warranty": "6 Months against manufacturing defects"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Calvin Klein",
    "soldByRating": 4.8,
    "sellerSince": "2012",
    "fulfillment": "Seller fulfilled",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 94,
        "detail": null
      },
      "returnRate": {
        "score": 95,
        "detail": null
      },
      "warrantyClaims": {
        "score": 87,
        "detail": null
      },
      "sellerReliability": {
        "score": 96,
        "detail": null
      },
      "priceStability": {
        "score": 87,
        "detail": null
      }
    }
  },
  {
    "id": "dj187",
    "name": "Golden Shoes Woman",
    "brand": "Fashion Diva",
    "category": "Fashion",
    "price": 4149,
    "originalPrice": 4824,
    "discount": 14,
    "rating": 3.3,
    "reviewCount": 4704,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Day after Tomorrow",
    "deliveryFree": true,
    "trustScore": 79,
    "trustLabel": "Genuine",
    "buyNowOrWait": "buy",
    "waitReason": null,
    "spikePriceMonths": [
      3
    ],
    "priceHistory": [
      4149,
      4149,
      4149,
      4992,
      4149,
      4149,
      4149,
      4149,
      4149,
      4149,
      4149,
      4149
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/womens-shoes/golden-shoes-woman/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/womens-shoes/golden-shoes-woman/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/womens-shoes/golden-shoes-woman/1.webp",
      "https://cdn.dummyjson.com/product-images/womens-shoes/golden-shoes-woman/2.webp"
    ],
    "description": "The Golden Shoes for Women are a glamorous choice for special occasions. Featuring a golden hue and stylish design, they add a touch of luxury to your outfit.",
    "features": [
      "The Golden Shoes for Women are a glamorous choice for special occasions",
      "Upper: genuine leather — on-trend and long-lasting",
      "Memory foam insole for all-day comfort",
      "Stiletto heel — flat height for comfortable all-day wear",
      "Flexible TPR sole — safe and steady on all surfaces"
    ],
    "specs": {
      "Upper Material": "Genuine Leather",
      "Heel Type": "Kitten Heel",
      "Heel Height": "4 cm",
      "Sole Material": "Synthetic Crepe",
      "Closure": "Slip-on",
      "Sizes": "UK 3 – UK 8",
      "Warranty": "6 Months against manufacturing defects"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Fashion Diva",
    "soldByRating": 4.8,
    "sellerSince": "2013",
    "fulfillment": "Seller fulfilled",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 91,
        "detail": null
      },
      "returnRate": {
        "score": 87,
        "detail": null
      },
      "warrantyClaims": {
        "score": 71,
        "detail": null
      },
      "sellerReliability": {
        "score": 86,
        "detail": null
      },
      "priceStability": {
        "score": 76,
        "detail": null
      }
    }
  },
  {
    "id": "dj188",
    "name": "Pampi Shoes",
    "brand": "Pampi",
    "category": "Fashion",
    "price": 2489,
    "originalPrice": 2894,
    "discount": 14,
    "rating": 3.1,
    "reviewCount": 10578,
    "inStock": true,
    "isPrime": false,
    "delivery": "Get it by Day after Tomorrow",
    "deliveryFree": true,
    "trustScore": 76,
    "trustLabel": "Genuine",
    "buyNowOrWait": "buy",
    "waitReason": null,
    "spikePriceMonths": [
      9
    ],
    "priceHistory": [
      2489,
      2656,
      2489,
      2489,
      2680,
      2489,
      2489,
      2489,
      2489,
      2962,
      2522,
      2489
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/womens-shoes/pampi-shoes/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/womens-shoes/pampi-shoes/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/womens-shoes/pampi-shoes/1.webp",
      "https://cdn.dummyjson.com/product-images/womens-shoes/pampi-shoes/2.webp"
    ],
    "description": "Pampi Shoes offer a blend of comfort and style for everyday use. With a versatile design, they are suitable for various casual occasions, providing a trendy and relaxed look.",
    "features": [
      "Pampi Shoes offer a blend of comfort and style for everyday use",
      "Upper: vegan leather — on-trend and long-lasting",
      "Padded ankle strap for all-day comfort",
      "Stiletto heel — 4cm height for comfortable all-day wear",
      "Non-slip rubber outsole — safe and steady on all surfaces"
    ],
    "specs": {
      "Upper Material": "Vegan Leather",
      "Heel Type": "Wedge",
      "Heel Height": "4 cm",
      "Sole Material": "Non-slip Rubber",
      "Closure": "Buckle Strap",
      "Sizes": "UK 3 – UK 8",
      "Warranty": "6 Months against manufacturing defects"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Pampi",
    "soldByRating": 4.6,
    "sellerSince": "2012",
    "fulfillment": "Fulfilled by Amazon",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 75,
        "detail": null
      },
      "returnRate": {
        "score": 87,
        "detail": null
      },
      "warrantyClaims": {
        "score": 78,
        "detail": null
      },
      "sellerReliability": {
        "score": 90,
        "detail": null
      },
      "priceStability": {
        "score": 81,
        "detail": null
      }
    }
  },
  {
    "id": "dj189",
    "name": "Red Shoes",
    "brand": "Fashion Express",
    "category": "Fashion",
    "price": 2904,
    "originalPrice": 3541,
    "discount": 18,
    "rating": 3.3,
    "reviewCount": 3783,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Day after Tomorrow",
    "deliveryFree": true,
    "trustScore": 79,
    "trustLabel": "Genuine",
    "buyNowOrWait": "wait",
    "waitReason": "Sale expected in the next 2 weeks based on historical patterns.",
    "spikePriceMonths": [
      9
    ],
    "priceHistory": [
      2904,
      2904,
      2904,
      2904,
      2904,
      2904,
      2904,
      2904,
      2904,
      3841,
      2904,
      2904
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/womens-shoes/red-shoes/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/womens-shoes/red-shoes/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/womens-shoes/red-shoes/1.webp",
      "https://cdn.dummyjson.com/product-images/womens-shoes/red-shoes/2.webp"
    ],
    "description": "The Red Shoes make a bold statement with their vibrant red color. Whether for a party or a casual outing, these shoes add a pop of color and style to your wardrobe.",
    "features": [
      "The Red Shoes make a bold statement with their vibrant red color",
      "Upper: genuine leather — on-trend and long-lasting",
      "Gel-cushion sole for all-day comfort",
      "Stiletto heel — 5cm height for comfortable all-day wear",
      "Non-slip rubber outsole — safe and steady on all surfaces"
    ],
    "specs": {
      "Upper Material": "Genuine Leather",
      "Heel Type": "Wedge",
      "Heel Height": "4 cm",
      "Sole Material": "Leather",
      "Closure": "Slip-on",
      "Sizes": "UK 3 – UK 8",
      "Warranty": "6 Months against manufacturing defects"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Fashion Express",
    "soldByRating": 4.4,
    "sellerSince": "2012",
    "fulfillment": "Seller fulfilled",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 66,
        "detail": null
      },
      "returnRate": {
        "score": 89,
        "detail": null
      },
      "warrantyClaims": {
        "score": 85,
        "detail": null
      },
      "sellerReliability": {
        "score": 94,
        "detail": null
      },
      "priceStability": {
        "score": 90,
        "detail": null
      }
    }
  },
  {
    "id": "dj190",
    "name": "IWC Ingenieur Automatic Steel",
    "brand": "IWC",
    "category": "Fashion",
    "price": 414999,
    "originalPrice": 456043,
    "discount": 9,
    "rating": 2.9,
    "reviewCount": 70190,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Tomorrow, 6 PM",
    "deliveryFree": true,
    "trustScore": 75,
    "trustLabel": "Mixed",
    "buyNowOrWait": "buy",
    "waitReason": null,
    "spikePriceMonths": [],
    "priceHistory": [
      414999,
      414999,
      414999,
      414999,
      414999,
      414999,
      414999,
      414999,
      414999,
      414999,
      414999,
      414999
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/womens-watches/iwc-ingenieur-automatic-steel/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/womens-watches/iwc-ingenieur-automatic-steel/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/womens-watches/iwc-ingenieur-automatic-steel/1.webp",
      "https://cdn.dummyjson.com/product-images/womens-watches/iwc-ingenieur-automatic-steel/2.webp"
    ],
    "description": "The IWC Ingenieur Automatic Steel watch is a durable and sophisticated timepiece. With a stainless steel case and automatic movement, it combines precision and style for watch enthusiasts.",
    "features": [
      "The IWC Ingenieur Automatic Steel watch is a durable and sophisticated timepiece",
      "Case: rose gold-plated stainless steel — 36mm delicate profile",
      "Kinetic movement — precision timekeeping",
      "Stone-studded bezel for a refined aesthetic",
      "Strap: silicone sport band — comfortable for everyday wear"
    ],
    "specs": {
      "Case Material": "Rose Gold-plated Stainless Steel",
      "Case Diameter": "36mm",
      "Movement": "Kinetic",
      "Water Resistance": "5 ATM",
      "Strap Material": "Silicone Sport Band",
      "Dial Feature": "Minimalist Clean Dial",
      "Warranty": "1 Year Manufacturer Warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "IWC",
    "soldByRating": 4.7,
    "sellerSince": "2019",
    "fulfillment": "Fulfilled by Amazon",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 66,
        "detail": null
      },
      "returnRate": {
        "score": 76,
        "detail": null
      },
      "warrantyClaims": {
        "score": 75,
        "detail": null
      },
      "sellerReliability": {
        "score": 80,
        "detail": null
      },
      "priceStability": {
        "score": 64,
        "detail": null
      }
    }
  },
  {
    "id": "dj191",
    "name": "Rolex Cellini Moonphase",
    "brand": "Rolex",
    "category": "Fashion",
    "price": 1327999,
    "originalPrice": 1383332,
    "discount": 4,
    "rating": 3.8,
    "reviewCount": 30644,
    "inStock": true,
    "isPrime": true,
    "delivery": "Get it by Day after Tomorrow",
    "deliveryFree": true,
    "trustScore": 83,
    "trustLabel": "Genuine",
    "buyNowOrWait": "wait",
    "waitReason": "Sale expected in the next 2 weeks based on historical patterns.",
    "spikePriceMonths": [],
    "priceHistory": [
      1327999,
      1327999,
      1327999,
      1327999,
      1327999,
      1327999,
      1327999,
      1327999,
      1327999,
      1327999,
      1327999,
      1327999
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/womens-watches/rolex-cellini-moonphase/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/womens-watches/rolex-cellini-moonphase/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/womens-watches/rolex-cellini-moonphase/1.webp",
      "https://cdn.dummyjson.com/product-images/womens-watches/rolex-cellini-moonphase/2.webp"
    ],
    "description": "The Rolex Cellini Moonphase watch is a masterpiece of horology. Featuring a moon phase complication, it showcases the craftsmanship and elegance that Rolex is renowned for.",
    "features": [
      "The Rolex Cellini Moonphase watch is a masterpiece of horology",
      "Case: rose gold-plated stainless steel — 36mm delicate profile",
      "Solar-powered quartz — precision timekeeping",
      "Mother-of-pearl dial for a refined aesthetic",
      "Strap: genuine leather — comfortable for everyday wear"
    ],
    "specs": {
      "Case Material": "Rose Gold-plated Stainless Steel",
      "Case Diameter": "36mm",
      "Movement": "Solar-powered Quartz",
      "Water Resistance": "Splash-proof",
      "Strap Material": "Genuine Leather",
      "Dial Feature": "Stone-studded Bezel",
      "Warranty": "1 Year Manufacturer Warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Rolex",
    "soldByRating": 3.9,
    "sellerSince": "2013",
    "fulfillment": "Seller fulfilled",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 87,
        "detail": null
      },
      "returnRate": {
        "score": 93,
        "detail": null
      },
      "warrantyClaims": {
        "score": 79,
        "detail": null
      },
      "sellerReliability": {
        "score": 96,
        "detail": null
      },
      "priceStability": {
        "score": 89,
        "detail": null
      }
    }
  },
  {
    "id": "dj192",
    "name": "Rolex Datejust Women",
    "brand": "Rolex",
    "category": "Fashion",
    "price": 912999,
    "originalPrice": 1086904,
    "discount": 16,
    "rating": 2.9,
    "reviewCount": 17289,
    "inStock": true,
    "isPrime": false,
    "delivery": "Get it by Day after Tomorrow",
    "deliveryFree": true,
    "trustScore": 78,
    "trustLabel": "Genuine",
    "buyNowOrWait": "wait",
    "waitReason": "Sale expected in the next 2 weeks based on historical patterns.",
    "spikePriceMonths": [],
    "priceHistory": [
      912999,
      912999,
      912999,
      912999,
      912999,
      912999,
      912999,
      963601,
      912999,
      912999,
      912999,
      912999
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/womens-watches/rolex-datejust-women/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/womens-watches/rolex-datejust-women/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/womens-watches/rolex-datejust-women/1.webp",
      "https://cdn.dummyjson.com/product-images/womens-watches/rolex-datejust-women/2.webp"
    ],
    "description": "The Rolex Datejust Women's watch is an iconic timepiece designed for women. With a timeless design and a date complication, it offers both elegance and functionality.",
    "features": [
      "The Rolex Datejust Women's watch is an iconic timepiece designed for women",
      "Case: silver-tone alloy — 36mm delicate profile",
      "Japanese quartz movement — precision timekeeping",
      "Embellished with Swarovski crystals for a refined aesthetic",
      "Strap: stainless mesh — comfortable for everyday wear"
    ],
    "specs": {
      "Case Material": "Silver-tone Alloy",
      "Case Diameter": "36mm",
      "Movement": "Japanese Quartz",
      "Water Resistance": "3 ATM",
      "Strap Material": "Stainless Mesh",
      "Dial Feature": "Stone-studded Bezel",
      "Warranty": "1 Year Manufacturer Warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Rolex",
    "soldByRating": 4.5,
    "sellerSince": "2014",
    "fulfillment": "Fulfilled by Amazon",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 75,
        "detail": null
      },
      "returnRate": {
        "score": 76,
        "detail": null
      },
      "warrantyClaims": {
        "score": 78,
        "detail": null
      },
      "sellerReliability": {
        "score": 91,
        "detail": null
      },
      "priceStability": {
        "score": 84,
        "detail": null
      }
    }
  },
  {
    "id": "dj193",
    "name": "Watch Gold for Women",
    "brand": "Fashion Gold",
    "category": "Fashion",
    "price": 66399,
    "originalPrice": 80974,
    "discount": 18,
    "rating": 4.2,
    "reviewCount": 38345,
    "inStock": false,
    "isPrime": true,
    "delivery": "Get it by Tomorrow, 6 PM",
    "deliveryFree": true,
    "trustScore": 93,
    "trustLabel": "Genuine",
    "buyNowOrWait": "buy",
    "waitReason": null,
    "spikePriceMonths": [
      5,
      10
    ],
    "priceHistory": [
      66399,
      66399,
      66399,
      66399,
      72627,
      74459,
      66399,
      66399,
      66399,
      66399,
      75841,
      66399
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/womens-watches/watch-gold-for-women/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/womens-watches/watch-gold-for-women/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/womens-watches/watch-gold-for-women/1.webp",
      "https://cdn.dummyjson.com/product-images/womens-watches/watch-gold-for-women/2.webp"
    ],
    "description": "The Gold Women's Watch is a stunning accessory that combines luxury and style. Featuring a gold-plated case and a chic design, it adds a touch of glamour to any outfit.",
    "features": [
      "The Gold Women's Watch is a stunning accessory that combines luxury and style",
      "Case: gold-plated brass — 36mm delicate profile",
      "Solar-powered quartz — precision timekeeping",
      "Stone-studded bezel for a refined aesthetic",
      "Strap: silicone sport band — comfortable for everyday wear"
    ],
    "specs": {
      "Case Material": "Gold-plated Brass",
      "Case Diameter": "36mm",
      "Movement": "Solar-powered Quartz",
      "Water Resistance": "5 ATM",
      "Strap Material": "Silicone Sport Band",
      "Dial Feature": "Mother-of-Pearl Dial",
      "Warranty": "1 Year Manufacturer Warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Fashion Gold",
    "soldByRating": 4.4,
    "sellerSince": "2015",
    "fulfillment": "Fulfilled by Amazon",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 96,
        "detail": null
      },
      "returnRate": {
        "score": 92,
        "detail": null
      },
      "warrantyClaims": {
        "score": 96,
        "detail": null
      },
      "sellerReliability": {
        "score": 96,
        "detail": null
      },
      "priceStability": {
        "score": 89,
        "detail": null
      }
    }
  },
  {
    "id": "dj194",
    "name": "Women's Wrist Watch",
    "brand": "Fashion Co.",
    "category": "Fashion",
    "price": 10789,
    "originalPrice": 12401,
    "discount": 13,
    "rating": 3.5,
    "reviewCount": 16766,
    "inStock": true,
    "isPrime": false,
    "delivery": "Get it by Day after Tomorrow",
    "deliveryFree": true,
    "trustScore": 87,
    "trustLabel": "Genuine",
    "buyNowOrWait": "wait",
    "waitReason": "Sale expected in the next 2 weeks based on historical patterns.",
    "spikePriceMonths": [
      0
    ],
    "priceHistory": [
      12597,
      11486,
      10789,
      10789,
      11489,
      11168,
      10639,
      10789,
      11564,
      10789,
      10789,
      10789
    ],
    "thumbnail": "https://cdn.dummyjson.com/product-images/womens-watches/women's-wrist-watch/thumbnail.webp",
    "images": [
      "https://cdn.dummyjson.com/product-images/womens-watches/women's-wrist-watch/thumbnail.webp",
      "https://cdn.dummyjson.com/product-images/womens-watches/women's-wrist-watch/1.webp",
      "https://cdn.dummyjson.com/product-images/womens-watches/women's-wrist-watch/2.webp"
    ],
    "description": "The Women's Wrist Watch is a versatile and fashionable timepiece for everyday wear. With a comfortable strap and a simple yet elegant design, it complements various styles.",
    "features": [
      "The Women's Wrist Watch is a versatile and fashionable timepiece for everyday wear",
      "Case: stainless steel 316L — 32mm delicate profile",
      "Japanese quartz movement — precision timekeeping",
      "Embellished with Swarovski crystals for a refined aesthetic",
      "Strap: stainless mesh — comfortable for everyday wear"
    ],
    "specs": {
      "Case Material": "Stainless Steel 316L",
      "Case Diameter": "32mm",
      "Movement": "Japanese Quartz",
      "Water Resistance": "3 ATM",
      "Strap Material": "Stainless Mesh",
      "Dial Feature": "Swarovski Crystal Embellishment",
      "Warranty": "1 Year Manufacturer Warranty"
    },
    "witnesses": [],
    "reviews": [],
    "soldBy": "Fashion Co.",
    "soldByRating": 4.6,
    "sellerSince": "2014",
    "fulfillment": "Seller fulfilled",
    "trustBreakdown": {
      "reviewAuthenticity": {
        "score": 96,
        "detail": null
      },
      "returnRate": {
        "score": 96,
        "detail": null
      },
      "warrantyClaims": {
        "score": 79,
        "detail": null
      },
      "sellerReliability": {
        "score": 87,
        "detail": null
      },
      "priceStability": {
        "score": 90,
        "detail": null
      }
    }
  }
];
