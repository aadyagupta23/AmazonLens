...
// Call Groq, auto-retry with fast model on 429 (pass noFallback:true to skip retry)
export async function groqCall({ messages, max_tokens = 300, temperature = 0.2, model = FAST_MODEL, noFallback = false }) {
  const groq = getGroq();
  ...
  try {
    ...
    if (is429 && model !== FAST_MODEL && !noFallback) {
